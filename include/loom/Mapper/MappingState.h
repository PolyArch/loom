...
template <>
struct DenseMapInfo<loom::MappingState::TaggedObservationKey> {
  static inline loom::MappingState::TaggedObservationKey getEmptyKey() {
    return {loom::MappingState::TaggedObservationKind::RoutingOutput,
            loom::INVALID_ID, loom::INVALID_ID, 0};
  }

  static inline loom::MappingState::TaggedObservationKey getTombstoneKey() {
    return {loom::MappingState::TaggedObservationKind::HardwareEdge,
            loom::INVALID_ID - 1, loom::INVALID_ID - 1, 0};
  }

  static unsigned getHashValue(
      const loom::MappingState::TaggedObservationKey &key) {
    return hash_combine(static_cast<unsigned>(key.kind), key.first, key.second,
                        key.tag);
  }

  static bool isEqual(const loom::MappingState::TaggedObservationKey &lhs,
                      const loom::MappingState::TaggedObservationKey &rhs) {
    return lhs == rhs;
  }
};

template <>
struct DenseMapInfo<loom::MappingState::TemporalRouteGroupKey> {
  static inline loom::MappingState::TemporalRouteGroupKey getEmptyKey() {
    return {loom::INVALID_ID, 0};
  }

  static inline loom::MappingState::TemporalRouteGroupKey getTombstoneKey() {
    return {loom::INVALID_ID - 1, 0};
  }

  static unsigned getHashValue(
      const loom::MappingState::TemporalRouteGroupKey &key) {
    return hash_combine(key.nodeId, key.tag);
  }

  static bool isEqual(const loom::MappingState::TemporalRouteGroupKey &lhs,
                      const loom::MappingState::TemporalRouteGroupKey &rhs) {
    return lhs == rhs;
  }
};
