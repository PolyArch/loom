...
inline std::optional<unsigned> getScalarWidth(mlir::Type t) {
  if (auto bits = mlir::dyn_cast<loom::fabric::BitsType>(t))
...
inline std::optional<PortTypeInfo> getPortTypeInfo(mlir::Type t) {
  if (auto tagged = mlir::dyn_cast<loom::fabric::TaggedType>(t)) {
...
inline bool canMapSoftwareTypeToBridgeHardware(mlir::Type swType,
  ...
  auto taggedHwType = mlir::dyn_cast<loom::fabric::TaggedType>(hwType);
  if (!taggedHwType)
    return false;
