...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--fcc", default="")
...
def load_cases(manifest_path: Path) -> dict[str, UnitCase]:
    ...
    with manifest_path.open(newline="", encoding="utf-8") as handle:
        ...
        for row in reader:
            ...
            while len(row) < 7:
                row.append("")
            name, _output_dir, dfg_file, fabric_file, builder_exec, generated_fabric_file, fcc_exec = row[:7]
...
def load_fcc_args(unit_dir: Path) -> list[str]:
    args_file = unit_dir / "fcc.args"
...
def run_case_mode(case: UnitCase, mode: str, fcc_path: Path,
    ...
    extra_args = strip_mapper_base_config_args(load_fcc_args(case.unit_dir))
    cmd = [
        str(fcc_path),
        "--adg",
        str(adg_path),
...
def main() -> int:
    ...
    if args.fcc:
        fcc_path = Path(args.fcc)
    else:
        fcc_path = next(iter(all_cases.values())).fcc_exec

    rows: list[dict[str, Any]] = []
    for mode in requested_modes:
        for case_name in requested_cases:
            row = run_case_mode(all_cases[case_name], mode, fcc_path,
