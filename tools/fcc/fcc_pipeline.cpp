...
compileOneSource(const std::string &srcPath, llvm::LLVMContext &llvmCtx,
  ...
  driverArgs.push_back(FCC_CLANG_RESOURCE_DIR);
  ...
  if (!cc1Cmd) {
    llvm::errs() << "fcc: no cc1 job found\n";
    return nullptr;
  }
  ...
  if (!compiler.ExecuteAction(*action)) {
    llvm::errs() << "fcc: clang failed for " << srcPath << "\n";
    return nullptr;
  }
...
OwningOpRef<ModuleOp> compileAndImport(const FccArgs &args,
                                        MLIRContext &ctx,
  ...
  for (const auto &src : args.sources) {
    ...
    if (!linkedModule) {
      ...
      if (llvm::Linker::linkModules(*linkedModule, std::move(mod))) {
        llvm::errs() << "fcc: failed to link modules\n";
        return nullptr;
      }
    }
  }
  ...
  if (!mlirModule) {
    llvm::errs() << "fcc: MLIR import failed\n";
    return nullptr;
  }
...
LogicalResult runLLVMToCF(ModuleOp module) {
  ...
  pm.addPass(fcc::createConvertLLVMToCFPass());
...
LogicalResult runCFToSCF(ModuleOp module) {
  ...
  pm.addPass(fcc::createUpliftWhileToForPass());
  pm.addPass(createCanonicalizerPass());
  pm.addPass(createCSEPass());
  return pm.run(module);
}
...
LogicalResult runSCFToDFG(ModuleOp module) {
  PassManager pm(module.getContext());
  pm.addPass(fcc::createMarkDFGDomainPass());
  pm.addPass(fcc::createConvertSCFToDFGPass());
  pm.addPass(createCanonicalizerPass());
  pm.addPass(createCSEPass());
  return pm.run(module);
}
...
LogicalResult runHostCodeGen(ModuleOp module, const std::string &outputPath,
  ...
  PassManager pm(module.getContext());
  pm.addPass(fcc::createHostCodeGenPass(outputPath, originalSource));
  return pm.run(module);
}
...
LogicalResult writeMLIR(ModuleOp module, const std::string &path) {
  ...
  if (ec) {
    llvm::errs() << "fcc: cannot open " << path << ": " << ec.message()
                 << "\n";
    return failure();
  }
