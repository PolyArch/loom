...
void applyMapperCliOverride(llvm::StringRef optionName, const T &cliValue,
  ...
  if (mapperValuesDiffer(mappedValue, promotedValue)) {
    errs() << "loom: warning: CLI option --" << optionName
...
bool parseArgs(int argc, char **argv, LoomArgs &args) {
  cl::ParseCommandLineOptions(argc, argv, "loom - fabric compiler\n");
  ...
  if (vizLayoutOpt == "default")
    ...
    errs() << "loom: --viz-layout must be 'default' or 'neato'\n";
    return false;
  }
  ...
  if (args.mapperResolvedBaseConfigPath.empty()) {
    errs() << "loom: no default mapper base config path is available\n";
    return false;
  }
  ...
  if (std::error_code ec = sys::fs::create_directories(args.outputDir)) {
    errs() << "loom: cannot create output directory '" << args.outputDir
           << "': " << ec.message() << "\n";
    return false;
  }
