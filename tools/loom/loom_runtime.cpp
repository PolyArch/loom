...
static OwningOpRef<ModuleOp> loadModule(const std::string &path,
  ...
  if (!buffer) {
    llvm::errs() << "loom: cannot open MLIR file: " << path << "\n";
    return {};
  }
...
static bool writeJsonToFile(const Value &value, const std::string &path) {
  ...
  if (ec) {
    llvm::errs() << "loom: failed to open " << path << ": " << ec.message()
                 << "\n";
    return false;
  }
...
static bool loadRuntimeConfigSlices(
  ...
  if (!manifest.simImageJsonPath.empty()) {
    loaded = loom::sim::loadRuntimeImageJson(manifest.simImageJsonPath,
                                            runtimeImage, error);
  } else if (!manifest.simImageBinPath.empty()) {
    loaded = loom::sim::loadRuntimeImageBinary(manifest.simImageBinPath,
                                              runtimeImage, error);
  } else {
  ...
  for (const auto &slice : runtimeImage.configImage.slices) {
    loom::ConfigGen::ConfigSlice converted;
...
int runRuntimeReplay(const LoomArgs &args, MLIRContext &context) {
  RuntimeManifestData manifest;
  ...
  if (!loadRuntimeManifest(args.runtimeManifestPath, manifest, error)) {
    llvm::errs() << "loom: runtime replay failed to load manifest: " << error
                 << "\n";
    return 1;
  }
  if (!loadRuntimeRequest(args.runtimeRequestPath, request, error)) {
    llvm::errs() << "loom: runtime replay failed to load request: " << error
                 << "\n";
    return 1;
  }
  if (!loadRuntimeConfigSlices(manifest, configSlices, error)) {
    llvm::errs() << "loom: runtime replay failed to load config slices: "
                 << error << "\n";
    return 1;
  }
  if (!manifest.simImageJsonPath.empty()) {
    if (!loom::sim::loadRuntimeImageJson(manifest.simImageJsonPath, runtimeImage,
                                        error)) {
      llvm::errs() << "loom: runtime replay failed to load sim image: " << error
                   << "\n";
      return 1;
    }
  } else if (!manifest.simImageBinPath.empty()) {
    if (!loom::sim::loadRuntimeImageBinary(manifest.simImageBinPath, runtimeImage,
                                          error)) {
      llvm::errs() << "loom: runtime replay failed to load sim image: " << error
                   << "\n";
      return 1;
    ...
    llvm::errs() << "loom: runtime replay manifest missing sim image path\n";
    return 1;
  }
  ...
  if (failed(verifyFabricModule(*adgModule))) {
    llvm::errs() << "loom: runtime replay ADG verification failed\n";
    return 1;
  }

  ADGFlattener flattener;
  if (!flattener.flatten(*adgModule, &context)) {
    llvm::errs() << "loom: runtime replay ADG flattening failed\n";
    return 1;
  }
  DFGBuilder dfgBuilder;
  if (!dfgBuilder.build(*dfgModule, &context)) {
    llvm::errs() << "loom: runtime replay DFG build failed\n";
    return 1;
  }
  ...
  if (!request.configWords.empty()) {
    ...
    llvm::errs() << "loom: runtime replay failed to load config: " << error
                 << "\n";
    return 1;
  }

  if (std::string err = session.connect(); !err.empty()) {
    llvm::errs() << "loom: runtime replay connect failed: " << err << "\n";
    return 1;
  }
  if (std::string err = session.buildFromStaticModel(runtimeImage.staticModel);
      !err.empty()) {
    llvm::errs() << "loom: runtime replay static model build failed: " << err
                 << "\n";
    return 1;
  }
  if (std::string err = session.loadConfig(configBlob, configSlices);
      !err.empty()) {
    llvm::errs() << "loom: runtime replay config load failed: " << err << "\n";
    return 1;
  }
  ...
  if (manifest.startTokenPortIdx != std::numeric_limits<unsigned>::max() &&
    ...
    if (std::string err =
        ...
        !err.empty()) {
      llvm::errs() << "loom: runtime replay start-token bind failed: " << err
                   << "\n";
      return 1;
    }
  }
  ...
  for (const RuntimeScalarBinding &binding : manifest.scalarBindings) {
    ...
    if (std::string err =
        ...
        !err.empty()) {
      llvm::errs() << "loom: runtime replay scalar bind failed for slot "
                   << binding.slot << ": " << err << "\n";
      return 1;
    }
  }
  ...
  for (const RuntimeMemoryBinding &binding : manifest.memoryBindings) {
    ...
    if (it == memoryBySlot.end()) {
      llvm::errs() << "loom: runtime replay missing memory slot " << binding.slot
                   << "\n";
      return 1;
    }
    ...
    if (std::string err = session.setExtMemoryBacking(binding.regionId,
                                                      ...
                                                      bytes.size());
        !err.empty()) {
      llvm::errs() << "loom: runtime replay memory bind failed for slot "
                   << binding.slot << ": " << err << "\n";
      return 1;
    }
  }
  ...
  if (!success) {
    llvm::errs() << "loom: runtime replay failed";
    if (!invokeErr.empty())
      llvm::errs() << ": " << invokeErr;
