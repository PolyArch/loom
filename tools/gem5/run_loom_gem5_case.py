...
def emit_host_source(case_dir: pathlib.Path, gem5_dir: pathlib.Path, runtime: dict,
    ...
    for memory in bundle.get("memory_regions", []):
        ...
        memory_binds.append(
            f"  loom_accel_set_mem_region({slot}, {array_name}, sizeof({array_name}));"
        )
    ...
    for scalar in bundle.get("scalar_inputs", []):
        ...
        arg_lines.append(f"  loom_accel_set_arg({slot}, {int(data[0])}ULL);")
    ...
    for expected in bundle.get("expected_outputs", []):
        ...
        output_checks.extend([
            f"  if (loom_accel_output_count({slot}) != {len(values)})",
            "    { loom_uart_puts(\"FAIL\\n\");",
            f"      return 0x200 + {slot}; }}",
            f"  for (unsigned i = 0; i < {len(values)}; ++i) {{",
            "    unsigned short tag = 0;",
            f"    unsigned long long value = loom_accel_read_output({slot}, i, &tag);",
    ...
    lines = [
        ...
        "",
        "#define LOOM_UART0_BASE 0x10000000UL",
        "#define LOOM_UART_THR 0x0UL",
        "#define LOOM_UART_LSR 0x5UL",
        "#define LOOM_UART_LSR_THRE 0x20U",
        "",
        "static void loom_uart_putc(char ch) {",
        "  volatile unsigned char *uart =",
        "      (volatile unsigned char *)(uintptr_t)LOOM_UART0_BASE;",
        "  while ((uart[LOOM_UART_LSR] & LOOM_UART_LSR_THRE) == 0) {",
        "  }",
        "  uart[LOOM_UART_THR] = (unsigned char)ch;",
        "}",
        "",
        "static void loom_uart_puts(const char *text) {",
        "  while (*text) {",
        "    if (*text == '\\n')",
        "      loom_uart_putc('\\r');",
        "    loom_uart_putc(*text++);",
    ...
    if config_words:
        lines.append(
            "static uint32_t loom_runtime_config_words[] = "
            f"{{{', '.join(str(word) + 'u' for word in config_words)}}};"
        )
        lines.append(
            "static const unsigned loom_runtime_config_word_count = "
            f"{len(config_words)};"
        )
        lines.append("")
    ...
    if config_patch_entries:
        ...
        for entry in config_patch_entries:
            lines.append(
                "  loom_runtime_config_words[{word_index}] = "
    ...
    if config_words:
        lines.extend([
            "  if (loom_runtime_config_word_count > 0)",
            "    loom_accel_load_config(loom_runtime_config_words,",
    ...
    lines.extend([
        "  loom_accel_launch();",
        "  if (loom_accel_wait() != 0)",
        "    { loom_uart_puts(\"FAIL\\n\"); return 0x100 + (int)loom_accel_error_code(); }",
    ])
...
def run_gem5(case_dir: pathlib.Path, gem5_dir: pathlib.Path, host_elf: pathlib.Path,
    ...
    config_script = REPO_ROOT / "configs/loom_cgra.py"
