...
def matching_cgra_report_path(paths: list[Path], workload: str, hardware: str, mapping_id: str) -> Path | None:
    for path in paths:
        if cgra_report_matches(read_json(path), workload, hardware, mapping_id):
            return path
    return None
...
def matching_runtime_package_path(
    paths: list[Path],
    *,
    workload: str,
    hardware: str,
