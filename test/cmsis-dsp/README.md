...
| Path                              | Role                                                                              |
|-----------------------------------|-----------------------------------------------------------------------------------|
| `cmsis_dsp_targets.txt`           | One row per source: triple, cpu, expected triple/symbols, per-row DFG-shape gate. |
| `run_cmsis_dsp_ir.sh`             | Loops over rows, invokes loom-cc, asserts IR invariants.                          |
| `run_cmsis_dsp_raise.sh`          | LLVM IR -> SCF MLIR raise smoke.                                                  |
| `run_cmsis_dsp_dfg.sh`            | SCF MLIR -> DFG MLIR lower smoke with per-row shape gate.                         |
...
2. Append a pipe-separated row to `cmsis_dsp_targets.txt`:
   `src_relpath | triple | mcpu | normalized_triple | expected_symbols | extra_cflags | expect_thread | expect_graph | expect_stream | expect_load | expect_store | expect_constant | expect_sync | expect_gate | expect_mux | expect_demux | expect_scf_residual`
   The eleven `expect_*` cells pin the per-row DFG shape gate (counts of
   `dataflow.thread @`, `dataflow.graph.func @`, `dataflow.stream`,
   `dataflow.load`, `dataflow.store`, `dataflow.constant`,
   `dataflow.sync`, `dataflow.gate`, `dataflow.mux`, `dataflow.demux`,
   and residual `scf.*` ops). Use a bare integer for exact equality or
