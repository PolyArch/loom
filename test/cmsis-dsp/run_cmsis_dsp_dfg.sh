#!/usr/bin/env bash
# SCF MLIR -> DFG MLIR lower smoke for the same cmsis-dsp source list
# that run_cmsis_dsp_ir.sh / run_cmsis_dsp_raise.sh cover. For every
# row in cmsis_dsp_targets.txt the runner does:
#
...
# cmsis_dsp_dfg_skip.txt with an inline `# reason` comment. The skip
...
echo
echo "==== cmsis-dsp dfg smoke summary ===="
echo "  passed:  ${#passed[@]}"
echo "  failed:  ${#failed[@]}"
...
if (( ${#failed[@]} > 0 )); then
    echo
    echo "${#failed[@]} cmsis-dsp source(s) failed loom-lower: ${failed[*]}" >&2
    exit 1
fi

# Corpus-level safety net: at least one passing row must emit a
# dataflow.thread or dataflow.graph.func symbol. If none do, the
# lowering pipeline has regressed across the board and the runner
# should fail loudly instead of declaring a happy all-pass over a
# corpus full of structureless modules.
...
echo "all ${#passed[@]} cmsis-dsp source(s) passed loom-lower"
echo "${#with_emission[@]} of ${#passed[@]} cmsis-dsp source(s) emitted dataflow.thread or dataflow.graph.func"
