...
// The common addi is commutative, so graph-MCS should share it even when
// the input subgraphs present its block arguments in opposite operand order.
...
func.func @pat_direct_swapped(%a: i32, %b: i32, %c: i32) -> i32
    attributes {loom.synth_group = "commutative_permuted"} {
  %r = dataflow.subgraph(%x = %a : i32, %y = %b : i32,
                         %z = %c : i32) -> i32 {
    %u = arith.addi %y, %x : i32
    %v = arith.subi %u, %z : i32
