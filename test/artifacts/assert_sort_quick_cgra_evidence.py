#!/usr/bin/env python3
"""Assert row-complete sort_quick CGRA-sim evidence."""
...
COPY_GRAPH = "g_t_sort_quick_kernel_0_0"
PARTITION_GRAPH = "g_t_sort_quick_kernel_red_0_0"
...
def expected_memories() -> tuple[list[str], list[str]]:
    source = Path(__file__).resolve().parents[2] / "test/app/sort_quick/main_func.cpp"
    input_values = parse_float_array(source, "kInput")
    expected_values = parse_float_array(source, "kExpected")
    require(len(input_values) == 12, f"sort_quick graph fixture expects 12 inputs, saw {len(input_values)}")
    require(len(expected_values) == 12, f"sort_quick graph fixture expects 12 outputs, saw {len(expected_values)}")
...
def require_orchestration(artifact: dict[str, object], expected_partitions: int) -> None:
    orchestration = artifact.get("scalar_orchestration")
    require(isinstance(orchestration, dict), f"missing scalar orchestration metadata: {artifact}")
    require(orchestration.get("kind") == "host_control_with_cgra_partition_slices", f"bad orchestration: {orchestration}")
    require(orchestration.get("control_owner") == "scalar_host_fixture", f"bad control owner: {orchestration}")
    events = orchestration.get("events")
    require(isinstance(events, list), f"orchestration events must be a list: {orchestration}")
    require(len(events) == expected_partitions, f"partition event count mismatch: {events}")
    for index, event in enumerate(events):
        require(isinstance(event, dict), f"event {index} must be an object: {event}")
        require(event.get("component_index") == index + 1, f"event {index} component index mismatch: {event}")
        require(event.get("graph") == PARTITION_GRAPH, f"event {index} graph mismatch: {event}")
        require(isinstance(event.get("low"), int), f"event {index} lacks low: {event}")
        require(isinstance(event.get("high"), int), f"event {index} lacks high: {event}")
        require(isinstance(event.get("pivot_index"), int), f"event {index} lacks pivot index: {event}")
        require(event.get("host_memory_action") == "pivot_swap_after_partition", f"event {index} action mismatch: {event}")


def memory_values(memory: object) -> dict[str, list[str]]:
    require(isinstance(memory, dict), f"final memory state must be an object: {memory}")
    typed: dict[str, list[str]] = {}
    for key, value in memory.items():
        require(isinstance(key, str), f"bad memory key: {key!r}")
        require(isinstance(value, list), f"bad memory value for {key}: {value!r}")
        typed[key] = value
    return typed


def require_memory_for_identity(
    ...
    candidates = [f"{identity}:{arg_name}"]
    if "." in identity:
        candidates.append(f"{identity.split('.', 1)[1]}:{arg_name}")
    for key in candidates:
        if memory.get(key) == expected:
            return
    raise AssertionError(f"{label} memory mismatch for {identity}:{arg_name}: {memory}")
...
def main(argv: list[str]) -> int:
    ...
    dfg = read_json(evidence / "sort_quick.dfg.report.json")
    mapping = read_json(evidence / "sort_quick.mapping.json")
    cgra = read_json(evidence / "sort_quick.cgra.report.json")
    comparison = read_json(evidence / "sort_quick.sim-comparison-report.json")
    ...
    require(isinstance(graphs, list), f"DFG graph coverage must be a list: {dfg}")
    require(graphs[0] == COPY_GRAPH, f"copy graph must be first: {graphs}")
    partition_graphs = graphs[1:]
    require(partition_graphs, f"sort_quick must execute at least one partition graph: {graphs}")
    require(all(graph == PARTITION_GRAPH for graph in partition_graphs), f"unexpected partition graph list: {graphs}")
    require(mapping.get("component_graphs") == graphs, f"mapping graph coverage mismatch: {mapping}")
    require(cgra.get("component_graphs") == graphs, f"CGRA graph coverage mismatch: {cgra}")

    expected_count = len(graphs)
    require_orchestration(dfg, len(partition_graphs))
    require_orchestration(cgra, len(partition_graphs))
    ...
    dfg_ids = require_component_identities(evidence, dfg, "component_dfg_sim_report_identities", expected_count)
    require_component_identities(evidence, mapping, "component_mapping_artifact_identities", expected_count)
    require_component_identities(evidence, cgra, "component_dfg_sim_report_identities", expected_count)
    require_component_identities(evidence, cgra, "component_cgra_sim_report_identities", expected_count)

    dfg_memory = memory_values(dfg.get("final_memory_state"))
    cgra_memory = memory_values(cgra.get("final_memory_state"))
    require(cgra_memory == dfg_memory, f"CGRA must carry DFG memory: {cgra}")
    require_memory_for_identity(dfg_memory, dfg_ids[0], "arg2", expected_input, "copy output")
    require_memory_for_identity(
        dfg_memory,
        dfg_ids[-1],
        "arg4",
        expected_sorted,
        "last partition output",
    )
    require(
