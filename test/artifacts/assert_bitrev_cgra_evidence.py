#!/usr/bin/env python3
"""Assert row-complete bitrev CGRA-sim evidence."""
...
def expected_tokens() -> list[str]:
    ...
    for index in range(SIZE):
        ...
        while mask > 0:
            ...
            current >>= 1
            mask >>= 1
        output[reversed_index] = float(index)
    return [f"f32:{int(value)}" if value.is_integer() else f"f32:{value:.6f}" for value in output]
...
def main(argv: list[str]) -> int:
    ...
    dfg = read_json(evidence / "bitrev.dfg.report.json")
    mapping = read_json(evidence / "bitrev.mapping.json")
    cgra = read_json(evidence / "bitrev.cgra.report.json")
    comparison = read_json(evidence / "bitrev.sim-comparison-report.json")
    ...
    require(fire_counts.get("dataflow.load") == SIZE, f"bitrev should load {SIZE} elements: {fire_counts}")
    require(fire_counts.get("dataflow.store") == SIZE, f"bitrev should store {SIZE} elements: {fire_counts}")
    require_memory(dfg, "arg2", expected, "DFG")
    require_memory(cgra, "arg2", expected, "CGRA")
    ...
    actual = [float(token.split(":", 1)[1]) for token in expected]
    checksum = sum(float(index + 1) * value for index, value in enumerate(actual))
    require(math.isclose(checksum, 538592.0, rel_tol=0.0, abs_tol=1.0e-6), f"checksum mismatch: {checksum}")
    return 0
