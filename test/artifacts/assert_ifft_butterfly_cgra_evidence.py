#!/usr/bin/env python3
"""Assert row-complete ifft_butterfly CGRA-sim evidence."""
...
import ifft_butterfly_fixtures


GRAPH_COPY = "g_t_ifft_butterfly_kernel_0_0"
GRAPH_BUTTERFLY = "g_t_ifft_butterfly_kernel_red_0_0"
GRAPH_SCALE = "g_t_ifft_butterfly_kernel_1_0"
EXPECTED_COMPONENTS = 64
EXPECTED_WORK_ITEMS = 64
TOLERANCE = 1.0e-4
...
def main(argv: list[str]) -> int:
    ...
    fixture = ifft_butterfly_fixtures.fixture_from_source()

    dfg = read_json(evidence / "ifft_butterfly.dfg.report.json")
    mapping = read_json(evidence / "ifft_butterfly.mapping.json")
    cgra = read_json(evidence / "ifft_butterfly.cgra.report.json")
    comparison = read_json(evidence / "ifft_butterfly.sim-comparison-report.json")
    ...
    require(graphs.count(GRAPH_SCALE) == 16, f"scale component count mismatch: {graphs}")
    ...
    final_real = memory_by_suffix(dfg.get("final_memory_state"), "scale15.report:arg1")
    final_imag = memory_by_suffix(dfg.get("final_memory_state"), "scale15.report:arg3")
