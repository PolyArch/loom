...
def assert_app_cgra_sweep_mode(repo: Path, out_dir: Path, legacy_root: Path) -> None:
    ...
    assert_loombench_cgra_pass_row(repo, rows, "kmp_table", expected_hardware="shared_memory_reduction_adg")
...
def assert_app_seed_batch_mode(repo: Path, out_dir: Path) -> None:
    ...
    write_legacy_case(legacy_root, "cross_product")
    write_legacy_case(legacy_root, "quat_mult")
    write_legacy_case(legacy_root, "hist_bin")
    write_legacy_case(legacy_root, "batchnorm")
    write_legacy_case(legacy_root, "moving_avg")
    ...
    seed_rows = (
        ...
        ("cross_product", "shared_vector_math_adg"),
        ("quat_mult", "shared_vector_math_adg"),
        ("hist_bin", "shared_signal_window_adg"),
        ("batchnorm", "shared_signal_window_adg"),
        ("moving_avg", "shared_signal_window_adg"),
        ...
        ("edge_update", "shared_memory_reduction_adg"),
        ("edge_update_batch", "shared_memory_reduction_adg"),
        ...
        ("compare_swap", "shared_reduction_adg"),
        ("cumsum", "shared_reduction_adg"),
        ("dotprod", "shared_reduction_adg"),
        ("mean", "shared_reduction_adg"),
        ("bitrev", "shared_memory_reduction_adg"),
        ("conv2d", "shared_memory_reduction_adg"),
        ("depthwise_conv", "shared_memory_reduction_adg"),
        ("interpolate_linear", "shared_signal_window_adg"),
        ("jacobi_stencil_5pt", "shared_signal_window_adg"),
        ("normalize_vec3", "shared_signal_window_adg"),
        ("bitrev_complex", "shared_memory_reduction_adg"),
        ("bisection_step", "shared_memory_reduction_adg"),
        ("im2col", "shared_memory_reduction_adg"),
        ("kmp_table", "shared_memory_reduction_adg"),
        ("distance_point", "shared_signal_window_adg"),
        ("window_blackman", "shared_signal_window_adg"),
        ("window_hamming", "shared_signal_window_adg"),
        ("window_hanning", "shared_signal_window_adg"),
        ("convolve_1d", "shared_reduction_adg"),
        ("convolve_1d_same", "shared_reduction_adg"),
        ("conv1d", "shared_reduction_adg"),
        ("fir_filter", "shared_reduction_adg"),
    ...
    assert_loombench_cgra_pass_row(repo, rows, "database_join", expected_hardware="shared_memory_reduction_adg")
    assert_loombench_cgra_pass_row(repo, rows, "cross_product", expected_hardware="shared_vector_math_adg")
    assert_loombench_cgra_pass_row(repo, rows, "quat_mult", expected_hardware="shared_vector_math_adg")
    assert_loombench_cgra_pass_row(repo, rows, "hist_bin", expected_hardware="shared_signal_window_adg")
    assert_loombench_cgra_pass_row(repo, rows, "batchnorm", expected_hardware="shared_signal_window_adg")
    assert_loombench_cgra_pass_row(repo, rows, "moving_avg", expected_hardware="shared_signal_window_adg")
...
def assert_seed_batch_candidate_evidence(evidence_dir: Path) -> None:
    ...
    cgra_sweep.assert_cross_product_evidence(evidence_dir)
    cgra_sweep.assert_quat_mult_evidence(evidence_dir)
    cgra_sweep.assert_hist_bin_evidence(evidence_dir)
    cgra_sweep.assert_moving_avg_evidence(evidence_dir)
    ...
    run(
        Path(__file__).resolve().parents[2],
        ["python3", "test/artifacts/assert_batchnorm_cgra_evidence.py", str(evidence_dir)],
    )
...
def assert_cmsis_softmax_u8_cgra_evidence(
    ...
    if (
        ...
        or dfg_report.get("operation_fire_counts", {}).get("arith.muli") != 88
        or dfg_report.get("operation_fire_counts", {}).get("llvm.trunc") != 91
        or dfg_report.get("final_outputs") != ["none"]
        or dfg_report.get("final_memory_state") != expected_memory
    ):
        raise AssertionError(f"unexpected {stem} DFG evidence: {dfg_report}")
...
def main() -> int:
    ...
    with artifact_test_common.repo_temp_dir(repo, "cmsis-cgra-status-rollup-") as raw_out_dir:
        ...
        write_legacy_case(legacy_root, "cdma")
        write_legacy_case(legacy_root, "line_intersect")
        write_legacy_case(legacy_root, "kmp_table")
        write_legacy_case(legacy_root, "database_join")
