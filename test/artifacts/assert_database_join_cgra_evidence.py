#!/usr/bin/env python3
"""Assert row-complete database_join CGRA-sim evidence."""
...
from database_join_fixtures import fixture_from_source


CASE = "database_join"
GRAPH = "g_t_database_join_kernel_red_0_0"
...
EXPECTED_ROUTE_EDGES = {
    ...
    "arith.index_cast#2.result0->dataflow.load#2.operand1",
    "arith.index_cast#3.result0->dataflow.load#3.operand1",
    "dataflow.load#0.result0->arith.cmpi#0.operand0",
    "dataflow.load#0.result0->dataflow.store#0.operand2",
    "dataflow.load#1.result0->arith.cmpi#0.operand1",
    "dataflow.load#2.result0->dataflow.store#1.operand2",
    "dataflow.load#3.result0->dataflow.store#2.operand2",
}
...
def require_memory(memory: object, expected_memory: dict[str, list[str]], label: str) -> None:
    ...
    require(memory.get("arg7") != ["i32:0"] * 5, f"{label} output ids should not be all zero")
    require(memory.get("arg8") != ["i32:0"] * 5, f"{label} output A values should not be all zero")
    require(memory.get("arg10") != ["i32:0"] * 5, f"{label} output B values should not be all zero")
...
def require_real_routes(mapping: dict[str, object]) -> None:
    ...
    require(edges == EXPECTED_ROUTE_EDGES, f"database_join route edge set changed: {edges}")
    saw_switch = False
    saw_load = False
    saw_store = False
    saw_buffer = False
    ...
    for route in routes:
        ...
        for segment in segments:
            ...
            segment_count += 1
    require(segment_count == 43, f"database_join should route 43 segments, got {segment_count}")
    require(saw_switch, "database_join routes should traverse a real switch")
    require(saw_load, "database_join routes should target real memory load ports")
    require(saw_store, "database_join routes should target real memory store ports")
    require(saw_buffer, "database_join routes should include real Fabric buffers")
    require(saw_module_path, "database_join routes should include Fabric module paths")
...
def main(argv: list[str]) -> int:
    ...
    require(
        ...
        and dfg.get("dynamic_work_items") == 3
        and dfg.get("optimistic_cycles") == 203
        and dfg.get("operation_fire_counts") == fixture.expected_fire_counts
        and dfg.get("final_outputs") == fixture.final_outputs,
        f"database_join DFG report should carry source-derived join evidence: {dfg}",
    )
    ...
    require(
        ...
        and mapping.get("placed_records") == 13
        and mapping.get("routed_edges") == 9
        and mapping.get("unrouted_edges") == 0
        and mapping.get("unplaced_records") == 0
        and mapping.get("config_records") == 254,
        f"database_join should map to shared memory-reduction ADG: {mapping}",
    )
    ...
    require(
        ...
        and cgra.get("dfg_cycles") == 203
        and cgra.get("hardware_aware_cycles") == 293
        and cgra.get("route_segments") == 43
        and cgra.get("config_records") == 254
        and cgra.get("placed_records") == 13
        and cgra.get("routed_edges") == 9
        and cgra.get("final_outputs") == fixture.final_outputs,
        f"database_join CGRA report should carry hardware-aware evidence: {cgra}",
    )
    require_memory(cgra.get("final_memory_state"), expected_memory, "CGRA")
    require(293 >= 203, "CGRA cycles must not be more optimistic than DFG")
    ...
    require(
        ...
        and comparison.get("dfg_sim_cycles") == 203
        and comparison.get("cgra_sim_cycles") == 293
        and comparison.get("performance_delta_cycles") == 90
        and comparison.get("difference_classification") == "expected_hardware_constraint",
        f"database_join comparison should pass real functional and memory checks: {comparison}",
    )
    return 0
