#!/usr/bin/env python3
"""Assert row-complete sort_bubble CGRA-sim evidence."""
...
EXPECTED_GRAPHS = [
    "g_t_sort_bubble_kernel_0_0",
    "g_t_sort_bubble_kernel_red_0_0",
]
...
def expected_memories() -> tuple[list[str], list[str]]:
    source = Path(__file__).resolve().parents[2] / "test/app/sort_bubble/main_func.cpp"
    input_values = parse_float_array(source, "kInput")
    expected_values = parse_float_array(source, "kExpected")
    require(len(input_values) == 12, f"sort_bubble graph fixture expects 12 inputs, saw {len(input_values)}")
    require(len(expected_values) == 12, f"sort_bubble graph fixture expects 12 expected values, saw {len(expected_values)}")
...
def main(argv: list[str]) -> int:
    ...
    dfg = read_json(evidence / "sort_bubble.dfg.report.json")
    mapping = read_json(evidence / "sort_bubble.mapping.json")
    cgra = read_json(evidence / "sort_bubble.cgra.report.json")
    comparison = read_json(evidence / "sort_bubble.sim-comparison-report.json")
    ...
    require(mapping.get("hardware") == "shared_memory_reduction_adg", f"unexpected hardware: {mapping}")
    ...
    require_memory(dfg.get("final_memory_state"), "g_t_sort_bubble_kernel_0_0:arg2", expected_input, "DFG")
    require_memory(cgra.get("final_memory_state"), "g_t_sort_bubble_kernel_0_0:arg2", expected_input, "CGRA")
    require_memory(dfg.get("final_memory_state"), "g_t_sort_bubble_kernel_red_0_0:arg5", expected_sorted, "DFG")
    require_memory(cgra.get("final_memory_state"), "g_t_sort_bubble_kernel_red_0_0:arg5", expected_sorted, "CGRA")
