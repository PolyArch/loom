#!/usr/bin/env python3
"""Source-derived fixtures for jacobi_stencil_5pt CGRA evidence."""
...
def parse_input(text: str) -> tuple[float, ...]:
    match = re.search(r"constexpr\s+std::array<float,\s*kSize>\s+kInput\s*=\s*\{(?P<body>.*?)\};", text, re.S)
    require(match is not None, "missing kInput array")
    values = []
    for token in re.findall(r"[-+]?[0-9]+(?:\.[0-9]+)?f?", match.group("body")):
...
def source_path() -> Path:
    return REPO_ROOT / "test" / "app" / "jacobi_stencil_5pt" / "main_func.cpp"


def fixture_from_source(source: Path | None = None) -> JacobiStencil5ptFixture:
    source = source or source_path()
    text = source.read_text()
    rows = parse_const(text, "kRows")
    cols = parse_const(text, "kCols")
    input_values = parse_input(text)
    require(len(input_values) == rows * cols, f"kInput length does not match {rows}x{cols}")
    ...
    return JacobiStencil5ptFixture(
        case="jacobi_stencil_5pt",
        graph="g_t_jacobi_stencil_5pt_kernel_0_0",
        input_arg=3,
        interior_arg=8,
        index_arg=9,
        scalar_args=(
...
def emit_dfg_args(fixture: JacobiStencil5ptFixture) -> None:
...
def emit_json(fixture: JacobiStencil5ptFixture) -> None:
    print(
        json.dumps(
            {
                "case": fixture.case,
                ...
                "checksum": math.fsum((index + 1) * value for index, value in enumerate(fixture.final_values)),
