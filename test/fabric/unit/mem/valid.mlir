...
// An operation engine may use a local service without external memory
// endpoints. The service capacity and behavior are explicit typed facts.

// CHECK-LABEL: fabric.mem @EngineAndLocalService
"fabric.mem"() <{
  function_type = (!fabric.bits<32>, !fabric.bits<0>)
      -> (!fabric.bits<32>, !fabric.bits<0>),
  hw_params = [{
    data_width = 32 : i32,
    load_group_size = 1 : i32,
  ...
  memory_contract = #fabric.memory_contract<
    ...
    subordinate_endpoints = []
  >,
  sym_name = "EngineAndLocalService"
}> : () -> ()

// A storage-only memory exposes subordinate storage through its local service
// and has no operation engine or schedule.

// CHECK-LABEL: fabric.mem @StorageOnly
"fabric.mem"() <{
