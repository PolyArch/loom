...
// RUN:   --memref 1=0x04030201,0x08070605 --output %t.offset.json
...
module {
  ...
  dataflow.graph private @offset_vector_element(
      %start: none, %value: vector<4xi8>,
      %memory: memref<?xvector<4xi8>>)
      -> vector<4xi8>
      attributes {input_segments = array<i32: 1, 0, 1>,
                  result_segments = array<i32: 1, 0, 0>} {
    %idx = dataflow.constant %start {const_value = 0 : index} : index
    %loaded, %read = dataflow.load %memory[%idx] %start
        : memref<?xvector<4xi8>>
    %done = dataflow.store %memory[%idx] %value %read
        : memref<?xvector<4xi8>>
    dataflow.graph.return %done, %loaded : none, vector<4xi8>
  }
}
