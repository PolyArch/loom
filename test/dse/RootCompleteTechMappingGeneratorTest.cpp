...
void descriptorReusesTheExactTechMappingOwnerContract() {
  ...
  if (descriptor.kind !=
      ...
      descriptor.workUnits[2].spelling != "publication_slot")
...
void finiteDataflowSetComposesIndependentOwnerInvocations() {
  ...
  for (const auto &candidate : completed->outputBindings.front().artifacts) {
...
void incompleteTraversalRetainsOnlyTheCompletedCanonicalPrefix() {
  ...
  std::vector<loom::ArtifactRootReference> candidates = {multiReference,
                                                         singleReference};
  llvm::sort(candidates, loom::artifactRootReferenceLess);
  const bool singleGraphCompletesBeforeLimit =
      candidates.front() == singleReference;
  ...
  if (!incomplete ||
      ...
      incomplete->retainedOutputBindings.front().artifacts.size() !=
          expectedPrefixSize ||
      incomplete->lineageEdges.size() != expectedPrefixSize)
    fail("incomplete traversal did not retain the completed canonical prefix");
}
...
int main() {
  ...
  incompleteTraversalRetainsOnlyTheCompletedCanonicalPrefix();
