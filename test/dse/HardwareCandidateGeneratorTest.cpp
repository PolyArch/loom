...
void computeContextFeedbackReconstructsAlternativeSupply(
  ...
  // Mapping layer and is offered only once that evidence exists. One
  // observation overrides that default, and only one: a composite capability
  // record binds several actors per realization, while each added Temporal
  // context only lets the cover admit one more single-actor realization.
  if (joint.direction ==
      loom::dse::TechMappingComputeContextGrowthDirection::
          SpatialFuOccurrence) {
    require(joint.spatialFuGrowth && joint.decisions.empty() &&
                joint.spatialFuGrowth->activeOperationCount >= 2,
            "the default Hall growth preference took a Spatial supply that is "
            "not a composite capability");
  } else {
    require(joint.direction == loom::dse::
                                   TechMappingComputeContextGrowthDirection::
                                       TemporalInstructionStore &&
                !joint.spatialFuGrowth,
            "the default Hall growth preference is not the atomic "
            "instruction-store closure");
    require(!joint.decisions.empty() &&
    ...
    auto jointModule = take(loom::fabric::importEntireFabricRoot(
        jointCompleted.outputBindings.front().artifacts.front(),
        fixture.store));
    ...
    require(jointModule.view().fuOccurrences().size() ==
                module.view().fuOccurrences().size(),
  ...
  // A chain that already spent its one Spatial FU occurrence probe asks for
  // the Temporal closure alone. That supply rebuilds the Module and reopens
  // every Mapping layer, so the budget covers the composite supply the default
  // preference admits as well.
  auto budgetedPlan =
      take(loom::dse::projectTechMappingComputeContextJointGrowthPlan(
          adopted, module.view(),
          loom::dse::TechMappingComputeContextSupplyPreference::
              TemporalInstructionStoreOnly));
  require(budgetedPlan.has_value(),
          "a spent Spatial probe left the Temporal closure unavailable");
  require(budgetedPlan->direction ==
                  loom::dse::TechMappingComputeContextGrowthDirection::
                      TemporalInstructionStore &&
              !budgetedPlan->spatialFuGrowth,
          "a spent Spatial probe was offered another Spatial supply");
  ...
  auto retreatPlan =
      take(loom::dse::projectTechMappingComputeContextJointGrowthPlan(
          adopted, module.view(),
          loom::dse::TechMappingComputeContextSupplyPreference::AnySupply));
  ...
  if (retreat.direction ==
    ...
    for (const auto fu : module.view().fuOccurrences()) {
      ...
      targetHostsCapability |= module.view().fuTemplateOf(fu) ==
                               retreat.spatialFuGrowth->capability.fu;
    }
    ...
    require(retreat.addedContextCount ==
                module.view().peResidentContextCount(inventory.target),
            "Spatial FU growth misreported the contexts it makes compatible");
    auto retreatConfig =
        take(loom::dse::resolveSpatialMicroarchitectureRewriteConfig(
            {loom::dse::SpatialMicroarchitectureDecisionDomain{
                loom::dse::ChangeFuInventoryDomain{inventory.target,
                                                   {inventory.prototypes}}}},
            1));
    auto retreatBinding = take(
        loom::dse::resolveSpatialMicroarchitectureCandidateGeneratorBinding(
            retreatConfig));
    auto retreatInputs =
        take(loom::dse::bindSpatialMicroarchitectureCandidateGeneratorInputs(
            {module.reference()}));
    auto retreatResult = take(loom::dse::invokeCandidateGenerator(
        retreatInputs, retreatBinding, fixture.store, fixture.blobs));
    const auto &retreatCompleted = completed(retreatResult);
    // The Builder is the legality owner. A refused decision publishes no
    // child, which the reopen owner treats as a typed retreat rather than a
    // malformed result, so both publications are admissible here.
    if (retreatCompleted.outputBindings.front().artifacts.size() == 1) {
      auto retreatModule = take(loom::fabric::importEntireFabricRoot(
          retreatCompleted.outputBindings.front().artifacts.front(),
          fixture.store));
      std::uint64_t retreatContexts = 0;
      for (const auto pe : retreatModule.view().peOccurrences())
        retreatContexts += retreatModule.view().peResidentContextCount(pe);
      require(retreatContexts == parentContexts &&
                  retreatModule.view().fuOccurrences().size() ==
                      module.view().fuOccurrences().size() + 1,
              "Spatial FU growth child changed resident capacity instead of "
              "capability");
    } else {
      require(retreatCompleted.outputBindings.front().artifacts.empty() &&
                  retreatCompleted.lineageEdges.empty(),
              "refused Spatial FU growth published a partial result");
    }
  } else {
    require(!retreat.spatialFuGrowth && !retreat.decisions.empty(),
            "withdrawn preference produced neither a closing Spatial decision "
            "nor a Temporal closure");
  }
...
void temporalOperandBufferRewrite(
    Fixture &fixture, const loom::fabric::FinalizedFabricRoot &module) {
  using BufferSignature = std::pair<::fabric::OperandBufferMode, std::uint32_t>;
  const auto bufferSignatures =
  ...
  for (const loom::dse::CandidateGeneratorLineageEdge &lineage :
    ...
    auto decision = take(
        loom::dse::adoptSpatialMicroarchitectureDecision(lineage.ownerPayload));
    auto child = take(
        loom::fabric::importEntireFabricRoot(lineage.output, fixture.store));
    ...
    if (const auto *mode =
            std::get_if<loom::dse::ChangeTemporalOperandBufferMode>(
                &decision.decision)) {
...
void hardwareImpactMatrix(const loom::fabric::FinalizedFabricRoot &system,
                          const loom::fabric::FinalizedFabricRoot &module) {
  ...
  if (!module.view().peOccurrences().empty()) {
    ...
    requireModuleImpact(loom::dse::ResizeInstructionStore{pe, 2},
  ...
  if (!module.view().memoryOccurrences().empty()) {
    ...
    requireModuleImpact(loom::dse::ResizeMemory{memory, 4096},
                        loom::dse::HardwareMutationFamily::SpatialMemory,
                        loom::dse::HardwareMutationLocality::LocalCone,
                        loom::dse::HardwareMappingImpactKind::Rebase,
                        loom::dse::HardwareMappingImpactKind::Rebase);
    requireModuleImpact(loom::dse::ChangeMemoryOperationTable{memory, memory},
                        loom::dse::HardwareMutationFamily::SpatialMemory,
  ...
  if (!module.view().fifoOccurrences().empty()) {
    ...
    requireModuleImpact(loom::dse::ResizeFifo{fifo, 2},
                        loom::dse::HardwareMutationFamily::SpatialFifo,
                        loom::dse::HardwareMutationLocality::LocalCone,
                        loom::dse::HardwareMappingImpactKind::Rebase,
                        loom::dse::HardwareMappingImpactKind::Rebase);
    requireModuleImpact(loom::dse::ChangeFifoBypassCapability{fifo, true},
                        loom::dse::HardwareMutationFamily::SpatialFifo,
                        loom::dse::HardwareMutationLocality::LocalCone,
                        loom::dse::HardwareMappingImpactKind::Rebase,
                        loom::dse::HardwareMappingImpactKind::Reopen);
  ...
  auto addImpact = loom::dse::projectHardwareImpact(
      loom::dse::SystemCompositionCandidateDecision{
          ...
          {}});
  require(
      addImpact.family == loom::dse::HardwareMutationFamily::SystemAccCore &&
          addImpact.system.kind == loom::dse::HardwareMappingImpactKind::Rebase,
  ...
  auto transportImpact = loom::dse::projectHardwareImpact(
      loom::dse::SystemCompositionCandidateDecision{
          system.reference(),
          loom::dse::ChangeTransportResource{transport, transport},
          {},
          {}});
