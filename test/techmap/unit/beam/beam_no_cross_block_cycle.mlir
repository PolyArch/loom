...
// create a multi-block SSA cycle. The beam partitioner must detect the
// would-be cycle in every surviving state and leave at least one op at
// graph level. We verify by counting the resulting subgraphs: exactly
// three of the four ops should be wrapped, the fourth (the cycle-closing
// op) is left as a plain arith op inside dataflow.graph. Same outcome as
// greedy and list, regardless of beam width.
