...
// output IR. This guards both the candidate cache (already covered for
// dump output) and the partitioner's serial search (which must not depend
// on candidate-cache iteration order).
//
// The temporary YAML config files override only the `threads` knob; the
// algorithm and weights stay at their defaults (greedy / alpha=beta=1 /
// gamma=0.5).

// RUN: echo "techmap:" > %t.cfg1.yaml
// RUN: echo "  algorithm: greedy" >> %t.cfg1.yaml
// RUN: echo "  threads: 1" >> %t.cfg1.yaml
// RUN: echo "techmap:" > %t.cfg4.yaml
// RUN: echo "  algorithm: greedy" >> %t.cfg4.yaml
