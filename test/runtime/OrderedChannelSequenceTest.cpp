...
void multicastAndRateConversion() {
  OrderedChannelSequence channel = take(OrderedChannelSequence::create(6, 2));
  ...
  require(channel.occupiedBytes() == second.size(),
  ...
  require(channel.occupiedBytes() == 0, "committed prefix was not reclaimed");
}

void CapacityAndClose() {
  OrderedChannelSequence channel = take(OrderedChannelSequence::create(2, 1));
  const std::vector<std::uint8_t> payload{7, 8};
  require(take(channel.publish(payload)) == 0, "capacity fixture publish failed");
  ...
  require(receive.kind == OrderedChannelReceiveKind::Message,
          "closed channel discarded committed-prefix message");
  require(!channel.commit(0, 0), "closed channel message commit failed");
