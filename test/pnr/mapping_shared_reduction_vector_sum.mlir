...
// RUN: loom-pnr-map --dfg-mlir %s --graph cmsis_vector_sum_s8 --hardware-mlir %t.hardware.mlir --hardware shared_memory_reduction_adg --workload cmsis_vector_sum_s8 --output %t.mapping.csv --artifact %t.mapping.json
// RUN: FileCheck %s --check-prefix=CSV < %t.mapping.csv
// RUN: FileCheck %s --check-prefix=JSON < %t.mapping.json

// CSV: workload,hardware,mapping_id,placed_records,routed_edges,unrouted_edges,unplaced_records,status,diagnostic
// CSV-NEXT: cmsis_vector_sum_s8,shared_memory_reduction_adg,cmsis_vector_sum_s8__cmsis_vector_sum_s8__shared_memory_reduction_adg,{{[1-9][0-9]*}},{{[1-9][0-9]*}},0,0,pass,mapped software graph to fabric resources
...
module {
  dataflow.graph.func private @cmsis_vector_sum_s8(
    ...
    %old, %dst_done = dataflow.load %dst[%dst_index] %src_done
        : memref<?xi32>
    %updated = arith.addi %old, %scaled : i32
    %stored = dataflow.store %dst[%dst_index] %updated %dst_done
        : memref<?xi32>
