...
// RUN: loom-pnr-map --dfg-mlir %t.lowered.mlir --graph crc32_shift_mix --hardware-mlir %S/shared_reduction_adg.mlir --hardware shared_reduction_adg --workload crc32_shift_mix --output %t.mapping.csv --artifact %t.mapping.json
// RUN: FileCheck %s --check-prefix=CSV < %t.mapping.csv
// RUN: FileCheck %s --check-prefix=JSON < %t.mapping.json

// CSV: workload,hardware,mapping_id,placed_records,routed_edges,unrouted_edges,unplaced_records,status,diagnostic
// CSV-NEXT: crc32_shift_mix,shared_reduction_adg,crc32_shift_mix__crc32_shift_mix__shared_reduction_adg,8,{{[1-9][0-9]*}},0,0,pass,mapped software graph to fabric resources
...
module {
  dataflow.graph.func private @crc32_shift_mix(
      %ctrl: none, %input: memref<?xi32>, %table: memref<?xi32>, %index: index,
      %carry: i32, %inner: i32, %byte_shift: i32, %mask: index)
      -> (none, i32) {
