// RUN: loom-pnr-map --dfg-mlir %s --graph direct_route --hardware-mlir %s --hardware direct_route_adg --workload direct_route --output %t.mapping.csv --artifact %t.mapping.json
// RUN: FileCheck %s --check-prefix=CSV < %t.mapping.csv
// RUN: FileCheck %s --check-prefix=JSON < %t.mapping.json

// CSV: workload,hardware,mapping_id,placed_records,routed_edges,unrouted_edges,unplaced_records,status,diagnostic
// CSV-NEXT: direct_route,direct_route_adg,direct_route__direct_route__direct_route_adg,2,1,0,0,pass
...
module {
  ...
  fabric.module @direct_route_adg(%i32a : !fabric.bits<32>,
                                  %i32b : !fabric.bits<32>,
                                  %i32c : !fabric.bits<32>) {
