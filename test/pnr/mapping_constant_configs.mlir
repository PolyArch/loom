...
// RUN: loom-pnr-map --dfg-mlir %t.lowered.mlir --graph shared_constant_five --hardware-mlir %t.dir/shared-memory-reduction.mlir --hardware shared_memory_reduction_adg --workload shared_constant_five --output %t.shared-five.csv --artifact %t.shared-five.json
...
// CSV-PASS-NEXT: constant_two,constant_adg,constant_two__constant_two__constant_adg,2,1,0,0,pass
...
// CSV-FAIL-NEXT: constant_three,constant_adg,constant_three__constant_three__constant_adg,1,0,0,1,fail,missing hardware resource for software op dataflow.constant
...
// CSV-SHARED-FIVE-NEXT: shared_constant_five,shared_memory_reduction_adg,shared_constant_five__shared_constant_five__shared_memory_reduction_adg,10,5,0,0,pass
...
// CSV-BOUNDS-NEXT: structured_constant_bounds,bounds_constant_adg,structured_constant_bounds__structured_constant_bounds__bounds_constant_adg,3,2,0,0,pass
...
// CSV-SHARED-EIGHT-NEXT: shared_constant_eight,shared_memory_reduction_adg,shared_constant_eight__shared_constant_eight__shared_memory_reduction_adg,2,1,0,0,pass
...
// CSV-WIDE-NEXT: wide_constant_thirty_one,wide_constant_adg,wide_constant_thirty_one__wide_constant_thirty_one__wide_constant_adg,2,1,0,0,pass
...
module {
  ...
  fabric.module @constant_adg(%ctrl: !fabric.bits<0>) {
    %constant_ctrl, %sync_ctrl = fabric.switch [spatial] %ctrl
        [{connectivity_table = ["1", "1"]}]
        : (!fabric.bits<0>) -> (!fabric.bits<0>, !fabric.bits<0>)
    fabric.pe [spatial]
        (%pa = %constant_ctrl : !fabric.bits<0> to !fabric.bits<32>,
         %pb = %sync_ctrl : !fabric.bits<0> to !fabric.bits<32>)
        -> !fabric.bits<32> {
      fabric.fu(%input = %pa : !fabric.bits<32> to !fabric.bits<0>,
                %token = %pb : !fabric.bits<32> to !fabric.bits<0>)
          -> !fabric.bits<32> {
  ...
  fabric.module @bounds_constant_adg(%ctrl: !fabric.bits<0>,
    ...
    fabric.pe [spatial] (%lhs = %data : !fabric.bits<32>,
                         ...
                         %pc = %sync_ctrl : !fabric.bits<0> to !fabric.bits<32>)
        -> !fabric.bits<32> {
  ...
  fabric.module @wide_constant_adg(%ctrl: !fabric.bits<0>) {
    %constant_ctrl, %sync_ctrl = fabric.switch [spatial] %ctrl
        ...
        : (!fabric.bits<0>) -> (!fabric.bits<0>, !fabric.bits<0>)
    fabric.pe [spatial]
        (%pa = %constant_ctrl : !fabric.bits<0> to !fabric.bits<64>,
         %pb = %sync_ctrl : !fabric.bits<0> to !fabric.bits<64>)
        -> !fabric.bits<64> {
      fabric.fu(%input = %pa : !fabric.bits<64> to !fabric.bits<0>,
                %token = %pb : !fabric.bits<64> to !fabric.bits<0>)
          -> !fabric.bits<64> {
        %value = fabric.op [@dataflow.constant] (%input)
