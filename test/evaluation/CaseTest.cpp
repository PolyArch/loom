...
void requestCanonicalRoundTripAndStoreImport() {
  ...
  require(
      ...
      llvm::StringRef(canonical).contains(
          "\"descriptor_ref\":{\"schema_major\":3,\"schema_minor\":2,") &&
          llvm::StringRef(canonical).contains(
...
void currentRegistryReferencesAreRequired() {
  // Registry 3.2 rejects obsolete case and model descriptor references
  ...
  if (evaluationSchemaVersion() != SchemaVersion{3, 2})
    fail(__func__, "the Evaluation registry did not move to schema 3.2");
