#!/usr/bin/env bash
# Raise lower_bound from .ll into SCF MLIR via loom-raise.
...
raise_one() {
    ...
    if ! grep -E -q 'scf\.(while|if)|arith\.cmpf' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no lower-bound control body in ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
        if ! grep -E -q "(func\\.call|[^[:alnum:]_]call) @[^[:space:]]*lower_bound_candidate[^[:space:]]*\\(" "${mlir}"; then
            echo "[${KERNEL}/${variant}] expected call to lower_bound_candidate" >&2
            return 1
        fi
    else
        if grep -E -q "(func\\.call|[^[:alnum:]_]call) @[^[:space:]]*lower_bound_candidate[^[:space:]]*\\(" "${mlir}"; then
            echo "[${KERNEL}/${variant}] unexpected lower_bound_candidate call in inline variant" >&2
            return 1
        fi
    fi
}
