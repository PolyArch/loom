// Lower-bound function variant migrated from the legacy app corpus.
...
void lower_bound_ref(const float *sorted, const float *targets,
...
void lower_bound_candidate(const float *sorted, const float *targets,
                           uint32_t *indices, uint32_t input_size,
                           uint32_t target_count) {
    for (uint32_t t = 0; t < target_count; ++t) {
        float target = targets[t];
...
int main() {
    ...
    lower_bound_ref(sorted.data(), targets.data(), reference.data(),
                    kInputSize, kTargetCount);
    lower_bound_candidate(sorted.data(), targets.data(), candidate.data(),
                          kInputSize, kTargetCount);
    ...
    std::printf("lower_bound checksum: %u\n", checksum(candidate));
    std::puts("PASSED");
    return 0;
}
