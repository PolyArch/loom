#!/usr/bin/env bash
# Raise line_intersect from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="line_intersect"
KERNEL_FN="line_intersect_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        ...
        in_func && in_loop {
            ...
            if ($0 ~ /llvm\.intr\.fabs|llvm\.call.*@fabsf/) {
                has_fabs = 1
            }
            ...
            if (loads >= 8 && stores >= 1 && has_fabs && has_div && has_cmp) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no line-intersection loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
