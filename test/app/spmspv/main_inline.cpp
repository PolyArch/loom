// Sparse matrix times sparse-vector inline variant.
...
int main() {
    ...
    for (uint32_t i = 0; i < kVectorNnz; ++i) {
        dense[kVectorIndices[i]] = kVectorValues[i];
    }
    for (uint32_t row = 0; row < kRows; ++row) {
        ...
        for (uint32_t idx = begin; idx < end; ++idx) {
            sum += kValues[idx] * dense[kColIndices[idx]];
        }
        output[row] = sum;
    }
