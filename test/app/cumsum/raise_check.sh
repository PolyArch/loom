#!/usr/bin/env bash
# Raise cumsum from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="cumsum"
KERNEL_FN="cumsum_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if (has_scan_loop && has_add && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no cumulative-sum loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
