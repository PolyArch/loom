#!/usr/bin/env bash
# Raise mat3x3_mult from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="mat3x3_mult"
KERNEL_FN="mat3x3_mult_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /arith\.mulf|arith\.addf|llvm\.intr\.fmuladd/) {
                fmath += 1
            }
            ...
            if (loops >= 1 && has_reduction && has_fma && fmath >= 1 &&
                loads >= 2 && stores >= 1) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no 3x3 matrix multiply loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
