// 3x3 matrix-multiply function variant migrated from the legacy app corpus.
...
void initialize_matrices(std::array<float, kElems> &a,
...
void mat3x3_mult_kernel(const float *a, const float *b, float *c,
                        uint32_t matrices) {
    for (uint32_t n = 0; n < matrices; ++n) {
        const float *mat_a = &a[n * 9u];
        const float *mat_b = &b[n * 9u];
...
int main() {
    ...
    mat3x3_mult_kernel(a.data(), b.data(), candidate.data(), kMatrices);
