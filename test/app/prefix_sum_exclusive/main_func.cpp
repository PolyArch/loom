// Exclusive prefix-sum function variant migrated from the legacy app corpus.
...
void prefix_sum_exclusive_kernel(const uint32_t *input, uint32_t *output, uint32_t size) {
    if (size == 0) {
        return;
    }
...
int main() {
    ...
    prefix_sum_exclusive_kernel(input.data(), candidate.data(), kSize);
