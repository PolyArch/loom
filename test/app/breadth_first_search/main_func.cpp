// Queue-based CSR breadth-first search variant migrated from the legacy app corpus.
...
void breadth_first_search_ref(const uint32_t *row_ptr, const uint32_t *col_idx,
...
void breadth_first_search_kernel(const uint32_t *row_ptr,
...
int main() {
    std::array<int32_t, kNumNodes> expected_distance = {};
    std::array<int32_t, kNumNodes> candidate_distance = {};
    std::array<uint32_t, kQueueSize> expected_queue = {};
    std::array<uint32_t, kQueueSize> candidate_queue = {};
    std::array<uint32_t, kNumNodes> expected_visited = {};
    std::array<uint32_t, kNumNodes> candidate_visited = {};

    breadth_first_search_ref(kRowPtr.data(), kColIdx.data(),
                             expected_distance.data(), expected_queue.data(),
                             expected_visited.data(), kNumNodes, kQueueSize,
                             kSource);
    breadth_first_search_kernel(kRowPtr.data(), kColIdx.data(),
                                candidate_distance.data(), candidate_queue.data(),
                                candidate_visited.data(), kNumNodes, kQueueSize,
                                kSource);

    for (uint32_t i = 0; i < kNumNodes; ++i) {
        if (expected_distance[i] != kExpectedDistance[i] ||
            candidate_distance[i] != expected_distance[i]) {
            std::puts("FAILED");
            return 1;
        }
    }
