#!/usr/bin/env bash
# Raise fft_butterfly from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="fft_butterfly"
KERNEL_FN="fft_butterfly_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /math\.(cos|sin)|llvm\.intr\.fmuladd|arith\.(mulf|addf|subf)/) {
                float_ops += 1
            }
        ...
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no FFT butterfly loop path in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
