// FFT butterfly variant migrated from the legacy app corpus.
...
void fft_butterfly_kernel(const float *input_real, const float *input_imag,
                          float *output_real, float *output_imag, uint32_t size) {
    for (uint32_t i = 0; i < size; ++i) {
        output_real[i] = input_real[i];
        output_imag[i] = input_imag[i];
...
int main() {
    ...
    fft_butterfly_kernel(input_real.data(), input_imag.data(), candidate_real.data(),
