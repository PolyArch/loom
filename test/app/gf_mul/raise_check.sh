#!/usr/bin/env bash
# Raise gf_mul from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="gf_mul"
KERNEL_FN="gf_mul_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /arith\.(andi|xori|shli|shrui)|llvm\.(and|xor|shl|lshr)/) {
                bit_ops += 1
            }
            if ($0 ~ /llvm\.load/) {
                loads += 1
            ...
            if (loops >= 2 && has_control && bit_ops >= 4 && loads >= 2 && stores >= 1) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no GF multiplication loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
