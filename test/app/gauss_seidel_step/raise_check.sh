#!/usr/bin/env bash
# Raise gauss_seidel_step from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="gauss_seidel_step"
KERNEL_FN="gauss_seidel_step_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /arith\.divf|llvm\.fdiv/) {
                has_div = 1
            }
            if (loops >= 1 && has_forall && has_load && has_store && has_mul && has_add && has_sub && has_div) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no Gauss-Seidel update loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
