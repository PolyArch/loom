#!/usr/bin/env bash
# Raise im2col from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="im2col"
KERNEL_FN="im2col_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /llvm\.intr\.memcpy/) {
                memcpys += 1
            }
            if (loops >= 5 && muls >= 3 && adds >= 2 && loads >= 1 && (stores >= 1 || memcpys >= 1)) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no im2col nested indexing path in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
