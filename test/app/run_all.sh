...
STATUS_ROOT="${BUILD_ROOT}/_run-all-status"
LOG_ROOT="${BUILD_ROOT}/_run-all-logs"
rm -rf "${STATUS_ROOT}" "${LOG_ROOT}"
mkdir -p "${STATUS_ROOT}" "${LOG_ROOT}"

run_kernel_job() {
    local k="$1"
    local script="${HERE}/${k}/run_check.sh"
...
echo
echo "==== summary ===="
echo "CC=${CC} CXX=${CXX} jobs=${PARALLEL_JOBS}"
for k in "${KERNELS[@]}"; do
    if [[ " ${passed[*]} " == *" ${k} "* ]]; then
        echo "  PASS  ${k}"
    else
