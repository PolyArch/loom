// Modified bitonic-stage function variant migrated from the legacy app corpus.
...
constexpr std::array<float, kSize> kExpected = {
    1.0f, 2.0f, 2.0f, 3.0f, 128.0f, 94.0f, 112.0f, 79.0f};
...
void bitonic_stage_modified_kernel(float *inplace, uint32_t size,
    ...
    for (uint32_t i = 0; i < size; ++i) {
        ...
        if ((idx_in_block & distance) == 0u) {
            ...
            for (uint32_t j = size / 2u; j < size; ++j) {
                inplace[j] *= 2.0f;
            }
        } else {
            inplace[i] -= 1.0f;
...
int main() {
    ...
    bitonic_stage_modified_kernel(candidate.data(), kSize, 1u, 0u);
    ...
    std::printf("bitonic_stage-modified checksum: %.3f\n", checksum(candidate));
    std::puts("PASSED");
    return 0;
}
