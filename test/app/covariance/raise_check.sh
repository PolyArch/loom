#!/usr/bin/env bash
# Raise covariance from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="covariance"
KERNEL_FN="covariance_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if (reduction_loops >= 2 && has_centering && has_mul && has_normalize) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no covariance loop structure in @${scope_name}: ${mlir}" >&2
        return 1
    fi
