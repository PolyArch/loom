#!/usr/bin/env bash
# Raise string_hash from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="string_hash"
KERNEL_FN="string_hash_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /remui|urem|llvm\.urem/) {
                has_mod = 1
            }
            if ($0 ~ /arith\.muli|arith\.addi|arith\.subi|llvm\.(mul|add|sub)/) {
                int_math += 1
            }
            if ($0 ~ /llvm\.store|memref\.store/) {
                has_store = 1
            }
            if (loops >= 3 && has_mod && int_math >= 3 && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no rolling-hash loop body in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
