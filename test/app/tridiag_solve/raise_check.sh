#!/usr/bin/env bash
# Raise tridiag_solve from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="tridiag_solve"
KERNEL_FN="tridiag_solve_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /arith\.(divf|mulf|subf)|llvm\.f(div|mul|sub)|llvm\.intr\.fmuladd/) {
                float_ops += 1
            }
            if (loops >= 1 && loads >= 4 && stores >= 3 && float_ops >= 4) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no tridiagonal solve loop path in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
