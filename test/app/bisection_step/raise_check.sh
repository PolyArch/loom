#!/usr/bin/env bash
# Raise bisection_step from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="bisection_step"
KERNEL_FN="bisection_step_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /arith\.cmpf|scf\.if|arith\.select/) {
                has_branch = 1
            }
            if ($0 ~ /arith\.mulf/) {
                has_mul = 1
            ...
            if (has_loop && has_branch && has_mul && has_midpoint && stores >= 2) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no bisection branch body in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
