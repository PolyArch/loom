#!/usr/bin/env bash
# Raise bitrev from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="bitrev"
KERNEL_FN="bitrev_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        ...
        in_func && in_loop {
            ...
            if (has_while && has_shift && has_mask && has_or && has_load && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no bit-reversal permutation loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
