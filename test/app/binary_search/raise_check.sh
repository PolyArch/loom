#!/usr/bin/env bash
# Raise binary_search from .ll into SCF MLIR via loom-raise.
...
raise_one() {
    ...
    if ! grep -E -q 'scf\.(while|if)|arith\.cmpf' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no search control body in ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
        if ! grep -E -q "(func\\.call|[^[:alnum:]_]call) @[^[:space:]]*binary_search_candidate[^[:space:]]*\\(" "${mlir}"; then
            echo "[${KERNEL}/${variant}] expected call to binary_search_candidate" >&2
            return 1
        fi
    else
        if grep -E -q "(func\\.call|[^[:alnum:]_]call) @[^[:space:]]*binary_search_candidate[^[:space:]]*\\(" "${mlir}"; then
            echo "[${KERNEL}/${variant}] unexpected binary_search_candidate call in inline variant" >&2
            return 1
        fi
    fi
}
