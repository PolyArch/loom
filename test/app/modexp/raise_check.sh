#!/usr/bin/env bash
# Raise modexp from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="modexp"
KERNEL_FN="modexp_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /arith\.remui|llvm\.urem/) {
                rems += 1
            }
            if ($0 ~ /arith\.shrui|llvm\.lshr/) {
                has_shift = 1
            }
            if ($0 ~ /llvm\.store/) {
                stores += 1
            }
            if (loops >= 2 && has_control && muls >= 2 && rems >= 2 && has_shift && stores >= 1) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no modular exponent loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
