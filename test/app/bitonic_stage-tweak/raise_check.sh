#!/usr/bin/env bash
# Raise bitonic_stage-tweak from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="bitonic_stage-tweak"
KERNEL_FN="bitonic_stage_tweak_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /arith\.cmpf|arith\.cmpi|scf\.if/) {
                has_control = 1
            }
            if ($0 ~ /arith\.(addf|subf)|llvm\.f(add|sub)/) {
                has_update = 1
            }
            if ($0 ~ /llvm\.load/) {
                loads += 1
            ...
            if (has_loop && has_control && has_update && loads >= 3 && stores >= 2) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no tweaked bitonic stage in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
