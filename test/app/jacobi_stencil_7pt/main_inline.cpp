// Seven-point Jacobi stencil inline variant migrated from the legacy app corpus.
...
constexpr uint32_t kInteriorCount =
    (kDepth - 2u) * (kRows - 2u) * (kCols - 2u);
    ...
    for (uint32_t z = 1; z < depth - 1; ++z) {
        for (uint32_t r = 1; r < rows - 1; ++r) {
            for (uint32_t c = 1; c < cols - 1; ++c) {
                ...
                output[idx] =
                    (front + back + north + south + west + east) *
                    (1.0f / 6.0f);
            }
        }
    }
...
int main() {
    ...
    jacobi_stencil_7pt_ref(kInput.data(), expected.data(), kDepth, kRows,
                           kCols);
    for (uint32_t inner = 0; inner < kInteriorCount; ++inner) {
        ...
        const float front = kInput[idx - kPlane];
        const float back = kInput[idx + kPlane];
