// Auto-correlation function variant migrated from the legacy app corpus.
...
void autocorrelation_kernel(const float *x, float *output,
                            uint32_t x_size, uint32_t max_lag) {
    for (uint32_t lag = 0; lag < max_lag; ++lag) {
        float sum = 0.0f;
        for (uint32_t i = 0; i < x_size - lag; ++i) {
...
int main() {
    ...
    autocorrelation_kernel(x.data(), candidate.data(), kXSize, kMaxLag);
