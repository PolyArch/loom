#!/usr/bin/env bash
# Raise autocorrelation from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="autocorrelation"
KERNEL_FN="autocorrelation_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /scf\.for .*iter_args/) {
                has_inner_sum = 1
            }
            if ($0 ~ /llvm\.intr\.umax/ || $0 ~ /scf\.for .* to %arg[0-9]+ step/) {
                has_dynamic_bound = 1
            }
            if ($0 ~ /arith\.addi .*c-1_i(32|64)/) {
                has_bound_decrement = 1
            }
            ...
            if (loop_count >= 2 && has_inner_sum && has_dynamic_bound &&
                    has_bound_decrement && has_mul && has_add && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no auto-correlation loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
