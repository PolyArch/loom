#!/usr/bin/env bash
# Raise ctz from .ll into SCF MLIR via loom-raise.
...
raise_one() {
    ...
    if ! grep -E -q 'scf\.while|arith\.(andi|shrui|cmpi|select)' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no count-trailing-zeros body in ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
        if ! grep -E -q "(func\\.call|[^[:alnum:]_]call) @[^[:space:]]*ctz_candidate[^[:space:]]*\\(" "${mlir}"; then
            echo "[${KERNEL}/${variant}] expected call to ctz_candidate" >&2
            return 1
        fi
    else
        if grep -E -q "(func\\.call|[^[:alnum:]_]call) @[^[:space:]]*ctz_candidate[^[:space:]]*\\(" "${mlir}"; then
            echo "[${KERNEL}/${variant}] unexpected ctz_candidate call in inline variant" >&2
            return 1
        fi
    fi
}
