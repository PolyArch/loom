#!/usr/bin/env bash
# Raise pool_avg from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="pool_avg"
KERNEL_FN="pool_avg_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        ...
        END { exit(has_loop && has_load && has_add && (has_div || has_mul) && has_store ? 0 : 1) }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no average-pooling loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
