#!/usr/bin/env bash
# Raise batchnorm from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="batchnorm"
KERNEL_FN="batchnorm_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /sqrtf|llvm\.intr\.sqrt|@sqrt/) {
                has_sqrt = 1
            }
            if ($0 ~ /arith\.divf|arith\.mulf/) {
                has_scale = 1
            }
            if ($0 ~ /arith\.subf|arith\.addf|llvm\.intr\.fmuladd/) {
                has_affine = 1
            }
            if ($0 ~ /llvm\.store|memref\.store/) {
                has_store = 1
            }
            if (loops >= 3 && has_sqrt && has_scale && has_affine && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no batchnorm nested body in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
