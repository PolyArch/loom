#!/usr/bin/env bash
# Raise spmspm from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="spmspm"
KERNEL_FN="spmspm_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if (loops >= 4 && has_control && has_mul && adds >= 3 && loads >= 8 && stores >= 4) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no SpMSpM CSR accumulation/compress path in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
