#!/usr/bin/env bash
# Raise upsample_linear from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="upsample_linear"
KERNEL_FN="upsample_linear_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /scf\.if|arith\.cmpi/) {
                has_guard = 1
            }
            if ($0 ~ /arith\.(shrui|andi)|llvm\.trunc/) {
                has_factor_four_addressing = 1
            }
            if ($0 ~ /arith\.(mulf|subf)|llvm\.intr\.fmuladd/) {
                float_math += 1
            }
            if ($0 ~ /llvm\.store|memref\.store/) {
                has_store = 1
            }
            if (loops >= 1 && has_guard && has_factor_four_addressing && float_math >= 2 && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no factor-four linear upsample body in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
