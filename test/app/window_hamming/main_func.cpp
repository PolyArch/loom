// Hamming window function variant migrated from the legacy app corpus.
...
void window_hamming_kernel(const float *input, float *output, uint32_t size) {
    for (uint32_t n = 0; n < size; ++n) {
        const float angle =
            2.0f * kWindowPi * static_cast<float>(n) / static_cast<float>(size - 1u);
        const float window = 0.54f - 0.46f * std::cos(angle);
...
int main() {
    ...
    window_hamming_kernel(input.data(), candidate.data(), kSize);
