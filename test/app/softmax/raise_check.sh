#!/usr/bin/env bash
# Raise softmax from .ll into SCF MLIR via loom-raise.
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /expf|llvm\.intr\.exp|@exp/) {
                has_exp = 1
            }
            if ($0 ~ /arith\.cmpf|arith\.maximumf|llvm\.intr\.maxnum/) {
                has_max = 1
            }
            if ($0 ~ /arith\.divf/) {
                has_div = 1
            ...
            if (loops >= 3 && has_exp && has_max && has_div && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no softmax multi-pass body in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
