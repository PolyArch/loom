#!/usr/bin/env bash
# Raise fir_filter_stateful from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="fir_filter_stateful"
KERNEL_FN="fir_filter_stateful_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /llvm\.intr\.memmove|llvm\.intr\.memcpy/) {
                has_state_transfer = 1
            }
            if (loops >= 3 && has_mul && has_add && loads >= 3 && stores >= 2 &&
                (stores >= 3 || has_state_transfer)) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no stateful FIR loop nest in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
