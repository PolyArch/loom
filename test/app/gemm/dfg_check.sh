#!/usr/bin/env bash
# Lower gemm from .scf.mlir into DFG MLIR via loom-lower. Asserts the
# resulting MLIR has at least one dataflow.thread @t_<sym> + matching
# dataflow.thread.launch @t_<sym>, plus at least one
# dataflow.graph.func @g_<sym> + matching dataflow.graph.launch
# @g_<sym> (the inner-product reduction).
