#!/usr/bin/env bash
# Raise downsample from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="downsample"
KERNEL_FN="downsample"
...
raise_one() {
    ...
    if ! awk -v has_call="${has_call}" '
        has_call == "yes" && /func\.func private @downsample/ {
            in_kernel = 1
            next
        }
        ...
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no strided downsample loop in ${mlir}" >&2
        return 1
    fi
