#!/usr/bin/env bash
# Raise compact from .ll into SCF MLIR via loom-raise.
...
raise_one() {
    ...
    if ! awk -v has_call="${has_call}" '
        has_call == "yes" && /func\.func private @compact/ {
            in_kernel = 1
            next
        }
        ...
        in_kernel {
            ...
            if (loops >= 1 && has_if && has_cmp && has_load && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no compact loop with compare/load/store in ${mlir}" >&2
        return 1
    fi
