#!/usr/bin/env bash
# Lower edge_update and require the full kernel graph evidence path.
...
required = (
    "dataflow.graph.func private @g_edge_update_kernel_0",
    "scf.while",
    "scf.index_switch",
    "dataflow.store",
)
...
if missing:
    raise SystemExit(f"edge_update lowered graph is missing: {', '.join(missing)}")
PY
