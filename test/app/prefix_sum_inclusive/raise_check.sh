#!/usr/bin/env bash
# Raise prefix_sum_inclusive from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="prefix_sum_inclusive"
KERNEL_FN="prefix_sum_inclusive_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        ...
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no inclusive prefix-sum loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
