#!/usr/bin/env bash
# Raise axpy from .ll into SCF MLIR via loom-raise.
...
raise_one() {
    ...
    if ! grep -E -q 'arith\.muli|arith\.addi' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no arithmetic body in ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
        if ! grep -E -q "(func\\.call|[^[:alnum:]_]call) @[^[:space:]]*axpy_candidate[^[:space:]]*\\(" "${mlir}"; then
            echo "[${KERNEL}/${variant}] expected call to axpy_candidate" >&2
            return 1
        fi
    else
        if grep -E -q "(func\\.call|[^[:alnum:]_]call) @[^[:space:]]*axpy_candidate[^[:space:]]*\\(" "${mlir}"; then
            echo "[${KERNEL}/${variant}] unexpected axpy_candidate call in inline variant" >&2
            return 1
        fi
    fi
}
