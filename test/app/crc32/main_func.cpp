// CRC32 function variant migrated from the legacy app corpus.
...
void crc32_kernel(const uint32_t *input_data, uint32_t *output_checksum,
                  uint32_t size) {
    uint32_t crc = 0xffffffffu;
...
int main() {
    ...
    crc32_kernel(input.data(), &candidate, kSize);
