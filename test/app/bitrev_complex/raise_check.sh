#!/usr/bin/env bash
# Raise bitrev_complex from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="bitrev_complex"
KERNEL_FN="bitrev_complex_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        ...
        in_func && in_loop {
            ...
            if (has_while && has_shift && has_mask && has_or && loads >= 2 && stores >= 2) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no complex bit-reversal loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
