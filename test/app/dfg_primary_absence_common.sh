#!/usr/bin/env bash
# Shared helper for app rows whose primary kernel call has not formed a graph.
...
primary_absence_one() {
    ...
    if grep -E 'dataflow\.graph\.(func|launch)' "${dfg}" | grep -q "${expected_token}"; then
        echo "[${KERNEL}/${variant}] unexpected ${expected_token} graph in ${dfg}" >&2
        return 1
    fi
    if ! grep -E -q "(func\\.call|call) @${expected_token}\\(" "${dfg}"; then
        echo "[${KERNEL}/${variant}] ${expected_token} call boundary missing in ${dfg}" >&2
        return 1
    fi
}
