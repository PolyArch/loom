// Strided histogram function variant migrated from the legacy app corpus.
...
void histogram_strided_ref(const uint32_t *input, uint32_t *hist,
                           uint32_t size, uint32_t bins, uint32_t stride) {
    for (uint32_t i = 0; i < bins; ++i) {
        hist[i] = 0;
    }
    for (uint32_t i = 0; i < size; ++i) {
        const uint32_t bin = input[i] / stride;
        if (bin < bins) {
            ++hist[bin];
        }
    }
...
void histogram_strided_kernel(const uint32_t *input, uint32_t *hist,
                              uint32_t size, uint32_t bins, uint32_t stride) {
    for (uint32_t i = 0; i < bins; ++i) {
        hist[i] = 0;
    }
...
int main() {
    ...
    histogram_strided_ref(input.data(), reference.data(), kSize, kBins, kStride);
    histogram_strided_kernel(input.data(), candidate.data(), kSize, kBins,
                             kStride);
