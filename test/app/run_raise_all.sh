...
if [[ -n "${BUILD_DIR:-}" ]]; then
    ...
    BUILD_ROOT="$(mktemp -d -p "${TEMP_ROOT}" "loom-app-raise-all.XXXXXX")"
fi
...
STATUS_ROOT="${BUILD_ROOT}/_raise-all-status"
LOG_ROOT="${BUILD_ROOT}/_raise-all-logs"
rm -rf "${STATUS_ROOT}" "${LOG_ROOT}"
mkdir -p "${STATUS_ROOT}" "${LOG_ROOT}"

run_kernel_job() {
    local k="$1"
    local script="${HERE}/${k}/raise_check.sh"
