// Blackman window function variant migrated from the legacy app corpus.
...
void window_blackman_kernel(const float *input, float *output, uint32_t size) {
    for (uint32_t n = 0; n < size; ++n) {
        const float t =
            2.0f * kWindowPi * static_cast<float>(n) / static_cast<float>(size - 1u);
        const float window = 0.42f - 0.5f * std::cos(t) +
...
int main() {
    ...
    window_blackman_kernel(input.data(), candidate.data(), kSize);
