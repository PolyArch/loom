#!/usr/bin/env bash
# Raise sort_bubble from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="sort_bubble"
KERNEL_FN="sort_bubble_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        ...
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no bubble-sort compare/swap loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
