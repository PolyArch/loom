// 32-bit bit-reversal function variant migrated from the legacy app corpus.
...
void bit_reverse_kernel(const uint32_t *input, uint32_t *output, uint32_t size) {
    for (uint32_t i = 0; i < size; ++i) {
...
int main() {
    ...
    bit_reverse_kernel(input.data(), candidate.data(), kSize);
