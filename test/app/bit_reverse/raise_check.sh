#!/usr/bin/env bash
# Raise bit_reverse from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="bit_reverse"
KERNEL_FN="bit_reverse_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        ...
        in_func && in_loop {
            ...
            if (has_shift && has_mask && has_or && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no bit-reversal loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
