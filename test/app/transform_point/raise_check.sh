#!/usr/bin/env bash
# Raise transform_point from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="transform_point"
KERNEL_FN="transform_point_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        ...
        in_func && in_loop {
            ...
            if (loads >= 3 && stores >= 3 && has_fma) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no affine-transform loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
