// 3D affine transform inline variant migrated from the legacy app corpus.
...
int main() {
    ...
    for (uint32_t i = 0; i < kSize; ++i) {
        ...
        candidate[i * 3 + 0] = m00 * px + m01 * py + m02 * pz + tx;
        candidate[i * 3 + 1] = m10 * px + m11 * py + m12 * pz + ty;
        candidate[i * 3 + 2] = m20 * px + m21 * py + m22 * pz + tz;
    }
