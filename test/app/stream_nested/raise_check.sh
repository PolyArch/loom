#!/usr/bin/env bash
# Raise stream_nested from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="stream_nested"
KERNEL_FN="stream_nested_kernel"
...
raise_one() {
    ...
    if ! awk -v has_call="${has_call}" "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /arith\.andi|arith\.remui|llvm\.urem/) {
                has_mod = 1
            }
            if ($0 ~ /arith\.shli|arith\.shrui|llvm\.(shl|lshr)/) {
                has_shift = 1
            }
            ...
            if (loops >= 3 && has_mod && has_shift && has_add && has_load) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no nested stream accumulation in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
