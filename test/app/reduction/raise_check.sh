#!/usr/bin/env bash
# Raise reduction from .ll into SCF MLIR via loom-raise. The reduction
# kernel is integer-typed (int sum), so we expect arith.addi inside an
# scf.for. main must be a func.func; the function variant must contain
# a call to @reduce_sum.
