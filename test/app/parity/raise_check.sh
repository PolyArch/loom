#!/usr/bin/env bash
# Raise parity from .ll into SCF MLIR via loom-raise.
...
raise_one() {
    ...
    if ! grep -E -q 'scf\.while|arith\.(xori|andi|shrui)|llvm\.intr\.ctpop' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no parity reduction body in ${mlir}" >&2
        return 1
    fi
