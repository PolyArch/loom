#!/usr/bin/env bash
# Raise hash_mix from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="hash_mix"
KERNEL_FN="hash_mix_kernel"
...
raise_one() {
    ...
    if ! awk '
        ...
        in_loop {
            ...
            if (loads >= 2 && has_rotate && has_xor && has_mul && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no hash-mix loop in ${mlir}" >&2
        return 1
    fi
