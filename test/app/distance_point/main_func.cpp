// Point-distance function variant migrated from the legacy app corpus.
...
void initialize_points(std::array<float, kPointElems> &a,
...
void distance_point_kernel(const float *a, const float *b, float *out,
                           uint32_t count) {
    for (uint32_t i = 0; i < count; ++i) {
        const float dx = a[i * 3u + 0u] - b[i * 3u + 0u];
        const float dy = a[i * 3u + 1u] - b[i * 3u + 1u];
...
int main() {
    ...
    initialize_points(a, b);
    distance_ref(a.data(), b.data(), reference.data(), kCount);
    distance_point_kernel(a.data(), b.data(), candidate.data(), kCount);
