#!/usr/bin/env bash
# Raise delta_encode from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="delta_encode"
KERNEL_FN="delta_encode"
...
raise_one() {
    ...
    if ! awk -v has_call="${has_call}" '
        has_call == "yes" && /func\.func private @delta_encode/ {
            in_kernel = 1
            next
        }
        in_kernel && /func\.func/ {
        ...
        in_loop {
            ...
            if (has_sub && loads >= 2 && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no adjacent-difference loop in ${mlir}" >&2
        return 1
    fi
