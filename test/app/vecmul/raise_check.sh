#!/usr/bin/env bash
# Raise vecmul from .ll into SCF MLIR via loom-raise.
...
raise_one() {
    ...
    if [[ "${has_call}" == "yes" ]]; then
        if ! grep -E -q "(func\\.call|[^[:alnum:]_]call) @[^[:space:]]*vecmul_candidate[^[:space:]]*\\(" "${mlir}"; then
            echo "[${KERNEL}/${variant}] expected call to vecmul_candidate" >&2
            return 1
        fi
    else
        if grep -E -q "(func\\.call|[^[:alnum:]_]call) @[^[:space:]]*vecmul_candidate[^[:space:]]*\\(" "${mlir}"; then
            echo "[${KERNEL}/${variant}] unexpected vecmul_candidate call in inline variant" >&2
            return 1
        fi
    fi
}
