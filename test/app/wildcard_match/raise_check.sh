#!/usr/bin/env bash
# Raise wildcard_match from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="wildcard_match"
KERNEL_FN="wildcard_match_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /arith\.cmpi|llvm\.icmp/) {
                compares += 1
            }
            if (loops >= 2 && loads >= 2 && stores >= 1 && compares >= 2) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no wildcard match loop path in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
