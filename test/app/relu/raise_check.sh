#!/usr/bin/env bash
# Raise relu from .ll into SCF MLIR via loom-raise.
...
raise_one() {
    ...
    if ! awk '
        ...
        in_loop {
            ...
            if ($0 ~ /^[[:space:]]*}[[:space:]]*$/) {
                if (has_cmp && has_select && has_store) {
                    found = 1
                }
                in_loop = 0
            }
        ...
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no relu loop with compare, select, and store in ${mlir}" >&2
        return 1
    fi
