#!/usr/bin/env bash
# Raise depthwise_conv from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="depthwise_conv"
KERNEL_FN="depthwise_conv_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if (loops >= 3 && has_mul && has_add && loads >= 2 && stores >= 1) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no depthwise convolution loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
