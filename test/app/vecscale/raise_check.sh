#!/usr/bin/env bash
# Raise vecscale from .ll into SCF MLIR via loom-raise.
...
raise_one() {
    ...
    if ! grep -q 'arith\.muli' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no integer multiply body in ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
        if ! grep -E -q "(func\\.call|[^[:alnum:]_]call) @[^[:space:]]*vecscale_candidate[^[:space:]]*\\(" "${mlir}"; then
            echo "[${KERNEL}/${variant}] expected call to vecscale_candidate" >&2
            return 1
        fi
    else
        if grep -E -q "(func\\.call|[^[:alnum:]_]call) @[^[:space:]]*vecscale_candidate[^[:space:]]*\\(" "${mlir}"; then
            echo "[${KERNEL}/${variant}] unexpected vecscale_candidate call in inline variant" >&2
            return 1
        fi
    fi
}
