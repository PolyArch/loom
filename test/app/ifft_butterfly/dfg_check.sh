#!/usr/bin/env bash
# Lower ifft_butterfly into dataflow graph MLIR and check the butterfly body.
...
if ! grep -E -q "arith\\.(addf|subf|mulf|divf)|llvm\\.intr\\.fmuladd" "${BUILD_DIR}/main_func.dfg.mlir"; then
