#!/usr/bin/env bash
# Raise ifft_butterfly from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="ifft_butterfly"
KERNEL_FN="ifft_butterfly_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /math\.(cos|sin)|llvm\.intr\.fmuladd|arith\.(mulf|addf|subf|divf)/) {
                float_ops += 1
            }
            if (loops >= 3 && loads >= 4 && stores >= 4 && float_ops >= 4) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no IFFT butterfly loop path in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
