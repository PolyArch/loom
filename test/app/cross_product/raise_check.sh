#!/usr/bin/env bash
# Raise cross_product from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="cross_product"
KERNEL_FN="cross_product_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        ...
        in_func && in_loop {
            ...
            if (loads >= 6 && stores >= 3 && has_mul && has_fma) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no cross-product loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
