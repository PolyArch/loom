// 3D cross-product function variant migrated from the legacy app corpus.
...
void cross_product_kernel(const float *input_vec_a, const float *input_vec_b,
                          float *output_result, uint32_t size) {
    for (uint32_t i = 0; i < size; ++i) {
        const float ax = input_vec_a[i * 3 + 0];
        const float ay = input_vec_a[i * 3 + 1];
...
int main() {
    ...
    cross_product_kernel(vec_a.data(), vec_b.data(), candidate.data(), kSize);
