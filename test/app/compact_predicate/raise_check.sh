#!/usr/bin/env bash
# Raise compact_predicate from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="compact_predicate"
KERNEL_FN="compact_predicate_candidate"
...
raise_one() {
    ...
    if ! awk -v has_call="${has_call}" '
        has_call == "yes" && /func\.func private @compact_predicate_candidate/ {
            in_kernel = 1
            next
        }
        ...
        in_kernel {
            ...
            if (loops >= 1 && has_if && has_cmp && loads >= 2 && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no predicate compaction loop with compare/two-load/store in ${mlir}" >&2
        return 1
    fi
