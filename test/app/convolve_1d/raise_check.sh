#!/usr/bin/env bash
# Raise convolve_1d from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="convolve_1d"
KERNEL_FN="convolve_1d_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        ...
        in_func && has_forall {
            ...
            if (has_forall && has_inner_sum && has_mul && has_add && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no valid-convolution loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
