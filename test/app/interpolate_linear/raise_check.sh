#!/usr/bin/env bash
# Raise interpolate_linear from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="interpolate_linear"
KERNEL_FN="interpolate_linear_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /scf\.if|arith\.cmpf/) {
                has_interval_guard = 1
            }
            if ($0 ~ /arith\.divf/) {
                has_div = 1
            }
            if ($0 ~ /arith\.subf|arith\.addf|arith\.mulf|llvm\.intr\.fmuladd/) {
                float_math += 1
            }
            if ($0 ~ /llvm\.store|memref\.store/) {
                has_store = 1
            }
            if (loops >= 2 && has_interval_guard && has_div && float_math >= 3 && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no interpolation search body in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
