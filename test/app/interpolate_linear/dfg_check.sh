#!/usr/bin/env bash
# Lower interpolate_linear into dataflow graph MLIR and check the kernel body.
...
EXPECT_LOAD="yes"
EXPECT_STORE="yes"
EXPECT_GRAPH_SYMBOL_MAIN_FUNC="g_t_interpolate_linear_kernel_0_0"
...
require_graph_body_op "main_func" "${EXPECT_GRAPH_SYMBOL_MAIN_FUNC}" "scf.index_switch" "scf.index_switch"
...
if ! grep -q "scf.index_switch" "${BUILD_DIR}/main_inline.dfg.mlir"; then
    echo "[${KERNEL}/main_inline] no scf.index_switch in ${BUILD_DIR}/main_inline.dfg.mlir" >&2
    exit 1
fi
