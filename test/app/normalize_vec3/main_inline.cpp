// 3D vector-normalization inline variant migrated from the legacy app corpus.
...
int main() {
    ...
    for (uint32_t i = 0; i < kSize; ++i) {
        ...
        if (length > kEpsilon) {
            candidate[i * 3 + 0] = x / length;
            candidate[i * 3 + 1] = y / length;
            candidate[i * 3 + 2] = z / length;
        } else {
            candidate[i * 3 + 0] = 0.0f;
            candidate[i * 3 + 1] = 0.0f;
            candidate[i * 3 + 2] = 0.0f;
        }
    }
