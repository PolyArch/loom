#!/usr/bin/env bash
# Raise string_compare from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="string_compare"
KERNEL_FN="string_compare_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /arith\.cmpi|scf\.if|arith\.select/) {
                branches += 1
            }
            if ($0 ~ /llvm\.store|memref\.store/) {
                has_store = 1
            }
            if (has_loop && branches >= 2 && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no string compare early-exit body in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
