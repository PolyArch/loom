#!/usr/bin/env bash
# Lower string_compare from SCF MLIR into DFG MLIR via loom-lower.
# This app currently exposes string_compare_kernel as a graph-only surface:
# the kernel graph is real DFG/CGRA evidence, while host call replacement
# awaits the synchronous thread wait/fence lowering slice.

set -euo pipefail
export LC_ALL=C

KERNEL="string_compare"
EXPECT_GRAPH="no"
EXPECT_GRAPH_SYMBOL_MAIN_FUNC="g_string_compare_kernel_0"
EXPECT_STREAM="no"
EXPECT_LOAD="yes"
EXPECT_STORE="yes"
