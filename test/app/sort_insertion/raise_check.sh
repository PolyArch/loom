#!/usr/bin/env bash
# Raise sort_insertion from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="sort_insertion"
KERNEL_FN="sort_insertion_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /arith\.cmpf|scf\.while|scf\.if/) {
                has_compare = 1
            }
            if ($0 ~ /llvm\.load/) {
                loads += 1
            ...
            if (loops >= 2 && has_compare && loads >= 2 && stores >= 2) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no insertion-sort shift loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
