#!/usr/bin/env bash
# Raise pack_bits from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="pack_bits"
KERNEL_FN="pack_bits_kernel"
...
raise_one() {
    ...
    if ! awk '
        ...
        in_loop {
            ...
            if (has_shift && has_or && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no bit-pack loop in ${mlir}" >&2
        return 1
    fi
