// Bit-packing function variant migrated from the legacy app corpus.
...
void pack_bits_kernel(const uint32_t *input_bits, uint32_t *output_packed,
                      uint32_t num_bits) {
    const uint32_t num_words = (num_bits + kBitsPerWord - 1) / kBitsPerWord;
...
int main() {
    ...
    pack_bits_kernel(input.data(), candidate.data(), kNumBits);
