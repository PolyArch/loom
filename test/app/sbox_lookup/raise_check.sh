#!/usr/bin/env bash
# Raise sbox_lookup from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="sbox_lookup"
KERNEL_FN="sbox_lookup_kernel"
...
raise_one() {
    ...
    if ! awk '
        ...
        in_loop {
            ...
            if (loads >= 2 && has_mask && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no sbox lookup loop in ${mlir}" >&2
        return 1
    fi
