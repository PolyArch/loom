// S-box lookup function variant migrated from the legacy app corpus.
...
void sbox_lookup_kernel(const uint32_t *input_data, const uint32_t *input_sbox,
                        uint32_t *output_result, uint32_t size) {
    for (uint32_t i = 0; i < size; ++i) {
        const uint32_t index = input_data[i] & 0xffu;
        output_result[i] = input_sbox[index];
...
int main() {
    ...
    sbox_lookup_kernel(input.data(), sbox.data(), candidate.data(), kSize);
