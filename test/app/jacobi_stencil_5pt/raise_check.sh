#!/usr/bin/env bash
# Raise jacobi_stencil_5pt from LLVM IR into SCF MLIR.

set -euo pipefail
export LC_ALL=C

KERNEL="jacobi_stencil_5pt"
KERNEL_FN="jacobi_stencil_5pt_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        ...
        END { exit(has_forall && loads >= 4 && has_add && has_mul && has_store ? 0 : 1) }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no 5-point stencil loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
