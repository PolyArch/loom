#!/usr/bin/env bash
# Raise vecsum-while from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="vecsum-while"
KERNEL_FN="vecsum_while_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if (has_sum_loop && has_load && has_add) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no vector-sum loop in @${scope_name}: ${mlir}" >&2
        return 1
    fi
