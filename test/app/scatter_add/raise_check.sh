#!/usr/bin/env bash
# Raise scatter_add from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="scatter_add"
KERNEL_FN="scatter_add"
...
raise_one() {
    ...
    if ! awk -v has_call="${has_call}" '
        has_call == "yes" && /func\.func private @scatter_add/ {
            in_kernel = 1
            next
        }
        ...
        in_kernel {
            ...
            if (has_cmp && has_load && has_add && has_store) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no scatter-add loop with bounds check in ${mlir}" >&2
        return 1
    fi
