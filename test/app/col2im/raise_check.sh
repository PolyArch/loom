#!/usr/bin/env bash
# Raise col2im from .ll into SCF MLIR via loom-raise.

set -euo pipefail
export LC_ALL=C

KERNEL="col2im"
KERNEL_FN="col2im_kernel"
...
raise_one() {
    ...
    if ! awk "${scope_pattern}"'
        in_func {
            ...
            if ($0 ~ /arith\.addf|llvm\.fadd|llvm\.intr\.fadd|llvm\.intr\.fmuladd/) {
                has_accum = 1
            }
            if ($0 ~ /arith\.muli|llvm\.mul/) {
                muls += 1
            }
            if ($0 ~ /llvm\.load/) {
                loads += 1
            ...
            if ($0 ~ /llvm\.intr\.memset/) {
                memsets += 1
            }
            if (loops >= 5 && has_accum && muls >= 2 && loads >= 2 && stores >= 1 && memsets >= 1) {
                found = 1
            }
        }
        END { exit found ? 0 : 1 }
    ' "${mlir}"; then
        echo "[${KERNEL}/${variant}] no col2im accumulation path in @${scope_name}: ${mlir}" >&2
        return 1
    fi
    if [[ "${has_call}" == "yes" ]]; then
