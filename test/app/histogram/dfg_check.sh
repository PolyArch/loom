#!/usr/bin/env bash
# Lower histogram and require a real primary kernel graph.
...
if ! grep -E -q "dataflow\\.graph\\.func (private )?@${graph}(\\(|\\b)" "${dfg}"; then
    echo "[${KERNEL}] no histogram_kernel graph in ${dfg}" >&2
    exit 1
fi
...
require_graph_body_op "scf.for " "histogram update loop"
require_graph_body_op "scf.if " "histogram bounds guard"
require_graph_body_op "dataflow.load " "histogram bin load"
require_graph_body_op "dataflow.store " "histogram bin store"
