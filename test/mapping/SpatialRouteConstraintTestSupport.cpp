...
selectedRouteSets(const loom::pnr::FrozenSpatialPnrProblem &problem,
  ...
  for (const loom::pnr::RouteTreeNode &node :
    ...
    if (!node.isActive() ||
        node.parentArc == loom::pnr::getInvalidPnrIndex())
      continue;
...
void loom::test::exerciseSpatialRouteConstraintRelations(
  ...
  const auto traversalDomain = [&](llvm::ArrayRef<pnr::PnrIndex> traversals,
                                   const pnr::FrozenSpatialPnrProblem &problem) {
    std::string text = "[";
    for (auto [ordinal, traversal] : llvm::enumerate(traversals)) {
      if (ordinal)
        text += ", ";
      text += fabricAttr("fabric_physical_traversal_ref",
  ...
  if (routableNets.size() < 2 || !statefulNet)
    fail("fixture lacks two nonzero-hop routes and one stateful route");
  ...
  const auto emptyRoute = makeConstraints(
  ...
  auto emptyCandidate = take(pnr::createCanonicalSpatialCandidate(emptyProblem));
  ...
  if (!resourceProbe)
    fail("exact traversal/resource-state domains rejected their source route: " +
  ...
  if (constrainedRoute.traversals.empty() ||
      ...
      !llvm::all_of(constrainedRoute.traversals, [&](pnr::PnrIndex value) {
        return llvm::is_contained(statefulRoute.traversals, value);
      }) ||
  ...
  auto equalCandidate = take(pnr::createCanonicalSpatialCandidate(equalProblem));
  ...
  if (equalProbe) {
    if (selectedRouteSets(*equalProblem, *equalCandidate, routedNet).traversals !=
