...
int main() {
  ...
  const std::string currentVersion = "version<6, 0>";
  const std::size_t versionPosition = currentText.find(currentVersion);
  require(versionPosition != std::string::npos,
          "canonical SystemMapping does not use version 6.0");
  currentText.replace(versionPosition, currentVersion.size(), "version<5, 0>");
  {
    mlir::ScopedDiagnosticHandler capture(
        &context, [](mlir::Diagnostic &) { return mlir::success(); });
    auto legacy = mlir::parseSourceString<mlir::ModuleOp>(
        "module {\n" + currentText + "}\n", &context);
    require(!legacy, "mapping.system 5.0 was accepted by the 6.0 parser");
  }
