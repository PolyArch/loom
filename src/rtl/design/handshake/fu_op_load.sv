// fu_op_load.sv -- Memory load forwarding adapter (handshake.load).
//
...
//
// This is a pure forwarding FU. Memory access happens through the fabric
// routing graph: the addr goes out to the memory module through switches,
// and the memory response data comes back as in_data_1 through the same
// visible routing graph. The FU simply synchronizes the operands and
// forwards them to outputs.
//
// Matches SimFunctionUnitCore computeOutputs for Load body type:
//   result[0] = operand[1]  (data from memory response path)
...
module fu_op_load #(
  ...
  // Input 1: data (DATA_WIDTH, from memory response via fabric routing)
  ...
  // Output 0: data_out = forwarded in_data_1 (DATA_WIDTH)
  output logic [DATA_WIDTH-1:0]   out_data_0,
  output logic                    out_valid_0,
  input  logic                    out_ready_0,

  // Output 1: addr_out = forwarded in_data_0 (ADDR_WIDTH)
