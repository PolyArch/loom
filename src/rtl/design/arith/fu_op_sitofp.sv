// fu_op_sitofp.sv -- Signed integer to floating-point conversion FU operation.
//
...
module fu_op_sitofp #(
  ...
  // Input operand A (signed integer)
...
`ifndef SYNTH_FP_IP
  ...
  // Behavioral model: interpret input as signed integer, convert to FP.
  logic [WIDTH-1:0] conv_result;

  generate
    if (WIDTH == 64) begin : gen_f64
      real fp_val;
      assign fp_val = $itor($signed(in_data_0));
...
`else
  ...
  // TODO: Instantiate vendor signed-int-to-FP IP here.
