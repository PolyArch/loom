// fu_op_fptoui.sv -- Floating-point to unsigned integer conversion FU operation.
//
// Behavioral model for simulation; vendor IP under ifdef SYNTH_FP_IP.
// Supports WIDTH=32 (f32) and WIDTH=64 (f64).
// The result is the truncated (toward zero) unsigned integer value.
// Intrinsic latency: >= 1.
...
`ifndef SYNTH_FP_IP
  ...
  // Behavioral model: convert FP bits to real, truncate to unsigned integer.
  ...
  generate
    if (WIDTH == 64) begin : gen_f64
      ...
      assign conv_result = unsigned'(WIDTH'($rtoi(fp_val)));
    end : gen_f64
    else begin : gen_f32
      shortreal fp_val;
...
`else
  ...
  // TODO: Instantiate vendor FP-to-unsigned-int IP here.
