...
module fu_op_fma #(
  ...
  // Handshake: accept inputs when all three valid and output can accept.
  logic fire;
  assign fire = in_valid_0 & in_valid_1 & in_valid_2 & (~out_valid | out_ready);
...
`ifndef SYNTH_FP_IP
  ...
  // Behavioral model: convert to real, compute a*b+c, convert back.
  // Note: behavioral model uses two separate operations (not truly fused),
  // which may differ from hardware FMA in rounding. Acceptable for simulation.
  logic [WIDTH-1:0] result_comb;

  generate
    if (WIDTH == 64) begin : gen_f64
      real a_real, b_real, c_real, r_real;
      assign a_real = $bitstoreal(in_data_0);
      assign b_real = $bitstoreal(in_data_1);
      assign c_real = $bitstoreal(in_data_2);
      assign r_real = a_real * b_real + c_real;
      assign result_comb = $realtobits(r_real);
    end : gen_f64
    else if (WIDTH == 32) begin : gen_f32
      shortreal a_real, b_real, c_real, r_real;
      assign a_real = $bitstoshortreal(in_data_0);
      assign b_real = $bitstoshortreal(in_data_1);
      assign c_real = $bitstoshortreal(in_data_2);
      assign r_real = a_real * b_real + c_real;
...
`else
  ...
  // TODO: Instantiate vendor FMA IP here.
