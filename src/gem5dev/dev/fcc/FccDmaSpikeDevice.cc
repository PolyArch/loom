#include "dev/fcc/FccDmaSpikeDevice.hh"
...
FccDmaSpikeDevice::FccDmaSpikeDevice(const Params &p)
...
AddrRangeList
FccDmaSpikeDevice::getAddrRanges() const
{
    return {RangeSize(pioAddr_, pioSize_)};
}

Tick
FccDmaSpikeDevice::read(PacketPtr pkt)
{
    if (pkt->getSize() != 4)
        panic("FccDmaSpikeDevice only supports 32-bit MMIO accesses");
...
Tick
FccDmaSpikeDevice::write(PacketPtr pkt)
{
    if (pkt->getSize() != 4)
        panic("FccDmaSpikeDevice only supports 32-bit MMIO accesses");
    Addr offset = pkt->getAddr() - pioAddr_;
...
uint32_t
FccDmaSpikeDevice::readReg32(Addr offset) const
{
    switch (offset) {
      case kRegStatus:
        return statusReg_;
...
void
FccDmaSpikeDevice::writeReg32(Addr offset, uint32_t value)
{
...
void
FccDmaSpikeDevice::finishError(uint32_t code)
{
...
void
FccDmaSpikeDevice::startTransfer()
{
    if ((statusReg_ & kStatusBusy) != 0)
        return;
    if (transferSize_ == 0) {
...
void
FccDmaSpikeDevice::recomputeChecksum()
{
    uint64_t accum = 0;
...
void
FccDmaSpikeDevice::onReadComplete()
{
    if ((statusReg_ & kStatusBusy) == 0)
        return;
    recomputeChecksum();
...
void
FccDmaSpikeDevice::onWriteComplete()
{
    if ((statusReg_ & kStatusBusy) == 0)
        return;
