#include "dev/loom/LoomCgraDevice.hh"
...
static bool writeTraceJson(const std::filesystem::path &path,
                           const loom::sim::TraceDocument &doc)
{
    ...
    out << "  \"trace_kind\": \"loom_cycle_trace\",\n";
    out << "  \"producer\": \"loom_gem5_device\",\n";
    ...
    for (size_t idx = 0; idx < doc.events.size(); ++idx) {
        ...
        out << "    {\"cycle\": " << event.cycle
            << ", \"phase\": "
            << std::quoted(loom::sim::simPhaseName(event.phase))
            << ", \"epoch_id\": " << event.epochId
            << ", \"invocation_id\": " << event.invocationId
            << ", \"core_id\": " << event.coreId
            << ", \"hw_node_id\": " << event.hwNodeId
            << ", \"event_kind\": "
            << std::quoted(loom::sim::eventKindName(event.eventKind))
...
static bool writeAcceleratorStatsJson(
    const std::filesystem::path &path, const loom::sim::AcceleratorStats &stats,
    bool success, loom::sim::BoundaryReason reason,
    const std::string &errorMessage)
{
    ...
    out << "  \"termination\": ";
    writeEscapedString(out, loom::sim::boundaryReasonName(reason));
    out << ",\n";
    out << "  \"error_message\": ";
...
static bool writeStatText(const std::filesystem::path &path,
                          bool success,
                          const loom::sim::AcceleratorStats &stats,
                          loom::sim::BoundaryReason reason,
                          const std::string &errorMessage)
{
    std::ofstream out(path);
    if (!out)
        return false;
    out << "success: " << (success ? "true" : "false") << "\n";
    out << "termination: " << loom::sim::boundaryReasonName(reason) << "\n";
    out << "total_cycles: " << stats.totalCycles << "\n";
    out << "kernel_cycles: " << stats.kernelCycles << "\n";
    out << "device_elapsed_ticks: " << stats.deviceElapsedTicks << "\n";
    out << "memory_io_ticks: " << stats.memoryIoTicks << "\n";
...
LoomCgraDevice::LoomCgraDevice(const Params &p)
    : DmaDevice(p), pioAddr_(p.pio_addr), pioDelay_(p.pio_latency),
      ...
      workDir(p.work_dir),
      runtimeImage(std::make_unique<loom::sim::RuntimeImage>()),
      kernel(std::make_unique<loom::sim::CycleKernel>()),
...
AddrRangeList
LoomCgraDevice::getAddrRanges() const
{
    return {RangeSize(pioAddr_, pioSize_)};
}

void
LoomCgraDevice::resetDevice()
{
...
bool
LoomCgraDevice::initializeDirectRuntime()
{
    ...
    if (!runtimeImage || !kernel)
        return false;
    std::string error;
    if (!loom::sim::loadRuntimeImageBinary(simImagePath, *runtimeImage, error)) {
        warn("LoomCgraDevice: failed to load sim image %s: %s",
             simImagePath, error);
        return false;
    }
    return true;
...
bool
LoomCgraDevice::startDirectInvocation(const std::filesystem::path &invocationDir,
...
LoomCgraDevice::onDirectInvoke()
{
    ...
    activeErrorMessage_.clear();
    if (!success) {
        activeErrorMessage_ = loom::sim::boundaryReasonName(reason);
...
void
LoomCgraDevice::scheduleNextKernelBatch(uint64_t cyclesAdvanced)
{
...
void
LoomCgraDevice::issueKernelTransfers()
{
    ...
    while (!pendingKernelTransfers_.empty() &&
        ...
        auto *doneEvent = new EventFunctionWrapper(
            ...
            true);
        if (active.request.kind == loom::sim::MemoryRequestKind::Load) {
...
void
LoomCgraDevice::onKernelTransferComplete(uint64_t requestId)
{
    if (directPhase_ != DirectPhase::MemoryIO)
        return;
    ...
    loom::sim::MemoryCompletion completion;
    ...
    if (transfer.request.kind == loom::sim::MemoryRequestKind::Load) {
        uint64_t value = 0;
...
void
LoomCgraDevice::finishDirectInvocation(bool success,
                                      const std::string &errorMessage)
{
    loom::sim::AcceleratorStats accelStats;
    if (kernel) {
    ...
    bool wroteArtifacts =
        ...
        writeStatText(statPath, success, accelStats,
                      success ? loom::sim::BoundaryReason::InvocationDone
                              : loom::sim::BoundaryReason::Deadlock,
                      errorMessage) &&
...
uint32_t
LoomCgraDevice::readRegister32(Addr offset)
{
    switch (offset) {
      case kRegStatus:
        return statusReg;
...
void
LoomCgraDevice::writeRegister32(Addr offset, uint32_t value)
{
...
bool
LoomCgraDevice::beginConfigLoad()
{
...
void
LoomCgraDevice::onConfigLoadComplete()
{
...
bool
LoomCgraDevice::writeDirectReplyArtifacts(
...
Tick
LoomCgraDevice::read(PacketPtr pkt)
{
    const Addr offset = pkt->getAddr() - pioAddr_;
    if (pkt->getSize() != 4)
        panic("LoomCgraDevice only supports 32-bit MMIO accesses");
...
Tick
LoomCgraDevice::write(PacketPtr pkt)
{
    const Addr offset = pkt->getAddr() - pioAddr_;
    if (pkt->getSize() != 4)
        panic("LoomCgraDevice only supports 32-bit MMIO accesses");
