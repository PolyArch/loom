...
| `fabric.system` | Typed AccCore, InstructionCore, SpatialCore attachment, memory/service, Transport Architecture, domains, and explicit architecture-level system connectivity. | Interconnect Implementation identity, software execution semantics, selected Mapping, runtime policy, or observations. |
| TechMapping | Compute and Memory Realizations, selected capability templates or memory semantic encodings, and exact actor, operation, port, boundary, and internal-edge relations. | Concrete physical placement, routes, copied semantic parameters, raw configuration bits, QoR, or candidate ranking. |
