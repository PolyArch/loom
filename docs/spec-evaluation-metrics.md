...
The exact Evaluation registry schema 3.1 owns every `MetricKind`,
...
registry 3.1 gives each metric one owner-defined form at
`ScopeFormRef(0)`:
...
The Evaluation registry 3.1 requirements are exact: `CycleCount` form 0 and
...
Evaluation registry 3.1 retains finding ordinal `0` for
...
Evaluation registry 3.1 appends finding ordinal `1` for `cgra_closed_wait`.
...
schema is `loom.cgra_closed_wait_certificate 1.0`. The witness owns the exact
...
Evaluation registry 3.1 has these condition kinds:
...
Evaluation registry 3.1 contains the typed `ExecutionActivity` source form.
...
`Quantile` is request-specific and Metric-only in Evaluation registry 3.1. Its
...
The other six kinds are Base-only in Evaluation registry 3.1. Their assignment
...
owners carry Evaluation registry 3.1 kind and scope-form references without
