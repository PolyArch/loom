...
The exact Evaluation registry schema 3.2 owns every `MetricKind`,
...
registry 3.2 gives each metric one owner-defined form at
`ScopeFormRef(0)`:
...
The Evaluation registry 3.2 requirements are exact: `CycleCount` form 0 and
...
Evaluation registry 3.2 retains finding ordinal `0` for
...
Evaluation registry 3.2 declares finding ordinal `1` for `cgra_closed_wait`.
Its only scope form is `WholeExactCase`; its occurrence codec is the
Simulation-owned `loom.terminal_witness_ref 1.0`, and its terminal-witness
schema is `loom.cgra_closed_wait_certificate 1.1`. The witness owns the exact
Dataflow, Fabric, TechMapping, and SpatialMapping roots, the canonical dynamic
transfer subset referenced by the certificate, and one verified closed SCC of
typed actor-firing and storage-queue wait edges. The concrete witness exists
only in the referenced `SimulationExecution::Halted` terminal. Evidence stores
only the output-relative terminal-witness reference and cannot substitute a
second certificate.
...
Evaluation registry 3.2 has these condition kinds:
...
Evaluation registry 3.2 contains the typed `ExecutionActivity` source form.
Consuming it requires an activity-summary adopter for
`loom.simulation_execution 5.0`, an ordinal resolver, same-Request validation,
...
`Quantile` is request-specific and Metric-only in Evaluation registry 3.2. Its
...
The other six kinds are Base-only in Evaluation registry 3.2. Their assignment
...
owners carry Evaluation registry 3.2 kind and scope-form references without
