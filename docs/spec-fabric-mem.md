...
`loom.fabric 7.2` persists one closed typed relation. It does not persist an
...
through `loom.fabric 7.2`. They do not inherit C++ enum
...
The Fabric-owned clause tags are stable `loom.fabric 7.2` wire values:
...
same values through `loom.fabric 7.2`. They do
...
`loom.fabric 7.2`. Reusing one tagged egress across different resident rows
