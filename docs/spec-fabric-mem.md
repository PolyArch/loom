...
`loom.fabric 6.0` persists one closed typed relation. It does not persist an
...
through `loom.fabric 6.0`. They do not inherit C++ enum
...
The Fabric-owned clause tags are stable `loom.fabric 6.0` wire values:
...
same values through `loom.fabric 6.0`. They do
...
`loom.fabric 6.0`. Reusing one tagged egress across different resident rows
