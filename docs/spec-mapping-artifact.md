...
primary automated producer of mapping artifacts. A verifier-clean
mapping artifact may also be hand-authored, imported, or generated by
DSE tooling. Artifact validity is producer-independent. The mapping
artifact is independent from both software dataflow IR and hardware
Fabric ADG.
...
Consumers must accept a mapping artifact based on schema, identity, and
consumer-profile verification, not on whether it came from PnR. A
hand-authored fixture, importer output, or DSE-generated candidate is a
valid input to CGRA-sim, RTL-oriented lowering, reporting, or other
consumers when it satisfies the selected verifier profile.
...
* one artifact can represent a full mapping from a dataflow graph to a
  Fabric system ADG;
* artifact validity is independent of producer and can be checked by a
  verifier profile;
