...
Version 8.1 adds the required typed
`specialMathCapabilityProfile` scale parameter. `FullCatalog` names the exact
elementary-math capability relation generated by version 8.0, while
`PortableProviderClosed` names the narrower frozen relation defined below.
The two values are ordinary hardware semantics: they change the finalized
operation capabilities and cannot be inferred from a backend, installed tool,
or provider registry. All current presets select `PortableProviderClosed`.
...
and resolves a new target and ResolvedConfig identity. An ordinary 8.1
consumer rejects an 8.0 descriptor and never inserts this value while reading.
Selecting a current preset instead is an intentional change to
`PortableProviderClosed`, not migration. The Fabric must be re-finalized from
the exact 8.1 scale, and every dependent Mapping, ConfigurationABI,
Deployment, HardwareImplementation, and EDA provenance record must be
regenerated against the resulting identities.
...
| `ScalarIntegerCountZeros` | integer widths `{8, 16, 32, 64}` |
...
vector integer add/subtract, saturating integer add/subtract,
leading/trailing-zero count, logic, shift, compare/min/max, select, multiply,
...
`BuiltinSpecialMathCapabilityProfile` is a closed typed domain:
...
It selects only elementary-math parameters. Scalar signed and unsigned integer
divide/remainder and scalar floating divide/remainder retain their complete
catalog parameters under both profiles. The version 8.1 elementary-math table
is:

| Profile | Implementation families | Floating formats | Behavior | Accuracy guarantee |
| --- | --- | --- | --- | --- |
| `FullCatalog` | every `ScalarMath*` family listed above | `{f16, bf16, f32, f64}` | strict IEEE | correctly rounded |
| `PortableProviderClosed` | `sin`, `cos`, `tan` | `{f16, bf16}` | strict IEEE | correctly rounded |
| `PortableProviderClosed` | `sinh`, `cosh`, `tanh`, `log`, `log2`, `log10`, `log1p` | `{f16, bf16, f32}` | round-to-nearest-even, IEEE NaN, preserved subnormal and signed zero, required `afn` | maximum 4 ULP |
| `PortableProviderClosed` | `exp`, `exp2`, `expm1`, `erf`, `pow` | `{f16, bf16}` | round-to-nearest-even, IEEE NaN, preserved subnormal and signed zero, required `afn` | maximum 4 ULP |
| `PortableProviderClosed` | `floor`, `ceil`, `round`, `trunc`, `roundeven`, `sqrt`, `rsqrt` | `{f16, bf16, f32, f64}` | strict IEEE | correctly rounded |

This table is the target-owned capability source of truth. The name
`PortableProviderClosed` imposes a conformance obligation on the production
portable RTL registry; it does not make that registry a second operation
catalog and does not cause Builder execution to query available providers.
Changing any row requires another template schema version. `FullCatalog`
remains a valid explicit target even when a selected RTL recipe reports typed
`Unsupported` for one of its capabilities.
