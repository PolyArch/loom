...
llvm::Expected<FinalizedSpatialMapping> rebaseSpatialMapping(
  ...
  if (llvm::Error error = detail::remapSpatialMappingModuleReferences(
          parsed->root, parentFabric->view(), childFabric,
          moduleCorrespondence))
    return std::move(error);
  parsed->root.setTechMappingAttr(::mapping::ArtifactIdentityAttr::get(
      parsed->context.get(),
      identityBytes(parsed->context.get(),
                    childTechMapping.view().identity())));
