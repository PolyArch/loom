...
#define DEBUG_TYPE "loom-mark-dfg-domain"
...
struct MarkDFGDomainPass
  ...
  StringRef getArgument() const override { return "loom-mark-dfg-domain"; }
  ...
  void runOnOperation() override {
    ...
    ADGCapacity adg;
    if (auto attr = module->getAttrOfType<IntegerAttr>("loom.adg_total_pes"))
      adg.totalPEs = attr.getInt();
    if (auto attr = module->getAttrOfType<IntegerAttr>("loom.adg_total_fus"))
      adg.totalFUs = attr.getInt();
    if (auto attr =
            module->getAttrOfType<IntegerAttr>("loom.adg_total_mem_modules"))
      adg.totalMemModules = attr.getInt();
    if (auto attr =
            module->getAttrOfType<IntegerAttr>("loom.adg_total_mem_regions"))
    ...
    llvm::outs() << "loom: enumerated " << candidates.size()
                 << " DFG candidate(s):\n";
    ...
    llvm::outs() << "loom: " << feasibleCount << " of " << candidates.size()
    ...
    if (!selected) {
      llvm::outs() << "loom: no feasible DFG candidate; skipping DFG "
                      "conversion\n";
      return;
    }

    llvm::outs() << "loom: selected candidate: " << selected->describe()
                 << "\n";

    // Mark the parent function with loom.dfg_candidate attribute.
    // For whole-function and loop candidates, the parent function
    // is the unit of SCF->DFG conversion.
    selected->parentFunc->setAttr("loom.dfg_candidate", UnitAttr::get(ctx));
...
std::unique_ptr<Pass> loom::createMarkDFGDomainPass() {
