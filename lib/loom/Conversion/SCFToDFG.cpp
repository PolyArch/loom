...
//===----------------------------------------------------------------------===//
//
// Converts SCF-level functions marked with loom.dfg_candidate to
...
//
// DFGConverter method implementations and the pass definition live here.
// Helper functions, types, MemoryCtrlBuilder, and candidate extraction
// utilities are in SCFToDFGInternal.h.
//
//===----------------------------------------------------------------------===//

#include "SCFToDFGInternal.h"

#define DEBUG_TYPE "loom-scf-to-dfg"

using namespace mlir;
using namespace loom::detail;

//===----------------------------------------------------------------------===//
// DFGConverter core implementation
//===----------------------------------------------------------------------===//
...
struct ConvertSCFToDFGPass
  ...
  StringRef getArgument() const override { return "loom-scf-to-dfg"; }
  ...
  void runOnOperation() override {
    ...
    unsigned maxJoinFanin =
        getModuleUIntAttr(module, "loom.adg_max_join_fanin").value_or(0);

    SmallVector<func::FuncOp, 4> candidates;
    module.walk([&](func::FuncOp func) {
      if (func->hasAttr("loom.dfg_candidate"))
    ...
    for (func::FuncOp func : candidates) {
      ...
      func.walk([&](Operation *op) {
        if (op != func.getOperation() && op->hasAttr("loom.selected_dfg_root"))
          selectedRoot = op;
      });
      ...
      if (sourceFunc != func) {
        ...
        Attribute regionIdAttr = func->getAttr("loom.dfg_region_id");
        Attribute estimatedPEsAttr = func->getAttr("loom.dfg_estimated_pes");
        Attribute estimatedMemAttr = func->getAttr("loom.dfg_estimated_mem");
        func.erase();
        hsFunc->setAttr(SymbolTable::getSymbolAttrName(),
                        StringAttr::get(module.getContext(), accelName));
        if (regionIdAttr)
          hsFunc->setAttr("loom.dfg_region_id", regionIdAttr);
        if (estimatedPEsAttr)
          hsFunc->setAttr("loom.dfg_estimated_pes", estimatedPEsAttr);
        if (estimatedMemAttr)
          hsFunc->setAttr("loom.dfg_estimated_mem", estimatedMemAttr);
      }
    }
...
std::unique_ptr<Pass> loom::createConvertSCFToDFGPass() {
