...
LogicalResult
BitsType::verify(function_ref<InFlightDiagnostic()> emitError,
                 unsigned width) {
  if (width < 1 || width > 4096)
    return emitError() << "bits width must be 1-4096; got " << width;
  return success();
}
...
TaggedType::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  // valueType must be one of the allowed native types or BitsType.
  bool validValue = valueType.isInteger(1) || valueType.isInteger(8) ||
                    ...
                    isa<BitsType>(valueType);
  if (!validValue)
    return emitError()
           << "tagged value type must be one of i1, i8, i16, i32, i64, "
              "bf16, f16, f32, f64, index, none, !dataflow.bits<N>; got "
           << valueType;
  ...
  if (width < 1 || width > 16)
    return emitError() << "[CPL_TAG_WIDTH_RANGE] "
