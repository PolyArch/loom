...
std::vector<IdIndex> collectCriticalBoundaryEdges(const Graph &dfg) {
  ...
  for (IdIndex edgeId = 0; edgeId < static_cast<IdIndex>(dfg.edges.size());
    ...
    if (!srcNode || !dstNode)
      continue;
    if (srcNode->kind != Node::OperationNode || dstNode->kind != Node::OperationNode)
      continue;
    bool srcBoundary = getNodeAttrStr(srcNode, "op_name") == "techmap_group";
    bool dstBoundary = getNodeAttrStr(dstNode, "op_name") == "techmap_group";
