...
static unsigned getOpConfigBits(mlir::Operation &op, unsigned dataWidth) {
  ...
  if (mlir::isa<loom::fabric::MuxOp>(op)) {
    auto muxOp = mlir::cast<loom::fabric::MuxOp>(op);
...
static std::string detectFUMemoryOpType(loom::fabric::FunctionUnitOp fuOp) {
...
computeMemoryWidths(loom::fabric::FunctionUnitOp fuOp,
...
std::string generateFUBody(loom::fabric::FunctionUnitOp fuOp,
  ...
  FUEmitContext ctx{emitter, registry, fpIpProfile,
                    llvm::DenseMap<mlir::Value, std::string>(), 0, false, 0,
                    dataWidth,
                    llvm::DenseMap<mlir::Value, llvm::SmallVector<std::string, 2>>()};
  ...
  if (auto yieldOp = mlir::dyn_cast<loom::fabric::YieldOp>(
