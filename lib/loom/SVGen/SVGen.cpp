...
static bool collectFUDeps(loom::fabric::FunctionUnitOp fuOp,
  ...
  for (auto &op : bodyBlock.getOperations()) {
    ...
    if (mlir::isa<loom::fabric::MuxOp>(op)) {
...
static bool registerOpDeps(mlir::Operation &op, SVModuleRegistry &registry,
  ...
  if (auto spatialPE = mlir::dyn_cast<loom::fabric::SpatialPEOp>(op)) {
    ...
    for (auto &bodyOp : spatialPE.getBody().front().getOperations()) {
      if (auto fuOp =
              mlir::dyn_cast<loom::fabric::FunctionUnitOp>(bodyOp)) {
...
bool generateSV(mlir::ModuleOp adgModule, mlir::MLIRContext *ctx,
  ...
  adgModule->walk([&](loom::fabric::ModuleOp mod) { fabricMod = mod; });
  if (!fabricMod) {
  ...
  llvm::StringMap<loom::fabric::FunctionUnitOp> fuDefs;
