...
static unsigned getBodyOpConfigBitsForLayout(mlir::Operation &op,
                                             unsigned dataWidth) {
  if (auto muxOp = mlir::dyn_cast<loom::fabric::MuxOp>(op)) {
...
static unsigned computeFUBodyCfgBits(loom::fabric::FunctionUnitOp fuOp) {
...
static unsigned computeSpatialSwConfigBits(loom::fabric::SpatialSwOp op) {
...
static unsigned computeTemporalSwConfigBits(loom::fabric::TemporalSwOp op) {
  auto fnType = op.getFunctionType();
  unsigned numIn = fnType.getNumInputs();
  unsigned numOut = fnType.getNumResults();
...
static unsigned computeAddTagConfigBits(loom::fabric::AddTagOp op) {
  mlir::Type resultType = op.getResult().getType();
  return SVEmitter::getTagWidth(resultType);
}

// Compute config bit count for a map_tag.
static unsigned computeMapTagConfigBits(loom::fabric::MapTagOp op) {
...
static unsigned computeFifoConfigBits(loom::fabric::FifoOp op) {
  if (op.getBypassable())
    return 1;
  return 0;
}
...
static loom::fabric::FunctionUnitOp
resolveFUInstance(loom::fabric::InstanceOp instOp) {
  llvm::StringRef refName = instOp.getModule();
  auto searchRoot = instOp->getParentOfType<mlir::ModuleOp>();
  if (!searchRoot)
    return nullptr;
  loom::fabric::FunctionUnitOp resolved = nullptr;
  searchRoot->walk([&](loom::fabric::FunctionUnitOp fuOp) {
...
static void accumulateFUStats(mlir::Block &peBody, unsigned &numFU,
  ...
  auto processFU = [&](loom::fabric::FunctionUnitOp fuOp) {
    ++numFU;
...
static unsigned computeSpatialPEConfigBits(loom::fabric::SpatialPEOp op) {
  auto fnType = op.getFunctionType();
...
static unsigned computeTemporalPEConfigBits(loom::fabric::TemporalPEOp op) {
  auto fnType = op.getFunctionType();
  unsigned numPEInputs = fnType.getNumInputs();
  unsigned numPEOutputs = fnType.getNumResults();
...
static unsigned computeMemoryConfigBits(loom::fabric::MemoryOp op) {
...
static unsigned computeExtMemoryConfigBits(loom::fabric::ExtMemoryOp op) {
  unsigned numRegion = static_cast<unsigned>(std::max<int64_t>(1, op.getNumRegion()));
  return numRegion * 32;
}
  ...
  for (auto &op : body.getOperations()) {
    if (auto spatialSw = mlir::dyn_cast<loom::fabric::SpatialSwOp>(op)) {
      ...
      addSlice(name, computeMemoryConfigBits(memOp));
                   ...
                   mlir::dyn_cast<loom::fabric::ExtMemoryOp>(op)) {
      std::string name = SVEmitter::sanitizeName(
      ...
      searchRoot->walk([&](loom::fabric::SpatialPEOp peOp) {
        if (found)
          return;
      ...
      if (!found) {
        searchRoot->walk([&](loom::fabric::TemporalPEOp peOp) {
          if (found)
            return;
          auto sym = peOp.getSymName();
          if (sym && *sym == refName) {
      ...
      if (!found) {
        searchRoot->walk([&](loom::fabric::MemoryOp memOp) {
          if (found)
            return;
      ...
      if (!found) {
        searchRoot->walk([&](loom::fabric::ExtMemoryOp extMemOp) {
          if (found)
            return;
