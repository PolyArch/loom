...
static std::string getModuleName(mlir::Operation &op) {
  if (auto spatialSw = mlir::dyn_cast<loom::fabric::SpatialSwOp>(op)) {
    return "fabric_spatial_sw";
  }
  if (auto temporalSw = mlir::dyn_cast<loom::fabric::TemporalSwOp>(op)) {
  ...
  if (auto fifo = mlir::dyn_cast<loom::fabric::FifoOp>(op))
    return "fabric_fifo";
  if (auto memOp = mlir::dyn_cast<loom::fabric::MemoryOp>(op))
    return "fabric_memory";
  if (auto extMem = mlir::dyn_cast<loom::fabric::ExtMemoryOp>(op))
...
static void emitSwitchConns(TopEmitContext &ctx, mlir::Operation &op,
  ...
  bool isTemporal = mlir::isa<loom::fabric::TemporalSwOp>(checkOp);
...
static void emitMemoryConns(TopEmitContext &ctx, mlir::Operation &op,
  ...
  bool isExtMemory = mlir::isa<loom::fabric::ExtMemoryOp>(memDef);
  bool isPrivate = false;
  if (auto memOp = mlir::dyn_cast<loom::fabric::MemoryOp>(memDef))
...
static void emitPrewrittenInstance(TopEmitContext &ctx, mlir::Operation &op,
  ...
  if (auto spatialSw = mlir::dyn_cast<loom::fabric::SpatialSwOp>(op)) {
    ...
    isSwitch = true;
                 ...
                 mlir::dyn_cast<loom::fabric::TemporalSwOp>(op)) {
    ...
    isSwitch = true;
  } else if (auto fifo = mlir::dyn_cast<loom::fabric::FifoOp>(op)) {
    params.push_back(".DEPTH(" + std::to_string(fifo.getDepth()) + ")");
    if (fifo.getBypassable())
      params.push_back(".BYPASSABLE(1)");
    isFifo = true;
  } else if (auto addTag = mlir::dyn_cast<loom::fabric::AddTagOp>(op)) {
  ...
  } else if (auto delTag = mlir::dyn_cast<loom::fabric::DelTagOp>(op)) {
  ...
  } else if (auto mapTag = mlir::dyn_cast<loom::fabric::MapTagOp>(op)) {
    ...
    params.push_back(".NUM_REGION(" + std::to_string(memOp.getNumRegion()) +
                      ")");
    isMemory = true;
  } else if (auto extMem = mlir::dyn_cast<loom::fabric::ExtMemoryOp>(op)) {
...
void generateTopModule(loom::fabric::ModuleOp fabricMod,
  ...
  if (auto yieldOp = mlir::dyn_cast<loom::fabric::YieldOp>(terminator)) {
  ...
  auto registerSymsFrom = [&](mlir::Operation &op) {
    ...
    if (auto pe = mlir::dyn_cast<loom::fabric::SpatialPEOp>(op))
      tryRegister(pe.getSymName());
    else if (auto pe = mlir::dyn_cast<loom::fabric::TemporalPEOp>(op))
      tryRegister(pe.getSymName());
    else if (auto mem = mlir::dyn_cast<loom::fabric::MemoryOp>(op))
      tryRegister(mem.getSymName());
    else if (auto ext = mlir::dyn_cast<loom::fabric::ExtMemoryOp>(op))
      tryRegister(ext.getSymName());
    else if (auto sw = mlir::dyn_cast<loom::fabric::SpatialSwOp>(op))
      tryRegister(sw.getSymName());
    else if (auto sw = mlir::dyn_cast<loom::fabric::TemporalSwOp>(op))
      tryRegister(sw.getSymName());
    else if (auto fu = mlir::dyn_cast<loom::fabric::FunctionUnitOp>(op))
  ...
  for (auto &op : body.getOperations()) {
    ...
    if (auto instOp = mlir::dyn_cast<loom::fabric::InstanceOp>(op)) {
      ...
      bool isPE = defOp &&
                  (mlir::isa<loom::fabric::SpatialPEOp>(defOp) ||
                   mlir::isa<loom::fabric::TemporalPEOp>(defOp));
      bool isMemory = defOp &&
                      (mlir::isa<loom::fabric::MemoryOp>(defOp) ||
                       mlir::isa<loom::fabric::ExtMemoryOp>(defOp));
      bool isSwitch = defOp &&
                      (mlir::isa<loom::fabric::SpatialSwOp>(defOp) ||
                       mlir::isa<loom::fabric::TemporalSwOp>(defOp));
      bool isFU = defOp &&
                  mlir::isa<loom::fabric::FunctionUnitOp>(defOp);
      bool isFifo = defOp &&
                    mlir::isa<loom::fabric::FifoOp>(defOp);
      ...
      if (isMemory || isSwitch || isFifo) {
        ...
        if (auto spatialSw =
                mlir::dyn_cast<loom::fabric::SpatialSwOp>(defOp)) {
          auto swFnType = spatialSw.getFunctionType();
          params.push_back(".NUM_IN(" +
                           std::to_string(swFnType.getNumInputs()) + ")");
          params.push_back(".NUM_OUT(" +
          ...
          defIsSwitch = true;
                       ...
                       mlir::dyn_cast<loom::fabric::TemporalSwOp>(defOp)) {
          auto tswFnType = temporalSw.getFunctionType();
          params.push_back(".NUM_IN(" +
          ...
          defIsSwitch = true;
                       ...
                       mlir::dyn_cast<loom::fabric::FifoOp>(defOp)) {
          params.push_back(".DEPTH(" + std::to_string(fifo.getDepth()) +
                           ")");
          if (fifo.getBypassable())
            params.push_back(".BYPASSABLE(1)");
        } else if (auto memOp =
                       mlir::dyn_cast<loom::fabric::MemoryOp>(defOp)) {
          ...
          defIsMemory = true;
                       ...
                       mlir::dyn_cast<loom::fabric::ExtMemoryOp>(defOp)) {
          params.push_back(".LD_COUNT(" +
        ...
        if (mlir::isa<loom::fabric::SpatialSwOp>(defOp) ||
            mlir::isa<loom::fabric::TemporalSwOp>(defOp) ||
            mlir::isa<loom::fabric::MemoryOp>(defOp) ||
            mlir::isa<loom::fabric::ExtMemoryOp>(defOp)) {
          hasConfig = true;
    ...
    if (auto spatialPE = mlir::dyn_cast<loom::fabric::SpatialPEOp>(op)) {
    ...
    if (auto temporalPE = mlir::dyn_cast<loom::fabric::TemporalPEOp>(op)) {
      auto peIt = peModuleNames.find(&op);
      std::string peModName =
          peIt != peModuleNames.end()
              ? peIt->second
    ...
    if (!modName.empty()) {
      ...
      if (mlir::isa<loom::fabric::SpatialSwOp>(op) ||
          mlir::isa<loom::fabric::TemporalSwOp>(op) ||
          mlir::isa<loom::fabric::AddTagOp>(op) ||
          mlir::isa<loom::fabric::MapTagOp>(op) ||
          mlir::isa<loom::fabric::MemoryOp>(op) ||
          mlir::isa<loom::fabric::ExtMemoryOp>(op)) {
        hasConfig = true;
      } else if (auto fifo = mlir::dyn_cast<loom::fabric::FifoOp>(op)) {
        if (fifo.getBypassable())
          hasConfig = true;
