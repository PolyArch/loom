...
std::string makeVizTitle(mlir::ModuleOp adgModule,
  ...
  return hasMapping ? "loom viz (mapped)" : "loom viz";
}
...
std::string loadVizSidecarJson(mlir::ModuleOp adgModule,
  ...
  loom::fabric::ModuleOp fabricMod;
  adgModule->walk([&](loom::fabric::ModuleOp mod) {
    if (!fabricMod)
      fabricMod = mod;
  });
  if (!fabricMod)
  ...
  if (!buffer) {
    llvm::errs() << "loom viz: warning: cannot open ADG viz sidecar "
                 << resolved << "\n";
    return "null";
  }
...
mlir::LogicalResult exportVizOnly(const std::string &outputPath,
  ...
  if (ec) {
    llvm::errs() << "loom viz: cannot open " << outputPath << "\n";
    return mlir::failure();
  }
