...
function gridLayoutEngine(allNodes, edges) {
  ...
  if (!useNeato) {
    ...
    var maxC = 0, maxR = 0;
    for (var i = 0; i < N; i++) {
      if (gcArr[i] > maxC) maxC = gcArr[i];
      if (grArr[i] > maxR) maxR = grArr[i];
    }
      ...
      for (var d = 1; d <= maxD; d++) {
        for (var dx = -d; dx <= d; dx++) {
          ...
          for (var ci = 0; ci < cands.length; ci++) {
            ...
            if (isFree(gc, gr)) return { gc: gc, gr: gr };
          }
        }
      }
...
function manhattanRouter(edges, layout) {
  ...
  function bfs(srcId, dstId) {
    ...
    // Directly adjacent (shared edge, not diagonal): skip BFS
    if (Math.abs(sp.gc - dp.gc) + Math.abs(sp.gr - dp.gr) <= 1)
      return [];
...
function buildManhattanSVG(srcP, dstP, path, lanes, crossings) {
  ...
  for (var i = 1; i < pts.length; i++) {
    ...
    if (i >= 1 && i <= path.length && crossings) {
      var pi = i - 1; // index into path array
...
function computeGridPorts(nodes, edges, gpos) {
  ...
  nodes.forEach(function(n) {
    ...
    function placeOnSide(ports, axis, fixedVal, start, end) {
      if (ports.length === 0) return;
      var step = (end - start) / (ports.length + 1);
      ports.forEach(function(pid, i) {
        var t = start + step * (i + 1);
        if (axis === "h") portPos[pid] = { x: cx + t, y: cy + fixedVal };
        else portPos[pid] = { x: cx + fixedVal, y: cy + t };
      });
    }
    placeOnSide(sides.top, "h", -halfH, -halfW, halfW);
    placeOnSide(sides.right, "v", halfW, -halfH, halfH);
    placeOnSide(sides.bottom, "h", halfH, -halfW, halfW);
    placeOnSide(sides.left, "v", -halfW, -halfH, halfH);
  });
...
function renderADG() {
  ...
  adgGraph.nodes.forEach(function(n) {
    ...
    // Anonymous switches: show "switch" on first line, "(XX)" on second line.
    var switchMatch = /^node_(\d+)$/.exec(label);
    if (switchMatch && n.type === "fabric.switch") {
      var tEl = ng.append("text")
        .attr("text-anchor", "middle")
        .attr("fill", colors.text).attr("font-size", "7px");
      tEl.append("tspan").attr("x", 0).attr("dy", "-0.3em").text("switch");
      tEl.append("tspan").attr("x", 0).attr("dy", "1.1em").text("(" + switchMatch[1] + ")");
    } else {
...
function highlightRoute(edgeId) {
  ...
  route.hwPath.forEach(function(hop) {
    if (hop.hwEdgeId) {
      ...
      if (!edgeEl.empty()) edgeEl.classed("route-trace", true);
    }
  });
}
...
function scrollToElement(el, panel) {
  ...
  if (!svg || !zoom) return;
  var svgNode = svg.node();
  var svgW = svgNode.clientWidth, svgH = svgNode.clientHeight;
  ...
  if (screenX < 0 || screenX > svgW || screenY < 0 || screenY > svgH) {
    ...
    svg.transition().duration(400)
      .call(zoom.transform, d3.zoomIdentity.translate(newTx, newTy).scale(currentTransform.k));
  }
}
