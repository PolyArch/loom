...
function initTracePlayback() {
  ...
  // Build per-node adjacency edge lookup for edge highlighting.
  var nodeEdgeAdj = {};
  if (typeof adgGraph !== "undefined" && adgGraph) {
    adgGraph.edges.forEach(function(e) {
      if (!nodeEdgeAdj[e.srcNode]) nodeEdgeAdj[e.srcNode] = [];
      if (!nodeEdgeAdj[e.dstNode]) nodeEdgeAdj[e.dstNode] = [];
      nodeEdgeAdj[e.srcNode].push(e.id);
      nodeEdgeAdj[e.dstNode].push(e.id);
    });
  }
  ...
  function showCycle(cycle) {
    ...
    for (var i = 0; i < events.length; i++) {
      ...
      // For EV_ROUTE_USE, also highlight adjacent edges.
      if (kind === 3 && nodeEdgeAdj[nodeId]) {
        var adjEdges = nodeEdgeAdj[nodeId];
        for (var j = 0; j < adjEdges.length; j++) {
          var edgeEl = document.getElementById(adjEdges[j]);
