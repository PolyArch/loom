...
function initTracePlayback() {
  ...
  // Stat time-axis playback: animate heatmap through time windows.
  var statTimeline = traceData.statTimeline || null;
  var statSlider = document.getElementById("stat-timeline-slider");
  var statLabel = document.getElementById("stat-timeline-label");
  var statPlayBtn = document.getElementById("stat-play");
  var statPlaying = false;
  var statTimer = null;
  var currentStatWindow = 0;

  function applyStatWindow(win) {
    if (!statTimeline || !statTimeline.nodes) return;
    currentStatWindow = Math.max(0, Math.min(win, statTimeline.numWindows - 1));
    if (statSlider) statSlider.value = currentStatWindow;
    if (statLabel) {
      var startCycle = (traceData.configCycles || 0) +
          currentStatWindow * statTimeline.windowSize;
      var endCycle = startCycle + statTimeline.windowSize;
      statLabel.textContent = "Stat: cycles " + startCycle + "-" + endCycle;
    }
    // Apply per-node utilization for this window.
    var nodes = statTimeline.nodes;
    var nids = Object.keys(nodes);
    for (var k = 0; k < nids.length; k++) {
      var nid = nids[k];
      var vals = nodes[nid];
      var util = (currentStatWindow < vals.length) ? vals[currentStatWindow] : 0;
      var nodeGroup = document.getElementById("hw_" + nid);
      if (!nodeGroup) continue;
      var rect = nodeGroup.querySelector("rect");
      if (!rect) continue;
      if (!savedNodeFills["st_" + nid])
        savedNodeFills["st_" + nid] = rect.getAttribute("fill");
      rect.setAttribute("fill", utilToColor(util));
      rect.setAttribute("opacity", "0.85");
    }
  }

  function clearStatWindow() {
    if (!statTimeline || !statTimeline.nodes) return;
    var nids = Object.keys(statTimeline.nodes);
    for (var k = 0; k < nids.length; k++) {
      var nid = nids[k];
      var nodeGroup = document.getElementById("hw_" + nid);
      if (!nodeGroup) continue;
      var rect = nodeGroup.querySelector("rect");
      if (!rect) continue;
      var saved = savedNodeFills["st_" + nid];
      if (saved) {
        rect.setAttribute("fill", saved);
        rect.removeAttribute("opacity");
      }
  ...
  if (statSlider && statTimeline) {
    statSlider.max = statTimeline.numWindows - 1;
    statSlider.addEventListener("input", function() {
      applyStatWindow(parseInt(statSlider.value, 10));
    });
  }
  if (statPlayBtn && statTimeline) {
    statPlayBtn.addEventListener("click", function() {
      statPlaying = !statPlaying;
      statPlayBtn.textContent = statPlaying ? "Pause" : "Play";
      if (statPlaying) {
        statTimer = setInterval(function() {
          currentStatWindow++;
          if (currentStatWindow >= statTimeline.numWindows) {
            currentStatWindow = 0;
          }
          applyStatWindow(currentStatWindow);
        }, 200);
      } else if (statTimer) {
