...
function initTracePlayback() {
  ...
  // Build per-node incoming/outgoing edge lookups for route highlighting.
  // nodeInEdges[nodeId] = array of edge IDs where dstNode === nodeId (input edges).
  // nodeOutEdges[nodeId] = array of edge IDs where srcNode === nodeId (output edges).
  var nodeInEdges = {};
  var nodeOutEdges = {};
  if (typeof adgGraph !== "undefined" && adgGraph) {
    adgGraph.edges.forEach(function(e) {
      if (!nodeInEdges[e.dstNode]) nodeInEdges[e.dstNode] = [];
      if (!nodeOutEdges[e.srcNode]) nodeOutEdges[e.srcNode] = [];
      nodeInEdges[e.dstNode].push(e.id);
      nodeOutEdges[e.srcNode].push(e.id);
    });
  }
  ...
  function showCycle(cycle) {
    ...
    for (var i = 0; i < events.length; i++) {
      ...
      // arg0 = input port index on the switch node.
      if (kind === 3) {
        var arg0 = events[i][2] || 0;
        var inEdges = nodeInEdges[nodeId];
        if (inEdges && arg0 < inEdges.length) {
          var edgeEl = document.getElementById(inEdges[arg0]);
