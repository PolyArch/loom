...
function init() {
  ...
  dfgStatusIdx = buildDFGStatusIndex();
  buildPortLookup();
  adgConnectionIdx = buildADGConnectionIndex();
  ...
  if (toolbar && MAPPING_DATA) {
    ...
    btnMap.className = "mapping-toggle" + (mappingEnabled ? " active" : "");
    btnMap.textContent = mappingEnabled ? "Mapping: On" : "Mapping: Off";
    ...
    btnSbs.className = "mode-btn" + (currentMode === "sidebyside" ? " active" : "");
    ...
    btnOvl.className = "mode-btn" +
      (currentMode === "overlay" ? " active" : "") +
      (mappingEnabled ? "" : " disabled");
    btnOvl.textContent = "Overlay";
    btnOvl.disabled = !mappingEnabled;
    btnOvl.addEventListener("click", function() { setMode("overlay"); });
    toolbar.insertBefore(btnOvl, statusBar);
  }

  if (typeof initSimulationPlayback === "function")
    initSimulationPlayback(toolbar, document.getElementById("status-bar"));
  ...
  var adgPanel = document.getElementById("panel-adg");
  var dfgPanel = document.getElementById("panel-dfg");
  function setSideBySideSplit(pct) {
    var clamped = Math.max(20, Math.min(80, pct));
    graphArea.style.setProperty("--dfg-panel-width", clamped + "%");
  }
  if (divider) {
    var dragging = false;
    function fitPanelsAfterResize() {
      if (renderADG._svg && renderADG._g && renderADG._zoom)
        fitView(renderADG._svg, renderADG._g, renderADG._zoom);
      if (renderDFG._svg && renderDFG._g && renderDFG._zoom)
        fitView(renderDFG._svg, renderDFG._g, renderDFG._zoom);
    ...
    document.addEventListener("mousemove", function(e) {
      ...
      setSideBySideSplit(pct);
    });
    document.addEventListener("mouseup", function() {
      if (!dragging) return;
      dragging = false;
      fitPanelsAfterResize();
    });
  }
