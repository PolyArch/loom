...
function buildSimulationIndex() {
  ...
  // Build per-port edge indices so we can highlight only the edges that
  // actually transferred (important for cond_br true/false and mux select).
  // outputEdgesByHwNodePort[hwNodeId][fromPort] = [swEdgeIds]
  // inputEdgesByHwNodePort[hwNodeId][toPort] = [swEdgeIds]
  idx.outputEdgesByHwNodePort = {};
  idx.inputEdgesByHwNodePort = {};
  ...
  if (mappingIdx && mappingIdx.hwNodeToSwNodes && DFG_DATA &&
    ...
    Object.keys(mappingIdx.hwNodeToSwNodes).forEach(function(hwNodeId) {
      ...
      DFG_DATA.edges.forEach(function(edge) {
        if (!edge) return;
        var isOut = swNodeSet[String(edge.from)];
        var isIn = swNodeSet[String(edge.to)];
        if (isOut || isIn)
          pushUnique(swEdges, edge.id);
        if (isOut) {
          var fp = edge.from_port || 0;
          if (!outByPort[fp]) outByPort[fp] = [];
          pushUnique(outByPort[fp], edge.id);
        }
        if (isIn) {
          var tp = edge.to_port || 0;
          if (!inByPort[tp]) inByPort[tp] = [];
          pushUnique(inByPort[tp], edge.id);
        }
      });
      idx.incidentSwEdgesByHwNode[hwNodeId] = swEdges;
      idx.outputEdgesByHwNodePort[hwNodeId] = outByPort;
      idx.inputEdgesByHwNodePort[hwNodeId] = inByPort;
    });
  }
  ...
  frame.events.forEach(function(ev) {
    ...
    if (meta) {
      if (meta.kind === "function_unit") {
        ...
        // Use per-port transfer bitmasks (arg0=output, arg1=input) to only
        // highlight edges that actually transferred data this cycle.
        // For cond_br, only the taken branch lights up; for mux, only
        // the selected input lights up.
        var outMask = ev.arg0 || 0;
        var inMask = ev.arg1 || 0;
        var outByPort = simTraceIdx.outputEdgesByHwNodePort[ev.hw_node_id] || {};
        var inByPort = simTraceIdx.inputEdgesByHwNodePort[ev.hw_node_id] || {};
        var hasPortInfo = (outMask !== 0 || inMask !== 0);
        if (hasPortInfo) {
          for (var p = 0; p < 32; ++p) {
            if (outMask & (1 << p)) {
              (outByPort[p] || []).forEach(function(eid) {
                drawSimRoutingPath(eid, color);
              });
            }
            if (inMask & (1 << p)) {
              (inByPort[p] || []).forEach(function(eid) {
                drawSimRoutingPath(eid, color);
              });
            }
          }
