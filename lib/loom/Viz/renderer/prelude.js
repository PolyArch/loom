...
function buildMappingIndex() {
  ...
  MAPPING_DATA.node_mappings.forEach(function(m) {
    ...
    if (m.hw_node !== undefined && m.hw_node !== null) {
      if (!idx.hwNodeToSwNodes[m.hw_node]) idx.hwNodeToSwNodes[m.hw_node] = [];
      idx.hwNodeToSwNodes[m.hw_node].push(m.sw_node);
    }
...
function buildPERouteIndex() {
  ...
  routes.forEach(function(route) {
    var swEdges = (route.sw_edges || route.swEdges || []).slice();
    var primarySwEdge = route.sw_edge !== undefined
      ? route.sw_edge
      : (swEdges.length > 0 ? swEdges[0] : undefined);
    var normalized = {
      swEdge: primarySwEdge,
      swEdges: swEdges,
