function routePEInternal(edgeG, peLabel, peX, peY, peW, peH,
  ...
  var fuRegions = [];
  (fuBoxes || []).forEach(function(box, fi) {
    if (!box) return;
    fuRegions.push({
      name: String(fi),
      x: peX + box.x,
  ...
  var innerLeft = peX + 24;
  var innerRight = peX + peW - 24;
  var innerTop = peY + 24;
  var innerBottom = peY + peH - 24;
  if (fuRegions.length > 0) {
    innerLeft = fuRegions[0].x;
    innerRight = fuRegions[0].x + fuRegions[0].w;
    innerTop = fuRegions[0].y;
    innerBottom = fuRegions[0].y + fuRegions[0].h;
    fuRegions.forEach(function(region) {
      innerLeft = Math.min(innerLeft, region.x);
      innerRight = Math.max(innerRight, region.x + region.w);
      innerTop = Math.min(innerTop, region.y);
      innerBottom = Math.max(innerBottom, region.y + region.h);
    });
  }

  var corridor = {
    top: Math.max(peY + 14, innerTop - 18),
    bottom: Math.min(peY + peH - 14, innerBottom + 18),
    left: Math.max(peX + 14, innerLeft - 18),
    right: Math.min(peX + peW - 14, innerRight + 18)
  };

  function simplifyPoints(path) {
  ...
  function interiorStub(port, dist) {
    if (!port) return null;
    if (port.side === "left") return { x: port.x + dist, y: port.y };
    if (port.side === "right") return { x: port.x - dist, y: port.y };
    if (port.side === "top") return { x: port.x, y: port.y + dist };
    if (port.side === "bottom") return { x: port.x, y: port.y - dist };
    return { x: port.x, y: port.y };
  }

  function getFUStub(port, direction) {
    if (!port) return null;
    return {
      x: port.x,
      y: port.y + (direction === "in" ? -10 : 10)
    };
  }

  function centeredOffset(ordinal, count, pitch) {
    if (count <= 1) return 0;
    return (ordinal - (count - 1) / 2) * pitch;
  }
  ...
  peConnections.forEach(function(conn) {
    ...
    if (!pePort) return;
    var groupKey = conn.direction + ":" + pePort.side;
    groupCounts[groupKey] = (groupCounts[groupKey] || 0) + 1;
  });
  var groupOrdinals = {};

  function buildOrthPath(conn, routeStart, routeEnd, pePort) {
    var groupKey = conn.direction + ":" + pePort.side;
    var ordinal = groupOrdinals[groupKey] || 0;
    groupOrdinals[groupKey] = ordinal + 1;
    var offset = centeredOffset(ordinal, groupCounts[groupKey] || 1, 7);
    var startStub = conn.direction === "in"
      ? interiorStub(routeStart, 10)
      : getFUStub(routeStart, "out");
    var endStub = conn.direction === "in"
      ? getFUStub(routeEnd, "in")
      : interiorStub(routeEnd, 10);
    if (!startStub || !endStub)
      return simplifyPoints([routeStart, routeEnd]);

    var pts = [
      { x: routeStart.x, y: routeStart.y },
      startStub
    ];
    if (pePort.side === "top" || pePort.side === "bottom") {
      var laneY = (pePort.side === "top" ? corridor.top : corridor.bottom) + offset;
      pts.push({ x: startStub.x, y: laneY });
      pts.push({ x: endStub.x, y: laneY });
    } else if (pePort.side === "left" || pePort.side === "right") {
      var laneX = (pePort.side === "left" ? corridor.left : corridor.right) + offset;
      pts.push({ x: laneX, y: startStub.y });
      pts.push({ x: laneX, y: endStub.y });
    } else {
      var midX = Math.round((startStub.x + endStub.x) / 2);
  ...
  peConnections.forEach(function(conn) {
    ...
    var pts = buildOrthPath(conn, routeStart, routeEnd, pePort);
...
function drawActiveSwitchRoutes(parentG) {
  ...
  switchRouteIdx.routes.forEach(function(route) {
    ...
    parentG.append("line")
      ...
      .attr("stroke", "rgba(200,214,229,0.78)")
      .attr("stroke-width", 1.9)
      .attr("stroke-linecap", "round")
      .attr("opacity", 0.95)
      .style("filter", "drop-shadow(0 0 2px rgba(200,214,229,0.18))")
...
function collectUsedVisibleEdges() {
  ...
  if (!primitiveIdx) return [];
  var edges = [];

  if (primitiveIdx.moduleEdges)
    edges = edges.concat(primitiveIdx.moduleEdges);

  if (primitiveIdx.memoryBindings) {
    primitiveIdx.memoryBindings.forEach(function(binding) {
      (binding.visibleEdges || []).forEach(function(edge) {
        edges.push({
          fromKey: edge.fromKey,
          toKey: edge.toKey,
          swEdges: binding.swEdges || []
        });
      });
    });
  }
...
function onDFGEdgeClick(fromIdx, toIdx) {
  ...
  if (DFG_DATA && DFG_DATA.edges && MAPPING_DATA && MAPPING_DATA.edge_routings) {
    ...
    if (dfgEdge) {
      highlightDFGEdge(dfgEdge.id);
      highlightRoutingPath(dfgEdge.id);
    }
  }
}

function onADGEdgeClick(fromKey, toKey) {
  if (!mappingEnabled) return;
  var primitiveIdx = ensureRoutingPrimitiveIndex();
  if (!primitiveIdx) return;
  ...
  function recordSwEdges(swEdges) {
    (swEdges || []).forEach(function(swEdgeId) {
      matchedSwEdges[swEdgeId] = true;
    });
  }

  (primitiveIdx.moduleEdges || []).forEach(function(primitive) {
    if (primitive.fromKey === fromKey && primitive.toKey === toKey)
      recordSwEdges(primitive.swEdges);
  });

  (primitiveIdx.memoryBindings || []).forEach(function(primitive) {
    (primitive.visibleEdges || []).forEach(function(edge) {
      if (edge.fromKey === fromKey && edge.toKey === toKey)
        recordSwEdges(primitive.swEdges);
    });
  });

  Object.keys(matchedSwEdges).forEach(function(swEdgeKey) {
    var swEdgeId = parseInt(swEdgeKey, 10);
    highlightRoutingPath(swEdgeId);
    highlightDFGEdge(swEdgeId);
    if (DFG_DATA && DFG_DATA.edges) {
      var edgeInfo = DFG_DATA.edges.find(function(edge) {
        return edge.id === swEdgeId;
      });
      if (edgeInfo) {
        highlightDFGNode(edgeInfo.from);
        highlightDFGNode(edgeInfo.to);
      }
    }
  });
}
...
function highlightSwitchRoutesForEdge(swEdgeId, color) {
  ...
  renderADG._g.selectAll(".sw-route-base[data-sw-edge='" + swEdgeId + "']")
    .classed("cross-highlight-hw-edge", true)
    .attr("stroke", c)
...
function highlightPERoutesForEdge(swEdgeId, color) {
  ...
  renderADG._g.selectAll(".pe-route-arrow[data-sw-edge='" + swEdgeId + "']")
    .classed("cross-highlight-hw-arrow", true)
    .attr("fill", c)
    .attr("opacity", 1);
}
...
function highlightMemoryInterfaceBinding(compName, color) {
  ...
  if (primitiveIdx && primitiveIdx.memoryBindings) {
    primitiveIdx.memoryBindings.forEach(function(binding) {
      if (binding.component !== compName) return;
      binding.visibleEdges.forEach(function(edge) {
        highlightHWEdge(edge.fromKey, edge.toKey, c);
        if (edge.fromKey) highlightModulePort(edge.fromKey, c);
        if (edge.toKey) highlightModulePort(edge.toKey, c);
        found = true;
      });
    });
  }
...
function highlightRoutingPrimitive(primitive, swEdgeId, color) {
  if (!primitive) return;
  var c = color || "#4ecdc4";

  if (primitive.kind === "module_edge") {
    if (primitive.fromKey) highlightModulePort(primitive.fromKey, c);
    if (primitive.toKey) highlightModulePort(primitive.toKey, c);
    if (primitive.fromKey && primitive.toKey)
      highlightHWEdge(primitive.fromKey, primitive.toKey, c);
    return;
  }

  if (primitive.kind === "switch_route") {
    highlightSwitchRoutesForEdge(swEdgeId, c);
    return;
  }

  if (primitive.kind === "component_route") {
    highlightPERoutesForEdge(swEdgeId, c);
    if (primitive.componentPortKey) highlightModulePort(primitive.componentPortKey, c);
    if (primitive.functionUnitPortKey) highlightFUPort(primitive.functionUnitPortKey, c);
    return;
  }
  ...
  if (primitive.kind === "memory_binding") {
    ...
    if (primitive.component) highlightADGComp(primitive.component);
    return;
  }

  if (primitive.kind === "function_unit_internal") {
    if (primitive.component && primitive.functionUnit)
      highlightADGFU(primitive.component, primitive.functionUnit);
  }
}
...
function highlightRoutingPath(swEdgeId) {
  ...
  var primitiveIdx = ensureRoutingPrimitiveIndex();
  var primitives = primitiveIdx && primitiveIdx.byEdge
    ? primitiveIdx.byEdge[swEdgeId]
    : null;
  if (primitives && primitives.length > 0) {
    primitives.forEach(function(primitive) {
      highlightRoutingPrimitive(primitive, swEdgeId, color);
    });
    return;
  }
...
function highlightHWEdge(fromKey, toKey, color) {
  ...
  renderADG._g.selectAll(".hw-edge-vis").each(function() {
    ...
    if (portsMatch(ef, fromKey) && portsMatch(et, toKey)) {
      el.classed("cross-highlight-hw-edge", true)
        .attr("stroke", c).attr("stroke-width", 3).attr("opacity", 1);
    }
  });
}
...
function setMappingEnabled(on) {
  ...
  if (typeof refreshSimulationPlayback === "function")
    refreshSimulationPlayback();
}
...
function highlightDFGEdge(edgeId) {
  var dfgEl = document.getElementById("svg-dfg");
  if (!dfgEl) return;
  var edgeG = dfgEl.querySelector("g.dfg-edge[data-dfg-edge='" + edgeId + "']");
  if (!edgeG) return;
  edgeG.querySelectorAll(".dfg-edge-path").forEach(function(el) {
    el.classList.add("cross-highlight-dfg-edge");
  });
  edgeG.querySelectorAll(".dfg-edge-arrow").forEach(function(el) {
    el.classList.add("cross-highlight-dfg-arrow");
  });
}
...
function clearCrossHighlight() {
  if (renderADG._g) {
    ...
    renderADG._g.selectAll(".cross-highlight-hw-edge")
      .classed("cross-highlight-hw-edge", false);
    renderADG._g.selectAll(".cross-highlight-hw-arrow")
      .classed("cross-highlight-hw-arrow", false);
  ...
  var dfgEl = document.getElementById("svg-dfg");
  if (dfgEl) {
    dfgEl.querySelectorAll(".cross-highlight-dfg-edge").forEach(function(el) {
      el.classList.remove("cross-highlight-dfg-edge");
    });
    dfgEl.querySelectorAll(".cross-highlight-dfg-arrow").forEach(function(el) {
      el.classList.remove("cross-highlight-dfg-arrow");
    });
