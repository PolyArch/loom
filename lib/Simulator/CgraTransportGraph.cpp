...
using StorageBinding = CgraTransportGraph::StorageBinding;
using OperandQueueUnitBinding = CgraTransportGraph::OperandQueueUnitBinding;
using OperandBufferBinding = CgraTransportGraph::OperandBufferBinding;
using OperandQueueBinding = CgraTransportGraph::OperandQueueBinding;
...
llvm::Expected<CgraTransportGraph> freezeCgraTransportGraph(
  ...
  const auto advanceTraversal =
    ...
    if (selected.storageKind != CgraTraversalStorageKind::None) {
      ...
      TraversalNodeKind kind = TraversalNodeKind::BufferedStorage;
    ...
    for (const CgraTraversalUsePlan &use :
      ...
      actions.insert({TraversalNodeKind::PhysicalAction, use.physicalUseOrdinal,
                      physicalTagOrdinal});
    }
    if (actions.empty())
      return predecessors;
  ...
  for (const CgraPeOperandBufferPlan &buffer : plan.transport.operandBuffers) {
    ...
    const std::size_t queueCount = buffer.contract.logicalQueues().size();
    const std::uint32_t unitCount = buffer.contract.allocationUnitCount();
    operandBuffers.push_back(
        {buffer.pe, buffer.contract,
  ...
  for (auto &[key, builder] : builders) {
    ...
    for (const auto &[action, predecessors] : builder.traversalPredecessors) {
      ...
      const bool physical = action.kind == TraversalNodeKind::PhysicalAction;
      const std::uint64_t physicalUseOrdinal =
          physical ? action.ordinal : invalidCgraTransportOrdinal;
      const std::uint64_t storageOrdinal =
          physical ? invalidCgraTransportOrdinal : action.ordinal;
    ...
    for (const SinkBuilder &sink : builder.sinks) {
      ...
      if (const auto *operand =
        ...
        if (queueProjection != projectedQueueSinks.end()) {
          ...
          if (insertedUnit) {
            operandQueueUnits.push_back({projected.queue.context.pe,
    ...
    bindings.push_back({builder.producer, sinkOffset,
                        ...
                        semanticActorOrdinal, builder.discard});
  }
  ...
  CgraTransportGraph result;
  result.bindings = std::move(bindings);
  result.sinks = std::move(sinks);
  result.publications = std::move(publications);
  result.publicationSinks = std::move(publicationSinks);
  result.physicalUses = std::move(physicalUses);
  result.traversalNodes = std::move(traversalNodes);
  result.traversalTargets = std::move(traversalTargets);
  result.traversalSuccessors = std::move(traversalSuccessors);
  result.storages = std::move(storages);
  result.operandBuffers = std::move(operandBuffers);
  result.operandQueueUnits = std::move(operandQueueUnits);
  result.operandQueues = std::move(operandQueues);
  ...
  for (const auto &[key, binding] : result.actorSourceBindings) {
    assert(key.first < result.actorSourceBindingOrdinals.size() &&
           "CGRA actor source binding must name a prepared actor");
    result.actorSourceBindingOrdinals[key.first].push_back(binding);
  }
  for (auto &bindings : result.actorSourceBindingOrdinals) {
    llvm::sort(bindings);
    bindings.erase(std::unique(bindings.begin(), bindings.end()),
                   bindings.end());
  }
  result.tagVirtualChannelKeys =
      internPhysicalTagChannelRanks(plan.transport.physicalTags);
  for (const TraversalNodeBinding &origin : result.traversalNodes) {
    if (origin.kind != TraversalNodeKind::BufferedStorage ||
        origin.storageOrdinal >= result.storages.size())
      continue;
    for (std::uint64_t downstream : origin.downstreamStorageNodes) {
      if (downstream >= result.traversalNodes.size())
        continue;
      const std::uint64_t targetStorage =
          result.traversalNodes[downstream].storageOrdinal;
      if (targetStorage >= result.storages.size() ||
          targetStorage == origin.storageOrdinal)
        continue;
      result.storages[targetStorage].upstreamStorageOrdinals.push_back(
          origin.storageOrdinal);
    }
  }
  for (StorageBinding &storage : result.storages) {
