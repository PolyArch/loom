...
function collectUsedVisibleEdges() {
  ...
  return primitiveIdx && primitiveIdx.moduleEdges
    ? primitiveIdx.moduleEdges.slice()
    : [];
}
...
function onADGEdgeClick(fromKey, toKey) {
  ...
  Object.keys(primitiveIdx.byEdge).forEach(function(swEdgeKey) {
    var swEdgeId = parseInt(swEdgeKey, 10);
    var primitives = primitiveIdx.byEdge[swEdgeId] || [];
    for (var i = 0; i < primitives.length; ++i) {
      var primitive = primitives[i];
      if (primitive.kind !== "module_edge") continue;
      if (primitive.fromKey !== fromKey || primitive.toKey !== toKey) continue;
      highlightRoutingPath(swEdgeId);
      highlightDFGEdge(swEdgeId);
      if (DFG_DATA && DFG_DATA.edges) {
        var edgeInfo = DFG_DATA.edges.find(function(edge) { return edge.id === swEdgeId; });
        if (edgeInfo) {
