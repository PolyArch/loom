...
function collectUsedVisibleEdges() {
  return routingPrimitiveIdx && routingPrimitiveIdx.moduleEdges
    ? routingPrimitiveIdx.moduleEdges.slice()
    : [];
}
...
function onADGEdgeClick(fromKey, toKey) {
  if (!mappingEnabled || !routingPrimitiveIdx || !routingPrimitiveIdx.byEdge) return;
  Object.keys(routingPrimitiveIdx.byEdge).forEach(function(swEdgeKey) {
    var swEdgeId = parseInt(swEdgeKey, 10);
    var primitives = routingPrimitiveIdx.byEdge[swEdgeId] || [];
...
function highlightMemoryInterfaceBinding(compName, color) {
  ...
  if (routingPrimitiveIdx && routingPrimitiveIdx.memoryBindings) {
    routingPrimitiveIdx.memoryBindings.forEach(function(binding) {
...
function highlightRoutingPath(swEdgeId) {
  ...
  var primitives = routingPrimitiveIdx && routingPrimitiveIdx.byEdge
    ? routingPrimitiveIdx.byEdge[swEdgeId]
    : null;
