...
var routingPrimitiveIdx = null; // unified visual routing primitives derived from mapping
...
var currentMode = mappingEnabled ? "overlay" : "sidebyside";
...
function buildSwitchRouteIndex() {
  ...
  var routes = routingPrimitiveIdx && routingPrimitiveIdx.switchRoutes &&
      routingPrimitiveIdx.switchRoutes.length > 0
    ? routingPrimitiveIdx.switchRoutes
    : (MAPPING_DATA ? (MAPPING_DATA.switch_routes || deriveSwitchRoutesFromPaths()) : []);
...
function buildPERouteIndex() {
  ...
  if (!routingPrimitiveIdx || !routingPrimitiveIdx.componentRoutes) return idx;

  routingPrimitiveIdx.componentRoutes.forEach(function(route) {
    var normalized = {
      swEdge: route.swEdges[0],
      swEdges: (route.swEdges || []).slice(),
      peName: route.component,
      direction: route.direction,
      pePortKey: route.componentPortKey,
      fuPortKey: route.functionUnitPortKey
    };
    idx.routes.push(normalized);
