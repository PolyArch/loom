...
function renderADG() {
  ...
  (((typeof ADG_LAYOUT_DATA !== "undefined") && ADG_LAYOUT_DATA &&
    ...
    explicitLayoutByName[entry.name] = entry;
  });
  (((typeof ADG_LAYOUT_DATA !== "undefined") && ADG_LAYOUT_DATA &&
    ...
    if (!route || !route.from || route.from_port === undefined ||
      ...
      return;
    var fromKey = route.from === "module_in"
      ? "module_in_" + route.from_port
      : route.from + "_out_" + route.from_port;
    var toKey = route.to === "module_out"
      ? "module_out_" + route.to_port
      : route.to + "_in_" + route.to_port;
  ...
  var skipDenseModuleEdges = !!(adgRenderHints && adgRenderHints.denseScene && !mappingEnabled);
  var rowY = compStartY;
  ...
  if (skipDenseModuleEdges) {
    g.append("text")
      .attr("x", modX + modW - 10)
      .attr("y", modY + 16)
      .attr("text-anchor", "end")
      .attr("fill", "rgba(155,89,182,0.65)")
      .attr("font-size", "9px")
      .text("compact mode: module edges hidden");
  }
  ...
  d3.select("#status-bar").text(
    adgRenderHints && adgRenderHints.denseScene && !mappingEnabled
      ? "Ready (compact ADG detail, module edges hidden)"
      : "Ready"
  );
