function renderMiniFUGraph(parentG, item) {
  ...
  function isStaticMuxOp(opIdx) {
    var opName = fu.ops && fu.ops[opIdx] ? fu.ops[opIdx] : "";
    return opName.indexOf("static_mux") >= 0;
  }
  function staticMuxMode(opIdx) {
    if (!isStaticMuxOp(opIdx)) return null;
  ...
  function styleStaticMuxEdge(edge) {
    ...
    if (edge.dstDesc.kind === "op_in" && isStaticMuxOp(edge.dstDesc.index)) {
    ...
    if (edge.srcDesc.kind === "op_out" && isStaticMuxOp(edge.srcDesc.index)) {
