function renderMiniFUGraph(parentG, item) {
  ...
  var configFieldsByOpIndex = {};
  if (fuConfig && fuConfig.fields) {
    fuConfig.fields.forEach(function(field) {
      if (!field || field.op_index === undefined)
        return;
      if (!configFieldsByOpIndex[field.op_index])
        configFieldsByOpIndex[field.op_index] = [];
      configFieldsByOpIndex[field.op_index].push(field);
      if (!field.kind || field.kind === "mux")
        muxFieldByOpIndex[field.op_index] = field;
    });
  }

  function opConfigLabel(opIdx) {
    var fields = configFieldsByOpIndex[opIdx] || [];
    for (var fi = 0; fi < fields.length; fi++) {
      var field = fields[fi];
      if (!field || field.kind === "mux") continue;
      return field.display || String(field.value);
    }
    return null;
  }
    ...
    if (muxField) {
      ...
      if (cfgLabel) {
        group.append("text")
          ...
          .attr("text-anchor", "middle")
          .attr("fill", "#ffd166")
          .attr("font-size", "4.8px")
          .text(shortenText(cfgLabel, 16));
      }
    }
  }
}
