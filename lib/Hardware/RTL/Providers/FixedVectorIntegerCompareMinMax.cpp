...
llvm::Error invalid(const llvm::Twine &message) {
  return llvm::createStringError(
      ...
      "portable_fixed_vector_integer_compare_min_max_invalid: " + message);
}
...
llvm::Expected<Predicate> modePredicate(const Mode &mode) {
  ...
  case Schema::ArithCmpI: {
    ...
    if (!payload)
      return invalid("comparison behavior has no typed predicate");
...
llvm::Expected<LoweredMode> lowerMode(const Mode &mode) {
  ...
  auto values = llvm::dyn_cast<mlir::VectorType>(mode.actor.type.getInput(0));
  if (!values || values.isScalable() || values.getRank() == 0 ||
      mode.actor.type.getInput(1) != values)
    return invalid("behavior does not have two matching fixed-vector inputs");
  auto element = llvm::dyn_cast<mlir::IntegerType>(values.getElementType());
  if (!element || element.getWidth() == 0)
    return invalid("behavior vector element is not a nonzero integer");
  ...
  const bool comparison = mode.actor.schema == Schema::ArithCmpI;
  auto result = llvm::dyn_cast<mlir::VectorType>(mode.actor.type.getResult(0));
  if (!result || result.isScalable() || result.getShape() != values.getShape())
    return invalid("behavior result has a different vector shape");
  if (comparison) {
    if (!result.getElementType().isInteger(1))
      return invalid("comparison behavior result is not one bit per lane");
  } else if (result != values) {
    return invalid("min/max behavior result changed the element type");
  }
  ...
  return LoweredMode{*loweredPredicate, element.getWidth(),
                     static_cast<unsigned>(lanes), comparison};
}
...
mlir::Value materializeMode(mlir::OpBuilder &builder, mlir::Location location,
  ...
  for (unsigned lane = 0; lane < mode.laneCount; ++lane) {
    ...
    mlir::Value condition = circt::comb::ICmpOp::create(
        builder, location, mode.predicate, lhs, rhs, true);
    laneResults.push_back(
        mode.comparison ? condition
                        : circt::comb::MuxOp::create(
                              builder, location, condition, lhs, rhs, true));
  }
...
materializePortableFixedVectorIntegerCompareMinMax(
  ...
  if (!std::holds_alternative<::fabric::FixedVectorIntegerCompareMinMaxParams>(
  ...
  if (request.capability.configurationFieldSchema.empty()) {
    if (relation->kind() != ::fabric::FabricOpSemanticFieldRelationKind::None)
      return invalid(
          "configuration-free compare/min-max relation is not fieldless");
  ...
  for (const Mode &mode : modes) {
    ...
    const std::uint64_t inputPayload =
        static_cast<std::uint64_t>(lowered->elementWidth) * lowered->laneCount;
    const std::uint64_t resultPayload =
        static_cast<std::uint64_t>(
            lowered->comparison ? 1 : lowered->elementWidth) *
        lowered->laneCount;
    if (inputPayload > inputs[0]->payloadWidthBits ||
        inputPayload > inputs[1]->payloadWidthBits ||
        resultPayload > outputs[0]->payloadWidthBits)
