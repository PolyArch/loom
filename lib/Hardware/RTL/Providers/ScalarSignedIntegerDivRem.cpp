...
llvm::Error invalid(const llvm::Twine &message) {
  return llvm::createStringError(
      ...
      "portable_scalar_signed_integer_div_rem_invalid: " + message);
}
...
materializePortableScalarSignedIntegerDivRem(
  ...
  if (descriptor.familyId != family)
    return invalid("generated family descriptor has the wrong identity");
  ...
  if (!parameters || !parameters->integerWidths.valid() ||
      parameters->integerWidths.empty() || !parameters->pointerFormats.valid())
    return invalid("capability has malformed scalar integer parameters");

  std::vector<Schema> enabled;
  enabled.reserve(request.capability.enabledOperationSchemas.size());
  for (Schema schema : request.capability.enabledOperationSchemas) {
  ...
  circt::hw::HWModuleOp::create(
      ...
      [&](mlir::OpBuilder &bodyBuilder,
        ...
        for (unsigned width : widths) {
          ...
          mlir::Value signedMinimum = circt::hw::ConstantOp::create(
              bodyBuilder, location, llvm::APInt::getSignedMinValue(width));
          mlir::Value negativeOne = circt::hw::ConstantOp::create(
              bodyBuilder, location, llvm::APInt::getAllOnes(width));
          mlir::Value divisorIsZero = circt::comb::ICmpOp::create(
              bodyBuilder, location, circt::comb::ICmpPredicate::eq, rhs, zero,
              true);
          mlir::Value dividendIsMinimum = circt::comb::ICmpOp::create(
              bodyBuilder, location, circt::comb::ICmpPredicate::eq, lhs,
              signedMinimum, true);
          mlir::Value divisorIsNegativeOne = circt::comb::ICmpOp::create(
              bodyBuilder, location, circt::comb::ICmpPredicate::eq, rhs,
              negativeOne, true);
          mlir::Value quotientOverflows = circt::comb::AndOp::create(
              bodyBuilder, location, dividendIsMinimum, divisorIsNegativeOne,
              true);
          mlir::Value quotientIsNonDefined = circt::comb::OrOp::create(
              bodyBuilder, location, divisorIsZero, quotientOverflows, true);
          mlir::Value safeDivisor = circt::comb::MuxOp::create(
              bodyBuilder, location, quotientIsNonDefined, one, rhs, true);
          mlir::Value rawQuotient = circt::comb::DivSOp::create(
              bodyBuilder, location, lhs, safeDivisor, true);
          mlir::Value product = circt::comb::MulOp::create(
              bodyBuilder, location, mlir::ValueRange{rawQuotient, safeDivisor},
              true);
          ...
          mlir::Value quotient = circt::comb::MuxOp::create(
              bodyBuilder, location, quotientIsNonDefined, zero, rawQuotient,
              true);
