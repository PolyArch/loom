...
llvm::Error bundleError(const llvm::Twine &message) {
  return invocationBundleError(message);
}
...
validateSpecification(const ExternalToolInvocationBundleSpec &specification) {
  ...
  for (const ResolvedAuxiliaryToolExecutable &tool :
    ...
    if (tool.localFileKey.empty() || containsNull(tool.localFileKey))
      return bundleError("auxiliary tool local key is invalid");
    if (tool.absolutePath.empty() || containsNull(tool.absolutePath))
      return bundleError("auxiliary tool path is empty or contains NUL");
...
llvm::Expected<InvocationManifestData> parseManifest(llvm::StringRef contents) {
  ...
  if (*version != kAuxiliaryToolCommandManifestVersion &&
    ...
    return bundleError("invocation manifest before 2.4 cannot contain "
  ...
  if (auxiliaryToolArray) {
    ...
    for (const llvm::json::Value &value : *auxiliaryToolArray) {
      ...
      if (!tool)
        return bundleError("auxiliary tool record must be an object");
      if (llvm::Error error = rejectUnknownFields(
              *tool, "auxiliary tool",
...
importExternalToolInvocationAttemptImpl(
  ...
  if (execution) {
    if (!receiptState)
      return bundleError("execution observation has no executor receipt");
  ...
  if (*currentManifest != bundle->manifestBytes)
    return bundleError("invocation manifest changed during import");
  ...
  if ((*completion)->attemptToken != *initialToken)
    return bundleError("completion belongs to another attempt generation");
