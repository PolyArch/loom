...
ProjectionKey
attachmentDispositionKey(const FrozenSpatialAttachmentOption &attachment) {
  ProjectionKey key;
  key.reserve(18);
...
SpatialBindingRelationModel::create(
  ...
  // remain shareable through their tag/row projection; a Spatial switch
  // traversal retains MappingResident route claims. Derive this distinction
  // from the frozen routing graph rather than treating endpoint dispersion as
  // a generic feasibility rule.
  if (ports.graphBoundaries().size() > 1) {
    const auto endpointHasResidentSwitchUse =
        [&](PnrIndex endpoint, bool source) -> llvm::Expected<bool> {
      if (endpoint >= routing.routingEndpoints().size() ||
          endpoint + 1 >= routing.adjacencyOffsets().size() ||
          endpoint + 1 >= routing.reverseAdjacencyOffsets().size())
        return invalid(Projection::SpatialTransferAttachment,
                       "graph-boundary endpoint is outside routing CSR");
      const auto traversalIsResidentSwitch =
          [&](PnrIndex traversal) -> llvm::Expected<bool> {
        if (traversal >= routing.traversals().size())
          return invalid(Projection::SpatialTransferAttachment,
                         "graph-boundary arc names a foreign traversal");
        const FrozenSpatialTraversal &record = routing.traversals()[traversal];
        return std::holds_alternative<FabricSwitchTraversalPayload>(
                   record.reference.payload) &&
               record.routeClaimCount != 0;
      };
      if (source) {
        for (PnrIndex arc = routing.adjacencyOffsets()[endpoint];
             arc < routing.adjacencyOffsets()[endpoint + 1]; ++arc) {
          if (arc >= routing.routingArcs().size())
            return invalid(Projection::SpatialTransferAttachment,
                           "graph-boundary outgoing arc is out of range");
          auto resident =
              traversalIsResidentSwitch(routing.routingArcs()[arc].traversal);
          if (!resident)
      ...
      for (PnrIndex cursor = routing.reverseAdjacencyOffsets()[endpoint];
           cursor < routing.reverseAdjacencyOffsets()[endpoint + 1]; ++cursor) {
        if (cursor >= routing.reverseArcOrdinals().size())
          return invalid(Projection::SpatialTransferAttachment,
                         "graph-boundary reverse arc is out of range");
        const PnrIndex arc = routing.reverseArcOrdinals()[cursor];
        if (arc >= routing.routingArcs().size())
          return invalid(Projection::SpatialTransferAttachment,
                         "graph-boundary incoming arc is out of range");
        auto resident =
            traversalIsResidentSwitch(routing.routingArcs()[arc].traversal);
        if (!resident)
          return resident.takeError();
    ...
    InitializerRelationInput exclusiveSpatialBoundaryEndpoints;
    exclusiveSpatialBoundaryEndpoints.kind = InitializerRelationKind::Disjoint;
    ...
    for (PnrIndex boundary = 0; boundary < ports.graphBoundaries().size();
      ...
      const bool source =
          std::holds_alternative<::dataflow::GraphIngressTokenRef>(
              record.terminal);
      const PnrIndex decision = graphBoundaryDecisionOffset + boundary;
      const auto choices =
      ...
      for (PnrIndex option : choices) {
        if (option >= ports.attachmentOptions().size())
          return invalid(Projection::SpatialTransferAttachment,
                         "graph-boundary exclusivity names a foreign option");
        const PnrIndex endpoint = ports.attachmentOptions()[option].endpoint;
        auto exclusive = endpointHasResidentSwitchUse(endpoint, source);
        if (!exclusive)
          return exclusive.takeError();
        ProjectionKey key;
        if (*exclusive) {
          key.push_back(0);
          appendComponent(key,
                          canonicalFabricBytes(
                              routing.routingEndpoints()[endpoint].reference));
          conflictMembers[key].insert(decision);
        } else {
          key.push_back(1);
          appendU32Be(key, decision);
          appendU32Be(key, option);
        }
        keys.push_back(std::move(key));
      }
      exclusiveSpatialBoundaryEndpoints.members.push_back({decision, {}});
      memberKeys.push_back(std::move(keys));
    }
    const bool conflictPossible =
        llvm::any_of(conflictMembers,
    ...
    if (conflictPossible) {
      ...
      if (universe.size() > getPnrIndexMax())
        return invalid(Projection::SpatialTransferAttachment,
                       "graph-boundary exclusivity domain overflows "
                       "PnrIndex");
      for (std::size_t member = 0; member < memberKeys.size(); ++member) {
        auto &values =
            exclusiveSpatialBoundaryEndpoints.members[member].projectedValues;
