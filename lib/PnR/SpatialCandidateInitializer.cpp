...
llvm::Error
loom::pnr::detail::spatialInitializerError(const llvm::Twine &message) {
...
using loom::pnr::detail::spatialInitializerError;
...
public:
  ...
  SpatialCandidateInitialization initialization() const {
    return {computeBindings_,
            ...
            memoryOperationPlans_,
            logicalMemoryBindings_,
            memoryUseDispatches_,
            memoryExposureSelections_,
            {}};
  }
...
private:
  ...
  llvm::Error appendDecision(DecisionKind kind, PnrIndex index,
    ...
    if (choiceCapacity >
      ...
      return spatialInitializerError("dependent choice storage size overflows");
  ...
  llvm::Error prepareDecisionInventory() {
    ...
    if (portAttachments_.size() != ports.portDemands().size())
      return spatialInitializerError(
          "root decision solver omitted PortAttachments");
    ...
    for (PnrIndex exposure = 0; exposure < memory.exposures().size();
      ...
      if (memory.exposureOptions().size() > getPnrIndexMax())
        return spatialInitializerError("memory exposure domain is too large");
  ...
  llvm::Expected<PnrIndex> fillChoices(const DecisionRecord &decision) {
    ...
    case DecisionKind::MemoryOperationPlan: {
      ...
      if (!domain)
        return spatialInitializerError("memory operation domain disappeared");
    ...
    case DecisionKind::MemoryUseDispatch: {
      ...
      if (!domain)
        return spatialInitializerError("memory dispatch domain disappeared");
      ...
      if (group != getInvalidPnrIndex()) {
        if (group >= memory.serviceUseGroups().size())
          return spatialInitializerError(
              "memory dispatch has a foreign use group");
    ...
    if (count > decision.choiceCapacity)
      return spatialInitializerError(
          "dependent choice domain exceeds its frozen cap");
    return count;
  }
  ...
  llvm::Expected<DependentSearchResult> search() {
    ...
    if (selected == std::numeric_limits<std::size_t>::max())
      return spatialInitializerError(
          "dependent decision prerequisites form a cycle");
...
loom::pnr::createSpatialCandidateInitializerAttempt(
  ...
  if (!problem)
    return spatialInitializerError("FrozenSpatialPnrProblem owner is null");
  const auto &policy = problem->config().policy();
  if (attemptOrdinal >= policy.search.initializer.seedAttemptCount)
    return spatialInitializerError(
        "initializer attempt ordinal is out of range");
  ...
  auto preferred = detail::preferScheduleAwareRootPlacements(
  ...
  SpatialCandidateInitialization initialization = builder.initialization();
  auto registerFifoTransfers =
      detail::derivePreferredSpatialLocalTransferSelections(
          *problem, initialization.computeBindings);
  if (!registerFifoTransfers)
    return registerFifoTransfers.takeError();
  initialization.registerFifoTransfers = *registerFifoTransfers;
