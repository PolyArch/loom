...
SpatialActionExecutorScratch::prepare(SpatialCandidateState &candidate,
  ...
  capacityProofDebtWitness_.competingLogicalNets.clear();
  capacityProofDebtWitness_.competingLogicalNets.reserve(netMarks_.size());
  capacityProofDebtWitness_.routeAnchors.clear();
  routeCostTraversals_.clear();
  ...
  localTransferClaimWords_.reserve(localTransferClaimBits_.size());
...
void SpatialActionExecutorScratch::markChangedBindingRoot(PnrIndex decision) {
  if (decision >= relationDecisionMarks_.size() ||
      relationDecisionMarks_[decision] == dependencyEpoch_)
    return;
  relationDecisionMarks_[decision] = dependencyEpoch_;
...
void SpatialActionExecutorScratch::markExplicitAttachment(PnrIndex decision) {
  if (decision < explicitAttachmentMarks_.size())
    explicitAttachmentMarks_[decision] = dependencyEpoch_;
}
...
llvm::Error SpatialActionExecutorScratch::markWholeNet(
  ...
  if (llvm::Error error = markNet(action.logicalNet))
    return error;
  explicitNetDispositionMarks_[action.logicalNet] = dependencyEpoch_;
...
llvm::Error SpatialActionExecutorScratch::markWitnessRegion(
  ...
  case ResolvedPnrViolationKind::ProgressProofDebt: {
    ...
    if (llvm::Error error = candidate_->rebuildCapacityProofDebtWitness(
      ...
      return error;
    for (PnrIndex logicalNet : capacityProofDebtWitness_.competingLogicalNets) {
      if (llvm::Error error = markNet(logicalNet))
        return error;
    }
    if (capacityProofDebtWitness_.competingLogicalNets.empty() ||
        capacityProofDebtWitness_.routeAnchors.empty())
    ...
    for (PnrIndex relation : bindings.decisionRelations(decision)) {
      ...
      for (const detail::InitializerRelationMember &member :
        ...
        if (member.decision < bindings.portDecisionOffset() ||
            relationDecisionMarks_[member.decision] == dependencyEpoch_)
          continue;
        relationDecisionMarks_[member.decision] = dependencyEpoch_;
...
std::size_t SpatialActionExecutorScratch::retainedStorageBytes() const {
  return candidateScratch_.retainedStorageBytes() +
         ...
         retainedBytes(capacityProofDebtWitness_.competingLogicalNets) +
         retainedBytes(capacityProofDebtWitness_.routeAnchors) +
         retainedBytes(routeCostTraversals_) +
         retainedBytes(routeCostLogicalNets_) +
         retainedBytes(localTransferClaimBits_) +
         retainedBytes(localTransferClaimWords_) +
