...
prepare(llvm::ArrayRef<CandidateGeneratorInputBinding> inputs,
  ...
  auto driver = renderDesignCompilerDriver(
      invocation->source.top(), {rtlPath.str()}, invocation->constraints,
...
llvm::Expected<ArtifactRootReference> publish(const InvocationFacts &facts,
  ...
  auto netlist = parseDesignCompilerGateNetlist(verilog, facts.source.top());
  if (!netlist)
    return netlist.takeError();
  auto digest = blobs.put(bytesOf(netlist->verilog));
  if (!digest)
