...
executeJointRepairPlan(const JointDesignExplorationPlan &plan,
                       ...
                       const ArtifactStore &artifacts, const BlobStore &blobs) {
  // Selection and its invocation-local attempt must come from the same
  // controller for both stopping policies. The repair plan already names its
  // exact hardware frontier, so this invocation cannot reopen hardware.
