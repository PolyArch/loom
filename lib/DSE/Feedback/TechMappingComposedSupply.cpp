//===- TechMappingComposedSupply.cpp - mined supply for a Hall deficit ---===//
//
// A compute-context Hall deficit has three answers. Two of them redistribute
// what a Module already offers, and this file owns the third: composing a new
// capability from the common subgraph of the software the cover refused. It
// selects a mined template and sizes its occurrences; it authors no hardware
// and describes no FU structure.
//
//===----------------------------------------------------------------------===//
...
llvm::Error invalid(const llvm::Twine &message) {
  return llvm::createStringError(llvm::inconvertibleErrorCode(),
                                 "tech_mapping_composed_supply_invalid: " +
                                     message);
}

/// Spatial PE sites one deficit needs when each realization of the proposal
...
std::uint32_t sizeOccurrences(std::uint64_t deficit,
                              std::uint64_t actorsPerRealization,
                              std::uint32_t spatialPeCount) {
  if (spatialPeCount == 0 || actorsPerRealization == 0)
    return 0;
  const std::uint64_t needed =
      (deficit + actorsPerRealization - 1) / actorsPerRealization;
  return static_cast<std::uint32_t>(
      std::min<std::uint64_t>(std::max<std::uint64_t>(needed, 1),
                              spatialPeCount));
}
...
proposeMinedCompositeFuSupply(
  ...
  if (feedback.deficit() == 0)
    return invalid("composed supply requires a positive deficit");
  ...
  for (const CompositeFuCandidate &candidate : mined->candidates) {
    ...
    const std::uint32_t occurrences =
        sizeOccurrences(feedback.deficit(), candidate.nodes.size(),
