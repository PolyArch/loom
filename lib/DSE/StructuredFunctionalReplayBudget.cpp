...
planStructuredFunctionalReplayBudget(
  ...
  if (!budget.maxWavefrontSteps || !budget.maxEventCount) {
    ...
    auto activity = evaluation::models::projectStructuredScopeActivationCounts(
        candidate.structuredProgram, *observations, regions);
    if (!activity)
      return activity.takeError();
    for (const auto &region : *activity) {
      auto sum = llvm::checkedAddUnsigned(activations, region);
      if (!sum)
    ...
    auto waves = llvm::checkedMulUnsigned(planningActivations,
                                          defaultWavefrontStepsPerActivation);
    auto events = llvm::checkedMulUnsigned(planningActivations,
                                           defaultEventsPerActivation);
