...
constexpr llvm::StringLiteral kGeneratedCarrierMetadata =
    "loom.relocatable_payload.compiler_generated";
...
llvm::Expected<bool>
hasGeneratedRelocatablePayloadCarrier(const llvm::Module &module) {
  const llvm::GlobalVariable *carrier = nullptr;
  for (const llvm::GlobalVariable &global : module.globals()) {
    if (!global.hasSection() ||
        global.getSection() != relocatablePayloadCarrierSection())
      continue;
    if (carrier != nullptr)
      return rejected("payload_carrier_ambiguous: the LLVM module contains "
                      "more than one payload carrier");
    carrier = &global;
  }
  if (carrier == nullptr)
    return false;
  if (carrier->getMetadata(kGeneratedCarrierMetadata) == nullptr)
    return rejected("payload_carrier_untrusted: the LLVM module contains a "
                    "payload carrier not generated by this owner");
  return true;
}
...
void embedRelocatablePayloadCarrier(
  ...
  carrier->setMetadata(kGeneratedCarrierMetadata,
...
removeGeneratedRelocatablePayloadCarriers(llvm::Module &module) {
  llvm::SmallVector<llvm::GlobalVariable *, 4> carriers;
  std::vector<std::vector<std::uint8_t>> contents;
  for (llvm::GlobalVariable &global : module.globals()) {
    if (!global.hasSection() ||
        global.getSection() != relocatablePayloadCarrierSection())
      continue;
    if (global.getMetadata(kGeneratedCarrierMetadata) == nullptr)
      return rejected("payload_carrier_untrusted: the linked LLVM module "
                      "contains a carrier not generated by this owner");
    if (!global.hasInitializer())
      return rejected("payload_carrier_unreadable: the linked LLVM carrier "
                      "has no initializer");
    const auto *bytes =
        llvm::dyn_cast<llvm::ConstantDataSequential>(global.getInitializer());
    if (bytes == nullptr || !bytes->getElementType()->isIntegerTy(8))
      return rejected("payload_carrier_unreadable: the linked LLVM carrier "
                      "is not a constant byte sequence");
    const llvm::StringRef raw = bytes->getRawDataValues();
    contents.emplace_back(raw.bytes_begin(), raw.bytes_end());
    carriers.push_back(&global);
  }

  llvm::removeFromUsedLists(module, [&](llvm::Constant *entry) {
    const llvm::Value *value = entry->stripPointerCasts();
    return llvm::is_contained(carriers, value);
  });

  for (llvm::GlobalVariable *carrier : carriers) {
    carrier->removeDeadConstantUsers();
    if (!carrier->use_empty())
      return rejected("payload_carrier_semantic_use: a linked LLVM carrier "
                      "has a use outside llvm.used or llvm.compiler.used");
