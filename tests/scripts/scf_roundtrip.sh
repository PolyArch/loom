#!/usr/bin/env bash
# SCF Roundtrip Test
# Compiles each C++ app to LLVM IR + SCF MLIR via loom, lowers through mlir-opt/mlir-translate,
...
if [[ "${1:-}" == "--single" ]]; then
  ...
  log_file="${output_dir}/${app_name}.scf.log"
...
PARALLEL_FILE="${APPS_DIR}/scf_roundtrip.parallel.sh"
...
for app_dir in "${app_dirs[@]}"; do
  ...
  line+=" && ${rel_mlir_opt} ${rel_out}/${app_name}.scf.mlir --test-lower-to-llvm -o - | ${rel_mlir_translate} --mlir-to-llvmir > ${rel_out}/${app_name}.scf.ll"
  line+=" && ${rel_clangxx} ${rel_out}/${app_name}.scf.ll -o ${rel_out}/${app_name}.scf.exe -lm"
  if [[ "${RUN_APPS}" == "true" ]]; then
    line+=" && ${rel_out}/${app_name}.scf.exe"
  fi
...
loom_print_summary "SCF Roundtrip"
loom_write_result "SCF Roundtrip"
