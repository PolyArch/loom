// 3x3 switch lattice mesh with 4 pe_subf PEs.

module {
  fabric.pe @pe_subf(%arg0: !dataflow.bits<32>, %arg1: !dataflow.bits<32>) -> (!dataflow.bits<32>) {
  ^bb0(%a: f32, %b: f32):
  ...
  fabric.module @subf(
    ...
    %pe_cell00 = fabric.instance @pe_subf(%sw00#2, %sw10#3)
        {sym_name = "pe_0_0"}
        : (!dataflow.bits<32>, !dataflow.bits<32>) -> !dataflow.bits<32>
    %pe_cell01 = fabric.instance @pe_subf(%sw01#3, %sw11#4)
        {sym_name = "pe_0_1"}
        : (!dataflow.bits<32>, !dataflow.bits<32>) -> !dataflow.bits<32>
    %pe_cell10 = fabric.instance @pe_subf(%sw10#4, %sw20#2)
        {sym_name = "pe_1_0"}
        : (!dataflow.bits<32>, !dataflow.bits<32>) -> !dataflow.bits<32>
    %pe_cell11 = fabric.instance @pe_subf(%sw11#5, %sw21#3)
        {sym_name = "pe_1_1"}
        : (!dataflow.bits<32>, !dataflow.bits<32>) -> !dataflow.bits<32>
