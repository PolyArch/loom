// Dual-mesh fabric: 32-bit input mesh + 64-bit output mesh, bridged by extui PEs.

module {
  fabric.pe @pe_extui(%arg0: !dataflow.bits<32>) -> (!dataflow.bits<64>) {
  ^bb0(%a: i32):
    %0 = arith.extui %a : i32 to i64
    fabric.yield %0 : i64
  }
  ...
  fabric.module @extui(
    ...
    // extui PEs (32-bit in, 64-bit out)
    %pe0 = fabric.instance @pe_extui(%sw0#1)
        {sym_name = "pe_0_0"}
        : (!dataflow.bits<32>) -> !dataflow.bits<64>

    %pe1 = fabric.instance @pe_extui(%sw1#1)
        {sym_name = "pe_0_1"}
        : (!dataflow.bits<32>) -> !dataflow.bits<64>
