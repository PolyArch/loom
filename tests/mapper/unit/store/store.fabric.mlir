// Fabric with store PE (handshake.store exclusive body).

module {
  fabric.pe @pe_store(%arg0: !dataflow.bits<57>, %arg1: !dataflow.bits<32>,
                      %arg2: none) -> (!dataflow.bits<32>, !dataflow.bits<57>) {
  ^bb0(%addr: index, %data: i32, %ctrl: none):
    %dataResult, %addressResult = handshake.store [%addr] %data, %ctrl : index, i32
    fabric.yield %data, %addr : i32, index
  }
  ...
  fabric.module @store(
    ...
    // store PE: (index, i32, none) -> (i32, index)
    %pe0:2 = fabric.instance @pe_store(%sw_a#0, %sw_d#0, %sw_c#0)
        {sym_name = "pe_0_0"}
    ...
    fabric.yield %sw_out_a#0, %sw_out_d#0 : !dataflow.bits<57>, !dataflow.bits<32>
  }
}
