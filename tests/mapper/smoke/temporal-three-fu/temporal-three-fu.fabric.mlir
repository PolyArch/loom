// Temporal-three-fu fabric: 3-FU opcode derivation test.
// 1 TPE with fu_add + fu_mul + fu_sub (3 FU types), num_instruction=4.
// Tagged domain: bits<32> with i4 tags.
//
// FU body order: fu_add=0, fu_mul=1, fu_sub=2.
// Opcode must match body order, not instruction slot.
//
// 2x2 switch mesh:
//       col0        col1
// row0: ST00 ------ ST01
...
module {
  // Temporal PE: add + mul + sub FUs, 4 instruction slots
  fabric.temporal_pe @tpe_alu3(
    ...
    fabric.pe @fu_sub(%a: i32, %b: i32) -> (i32) {
  ...
  fabric.module @temporal_three_fu(
    ...
    // ======== TPE Instance ========
    %tpe = fabric.instance @tpe_alu3(%st00#2, %st10#2)
        {sym_name = "tpe_0"}
        : (!dataflow.tagged<!dataflow.bits<32>, i4>,
           !dataflow.tagged<!dataflow.bits<32>, i4>)
          -> !dataflow.tagged<!dataflow.bits<32>, i4>
