...
// CHECK: COMP_TEMPORAL_PE_SRC_COUNT

// 2 inputs but instruction has 3 sources.
fabric.temporal_pe @tpe_bad(%in0: !dataflow.tagged<i32, i4>, %in1: !dataflow.tagged<i32, i4>)
  [num_register = 2, num_instruction = 2, reg_fifo_depth = 1]
  {instruction_mem = [
    "inst[0]: when(tag=3) out(0, tag=1) = add(0) in(0), in(1), reg(0)"
  ]}
