...
// and a sequential path (switch output 1 -> fifo -> yield 1). The switch
// has route_table enabling all paths, so feedback through the combinational
// path must still be detected.
fabric.module @mixed(%a: !dataflow.bits<32>, %b: !dataflow.bits<32>) -> (!dataflow.bits<32>, !dataflow.bits<32>) {
  %sw:2 = fabric.switch [connectivity_table = [1, 1, 1, 1]] {route_table = [1, 1, 1, 1]} %a, %b : !dataflow.bits<32> -> !dataflow.bits<32>, !dataflow.bits<32>
