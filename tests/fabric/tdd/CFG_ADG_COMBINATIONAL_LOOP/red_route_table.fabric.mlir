// RUN: not loom --adg %s 2>&1 | FileCheck %s
// CHECK: CFG_ADG_COMBINATIONAL_LOOP

// Two switches with route_table enabling all paths form a combinational cycle.
fabric.module @test_comb_loop(%a: !dataflow.bits<32>) -> (!dataflow.bits<32>) {
  %sw0:2 = fabric.switch [connectivity_table = [1, 1, 1, 1]] {route_table = [1, 1, 1, 1]} %a, %sw1#0 : !dataflow.bits<32> -> !dataflow.bits<32>, !dataflow.bits<32>
  %sw1:2 = fabric.switch [connectivity_table = [1, 1, 1, 1]] {route_table = [1, 1, 1, 1]} %sw0#0, %sw0#1 : !dataflow.bits<32> -> !dataflow.bits<32>, !dataflow.bits<32>
  fabric.yield %sw1#1 : !dataflow.bits<32>
}
