...
upper_bound(const uint32_t *restrict input_sorted,
...
int main(void) {
  ...
  upper_bound(input_sorted, input_targets, output_indices, 5u, 5u);
  return (output_indices[0] == 0u && output_indices[1] == 1u &&
          output_indices[2] == 2u && output_indices[3] == 4u &&
          output_indices[4] == 5u)
             ? 0
             : 1;
}
