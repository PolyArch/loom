//===-- topo_mesh_3x3.cpp - ADG test: 3x3 mesh topology --------*- C++ -*-===//
//
...
int main() {
  ADGBuilder builder("topo_mesh_3x3");
  ...
  auto mesh = builder.buildMesh(3, 3, pe, sw, Topology::Mesh);
  ...
  builder.exportMLIR("Output/topo_mesh_3x3.fabric.mlir");

  const char *mlir = "Output/topo_mesh_3x3.fabric.mlir";
  ...
  assert(mlirCount(mlir, "connectivity_table") == 9 && "each switch has connectivity_table");
