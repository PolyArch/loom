//===-- topo_mesh_2x3.cpp - ADG test: 2x3 non-square mesh ------*- C++ -*-===//
//
...
int main() {
  ADGBuilder builder("topo_mesh_2x3");
  ...
  auto mesh = builder.buildMesh(2, 3, pe, sw, Topology::Mesh);

  // Chain PEs: [0][0] -> [0][1] -> [0][2] -> [1][0] -> [1][1] -> [1][2]
  ...
  const char *mlir = "Output/topo_mesh_2x3.fabric.mlir";
  assert(mlirCount(mlir, "fabric.instance") == 6 && "expected 6 PE instances");
  assert(mlirCount(mlir, "fabric.switch") == 6 && "expected 6 switch instances");
  // 5-port switch: connectivity table has 25 entries
  assert(mlirCount(mlir, "connectivity_table") == 6 && "each switch has connectivity_table");
  assert(countInterSwitchEdges(mlir) == 7 && "expected 7 inter-switch edges");
