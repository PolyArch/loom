//===-- topo_diag_torus_4x4.cpp - ADG test: 4x4 diagonal torus -*- C++ -*-===//
//
...
int main() {
  ADGBuilder builder("topo_diag_torus_4x4");
  ...
  auto mesh = builder.buildMesh(4, 4, pe, sw, Topology::DiagonalTorus);
  ...
  builder.exportMLIR("Output/topo_diag_torus_4x4.fabric.mlir");

  // Verify MLIR contains expected instances: 16 PEs and 16 switches (9-port).
  const char *mlir = "Output/topo_diag_torus_4x4.fabric.mlir";
