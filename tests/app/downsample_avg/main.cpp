...
int main() {
    ...
    downsample_avg_cpu(input, expect_output, input_size, factor);
    downsample_avg_dsa(input, calculated_output, input_size, factor);
    
    for (uint32_t i = 0; i < output_size; i++) {
        if (fabsf(expect_output[i] - calculated_output[i]) > 1e-6f) {
            printf("downsample_avg: FAILED\n");
            return 1;
        }
    }
    
    printf("downsample_avg: PASSED\n");
    return 0;
}
