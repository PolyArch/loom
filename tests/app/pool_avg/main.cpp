...
int main() {
    ...
    pool_avg_cpu(input, expect_output, H, W, pool_h, pool_w, stride_h, stride_w);
    
    // Compute result with accelerator version
    pool_avg_dsa(input, calculated_output, H, W, pool_h, pool_w, stride_h, stride_w);
    
    // Compare results
    for (uint32_t i = 0; i < OH * OW; i++) {
        if (fabsf(expect_output[i] - calculated_output[i]) > 1e-5f) {
            printf("pool_avg: FAILED\n");
            return 1;
        }
    }
    
    printf("pool_avg: PASSED\n");
    return 0;
}
