...
int main() {
    ...
    upper_bound_cpu(input_sorted, input_targets, expect_indices, N, M);

    // Compute result with DSA version
    upper_bound_dsa(input_sorted, input_targets, calculated_indices, N, M);

    // Compare results
    for (uint32_t i = 0; i < M; i++) {
        if (expect_indices[i] != calculated_indices[i]) {
            printf("upper_bound: FAILED\n");
            return 1;
        }
    }

    printf("upper_bound: PASSED\n");
    return 0;
}
