...
int main() {
    ...
    upsample_linear_cpu(input, expect_output, input_size, factor);
    upsample_linear_dsa(input, calculated_output, input_size, factor);
    
    for (uint32_t i = 0; i < output_size; i++) {
        if (fabsf(expect_output[i] - calculated_output[i]) > 1e-6f) {
            printf("upsample_linear: FAILED\n");
            return 1;
        }
    }
    
    printf("upsample_linear: PASSED\n");
    return 0;
}
