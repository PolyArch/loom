// One hardware extmemory with ldCount=2 serving two load ports in one region.
module {
  ...
  fabric.module @extmemory_dual_load_test(
      %dram: memref<?xi32>,
    ...
    %tag_addr1 = fabric.add_tag %pe1#1 {tag = 1 : i64}
        : index -> !fabric.tagged<!fabric.bits<57>, i1>
    %tsw_ld_addr:1 = fabric.instance @ld_addr_mux(%tag_addr0, %tag_addr1)
        {sym_name = "tsw_0"}
        : (!fabric.tagged<!fabric.bits<57>, i1>,
           !fabric.tagged<!fabric.bits<57>, i1>)
          -> (!fabric.tagged<!fabric.bits<57>, i1>)
