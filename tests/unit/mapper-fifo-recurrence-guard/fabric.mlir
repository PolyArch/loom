module {
  fabric.spatial_pe @stream_pe(
    ...
    fabric.function_unit @fu_stream(%fu_start: index, %fu_step: index,
                                    %fu_bound: index)
  ...
  fabric.spatial_pe @carry_pe(
      ...
      %next: !fabric.bits<64>)
      -> (!fabric.bits<64>) {
    fabric.function_unit @fu_carry(%fu_cond: i1, %fu_init: index,
                                   %fu_next: index) -> (index)
        [latency = -1, interval = -1] {
  ...
  fabric.module @mapper_fifo_recurrence_guard_test(
      %start: !fabric.bits<64>,
      %step: !fabric.bits<64>,
      %bound: !fabric.bits<64>,
      %init: !fabric.bits<64>) -> (!fabric.bits<64>) {
    ...
    %gate:2 = fabric.instance @gate_pe(%stream#0, %stream#1)
        ...
        : (!fabric.bits<64>, !fabric.bits<1>)
          -> (!fabric.bits<64>, !fabric.bits<1>)
    %fifo = fabric.fifo @fifo_0 [depth = 4, bypassable] (%gate#0)
        attributes {bypassed = true}
        : (!fabric.bits<64>) -> (!fabric.bits<64>)
    %carry = fabric.instance @carry_pe(%gate#1, %init, %fifo)
        {sym_name = "pe_carry"}
