...
def main() -> int:
    ...
    mem_slice = slices.get(("mem_0", "memory"))
    if mem_slice is None:
        raise SystemExit("missing memory slice")
    if not mem_slice["complete"]:
        raise SystemExit("memory slice marked incomplete")

    words = config["words"][
        mem_slice["word_offset"]:mem_slice["word_offset"] + mem_slice["word_count"]
    ]
    expected = [1, 0, 1, 0, 2, 1, 1, 2, 4096, 2]
    if words != expected:
        raise SystemExit(f"unexpected memory addr_offset_table words: {words}")
    ...
    if region.get("memory_kind") != "memory":
        raise SystemExit(f"expected memory region payload, got {region}")
