// One hardware on-chip memory shared by two software memory regions.
module {
  ...
  fabric.module @multi_memory_test(
    ...
    %mem0:2 = fabric.memory @mem_0
        [ldCount = 2, stCount = 0, lsqDepth = 0,
         memrefType = memref<256xi32>, numRegion = 2]
        (%tsw_addr#0)
        : (!fabric.tagged<!fabric.bits<57>, i1>)
          -> (!fabric.tagged<!fabric.bits<32>, i1>,
              !fabric.tagged<none, i1>)

    %tsw_data:2 = fabric.instance @data_demux(%mem0#0)
        {sym_name = "tsw_1"}
        : (!fabric.tagged<!fabric.bits<32>, i1>)
          -> (!fabric.tagged<!fabric.bits<32>, i1>,
              !fabric.tagged<!fabric.bits<32>, i1>)
    %tsw_done:2 = fabric.instance @done_demux(%mem0#1)
        {sym_name = "tsw_2"}
        : (!fabric.tagged<none, i1>)
          -> (!fabric.tagged<none, i1>,
              !fabric.tagged<none, i1>)
