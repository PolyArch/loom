...
def main() -> int:
    ...
    if case_name == "temporal-pe-gui-mul":
        stem = "dfg-mul"
    elif case_name == "temporal-pe-gui-mac":
        stem = "dfg-mac"
    elif case_name == "temporal-pe-gui":
        stem = "dfg"
    else:
        stem = "dfg"
    config_path = out_dir / f"{stem}.config.json"
    ...
    result_width = tag_bits
    instruction_width = 1 + tag_bits + opcode_bits + 3 * input_field_width + output_field_width + result_width

    offset = 0
    valid = read_bits(words, offset, 1)
    offset += 1
    ...
    result_tag = read_bits(words, offset, result_width)

    slot1_offset = instruction_width
    valid1 = read_bits(words, slot1_offset, 1)
    tag1 = read_bits(words, slot1_offset + 1, tag_bits)
    fu_cfg = read_bits(words, instruction_width * 2, 3)

    expected = {
        "temporal-pe-gui": {"opcode": 0, "mux2": (0, False, True), "fu_cfg": 0},
        "temporal-pe-gui-mul": {"opcode": 1, "mux2": (0, False, True), "fu_cfg": 0},
        "temporal-pe-gui-mac": {"opcode": 1, "mux2": (2, False, False), "fu_cfg": 1},
    }[case_name]

    if valid != 1:
        raise SystemExit("instruction valid bit mismatch")
    if tag != 0 or result_tag != 0:
        raise SystemExit("temporal_pe tag fields should be zero in this test")
    ...
    if valid1 != 0 or tag1 != 0:
        raise SystemExit("unused temporal slot should be zeroed")
    if fu_cfg != expected["fu_cfg"]:
        raise SystemExit(f"unexpected temporal FU config {fu_cfg}")

    node_mappings = {entry["sw_op"]: entry["hw_name"] for entry in mapping["node_mappings"]}
    if case_name == "temporal-pe-gui" and node_mappings.get("arith.addi") != "fu_add":
        raise SystemExit("arith.addi should map to fu_add")
    if case_name == "temporal-pe-gui-mul" and node_mappings.get("arith.muli") != "fu_mac":
        raise SystemExit("arith.muli should map to fu_mac")
    if case_name == "temporal-pe-gui-mac":
