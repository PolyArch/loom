module {
  fabric.temporal_pe @tpe_shared(
      ...
      [
        ...
        enable_share_operand_buffer = true,
        operand_buffer_size = 8 : i64
      ] {
    fabric.function_unit @fu_add(%a: i32, %b: i32, %c: i32) -> (i32)
        [latency = 1, interval = 1] {
      %r = arith.addi %a, %b : i32
      %zero = arith.constant 0 : i32
      %unused = arith.addi %c, %zero : i32
      fabric.yield %r : i32
    }
    fabric.yield
  }

  fabric.module @temporal_pe_buffer_shared_test(%a: i32, %b: i32, %c: i32) -> (i32) {
    ...
    %tc = fabric.add_tag %c {tag = 0 : i64}
        : i32 -> !fabric.tagged<!fabric.bits<32>, i1>
    %tout = fabric.instance @tpe_shared(%ta, %tb, %tc) {sym_name = "tpe_0"}
        : (!fabric.tagged<!fabric.bits<32>, i1>,
           !fabric.tagged<!fabric.bits<32>, i1>,
           !fabric.tagged<!fabric.bits<32>, i1>)
          -> (!fabric.tagged<!fabric.bits<32>, i1>)
