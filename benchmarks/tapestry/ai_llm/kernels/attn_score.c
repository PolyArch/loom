 ...
 *
 * Variants: tile32, tile64, causal_masked, flash_tiled.
 */
...
static void attn_score_impl(const float *Q, const float *K, float *score,
...
void attn_score(const float *Q, const float *K, float *score,
                int H, int N, int D) {
    attn_score_impl(Q, K, score, H, N, D, 32);
}

/* --- Tile-size variants --- */

void attn_score_tile32(const float *Q, const float *K, float *score,
                       int H, int N, int D) {
    attn_score_impl(Q, K, score, H, N, D, 32);
}

void attn_score_tile64(const float *Q, const float *K, float *score,
                       int H, int N, int D) {
    attn_score_impl(Q, K, score, H, N, D, 64);
}

/* Causal masked variant: upper triangle zeroed for autoregressive attention */
void attn_score_causal_masked(const float *Q, const float *K, float *score,
                              int H, int N, int D) {
    float scale = 1.0f / sqrtf((float)D);
    for (int h = 0; h < H; h++) {
        const float *Qh = Q + (size_t)h * N * D;
        ...
        TILE_FOR(ti, 0, N, 32) {
            int ti_end = TILE_END(ti, N, 32);
            TILE_FOR(tj, 0, N, 32) {
                int tj_end = TILE_END(tj, N, 32);
                for (int i = ti; i < ti_end; i++) {
                    for (int j = tj; j < tj_end; j++) {
                        if (j > i) {
                            Sh[i * N + j] = -1e9f;
...
/* Flash-tiled variant: fused score computation with larger tiles */
void attn_score_flash_tiled(const float *Q, const float *K, float *score,
                            int H, int N, int D) {
    attn_score_impl(Q, K, score, H, N, D, 64);
}
...
int main(void) {
    ...
    float *S_v64  = (float *)malloc(s_size * sizeof(float));

    if (!Q || !K || !S_tile || !S_ref || !S_v64) {
    ...
    attn_score_tile64(Q, K, S_v64, H, N, D);

    float max_err = 0.0f, max_err64 = 0.0f;
    for (size_t i = 0; i < s_size; i++) {
        float err = fabsf(S_tile[i] - S_ref[i]);
        if (err > max_err) max_err = err;
        err = fabsf(S_v64[i] - S_ref[i]);
        if (err > max_err64) max_err64 = err;
    }

    printf("attn_score: max_error = %e\n", max_err);
    printf("attn_score_tile64: max_error = %e\n", max_err64);
    int pass = (max_err < 1e-3f) && (max_err64 < 1e-3f);
    printf("attn_score: %s\n", pass ? "PASS" : "FAIL");

    free(Q); free(K); free(S_tile); free(S_ref); free(S_v64);
    return pass ? 0 : 1;
}
