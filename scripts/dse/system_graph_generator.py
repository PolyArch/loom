...
def to_system_topology_text(topology_spec: SystemTopologySpec) -> str:
    ...
    This function produces a human-readable textual representation of the
    system topology using fabric.core / fabric.l2bank / fabric.router
    pseudo-ops. The output is intended exclusively for DSE visualization
    and debugging and is NOT valid MLIR.

    Real system MLIR is generated by C++ SystemADGBuilder::build()
    (include/loom/ADG/SystemADGBuilder.h), which composes per-core
    ModuleOps produced by DomainADGGenerator and KHGGenerator into a
    complete system-level fabric.module and exports valid MLIR via
    exportMLIR(). HWOuterOptimizer::generateSystemMLIR() orchestrates
