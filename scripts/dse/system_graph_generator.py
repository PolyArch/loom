...
def to_system_mlir(topology_spec: SystemTopologySpec) -> str:
    """Generate a pseudo-MLIR system description for visualization only.

    WARNING: This function produces pseudo-MLIR text intended exclusively
    for DSE visualization and debugging. The output uses fabricated ops
    (fabric.core, fabric.l2bank, fabric.router) that are NOT part of the
    real Fabric MLIR dialect and cannot be parsed by MLIR tooling.

    Real system MLIR comes from the C++ SystemADGBuilder path:
      - Per-core Fabric MLIR ADGs are generated by DomainADGGenerator
        (include/loom/ADG/DomainADGGenerator.h) for domain-specific types
        D1-D6 and by KHGGenerator (include/loom/ADG/KHGGenerator.h) for
        combinatorial KHG types, both using the ADGBuilder API.
      - The C++ SystemADGBuilder (include/loom/ADG/SystemADGBuilder.h)
        composes these per-core ModuleOps into a complete system-level
        fabric.module with NoC connectivity and shared memory hierarchy,
        then exports valid MLIR via exportMLIR().
      - HWOuterOptimizer::generateSystemMLIR() orchestrates this pipeline
        end to end, resolving type IDs to real ADGs automatically.

    Do NOT use the output of this function as input to the mapper or any
    downstream compilation stage.
    """
    lines = []
